...
def main() -> None:
    ...
    if args.export_obsidian:
        ...
        print(f"[metadata] Создан outputs/youtube_metadata.json, идей заголовков: {len(metadata['title_variants'])}")
    ...
    print("[plans] Созданы quote_plan, scene_plan, asset_plan, render_plan")

    if args.skip_render:
        output_path = render_plan["output_path"]
        eval_result = {"ok": True, "checks": ["Рендер пропущен через --skip-render."], "warnings": []}
