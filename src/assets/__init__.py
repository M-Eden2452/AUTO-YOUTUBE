"""Общий слой визуального материала: провайдеры, права, отбор, завершённость.

Пакет владеет контрактом провайдера (``provider_contract.StockProvider``),
моделями ассета, политикой прав, формированием строки запроса, скачиванием с
валидацией, semantic evidence и лестницей завершённости. Он переиспользуется
workflow и ни одному из них не принадлежит.

Он не владеет сценарием, визуальным планом, voice/TTS, SceneTimeline,
субтитрами, финальной сборкой ролика и состоянием проекта. Второй provider
contract, второй asset pipeline и второй semantic selector не создаются.

Область целиком описана в ``src/assets/README.md``; этот модуль остаётся только
списком публичных re-exports.
"""
