"""Canonical boundary стадии ``quality_check`` workflow ``news_to_short``.

Единственный публичный вход — ``run_quality_check``. Его вызывает одноимённая
стадия в ``src.news.pipeline``. Проверка читает уже созданные манифесты и
ничего не исправляет: её результат — отчёт, а не действие.

Responsibilities:
- сведение проверок сценария, research, ассетов, озвучки и субтитров в один
  отчёт ``status`` / ``errors`` / ``warnings`` / ``checks``;
- разделение errors и warnings: что делает результат непригодным против того,
  что требует человеческого взгляда;
- поддержка двух форм манифеста ассетов — schema v1 и legacy — без миграции
  сохранённых данных;
- перепроверка прав каждого использованного ассета через существующего
  владельца политики.

Does not own:
- саму политику прав — ``src.assets.license_policy.evaluate_asset_policy``
  остаётся авторитетом, здесь она только применяется повторно;
- словарь состояний завершённости и причин блокировки —
  ``src.assets.completion``;
- отбор, замену и скачивание материала — ``src.assets``, ``src.news.asset_*``;
- решение о том, что делать с отчётом: остановиться, показать пользователю или
  продолжить — ``src.news.pipeline``;
- анализ готового файла: длительность, разрешение и переполнение текста в
  выходном MP4 здесь не проверяются.

Important invariants:
- проверка не изменяет ни один манифест и ничего не скачивает;
- ``failed`` возникает только от errors; warnings дают ``needs_review``, а не
  тихий пропуск;
- пустая сцена сценария и unsafe claim остаются errors в любом режиме
  завершённости;
- ``draft_complete`` ослабляет ожидания по покрытию материалом, но не снимает
  требований к правам.

See also: ``src/news/pipeline.py``, ``src/assets/license_policy.py``,
``src/news/models.py``.
"""
