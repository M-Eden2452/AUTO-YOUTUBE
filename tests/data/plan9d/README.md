...
каталоге собираются из **токена сцены** и blind id (`S346ba272cd_C1_v0_0.jpg`);
для клипов кадры снимает `ffmpeg` локально.

Сцена на странице названа непрозрачным токеном `S<10 hex>`, а не `scene_key`:
ключ несёт имя прогона (один из двух — только локальная библиотека) и, у
инцидентной сцены, слаг случая, то есть подсказку до того, как человек посмотрел
на кадры. Разметка возвращается с токеном, обратное отображение делает harness
через `scene_key_index` — на странице обратного отображения нет. Замороженная
разметка v1 сделана по ключам и продолжает читаться: обе записи резолвятся.
...
### Какое число можно цитировать

`measure` печатает четыре строки, и путать их нельзя:
...
- **`blind agreement`** — единственное число про поведение в поле;
- **`incident scenes`** — сцены с требованием, написанным рукой: доказательство про
  механизм, не про поле;
- **`all scenes together`** — сумма, оставленная для сопоставимости со старыми
  записями (на v1 совпадает с первой строкой, инцидентных сцен там нет);
- **`cards not shown to anyone`** — карточки без картинки. Это «не спрашивали», а
  не «владелец отказал», и строка обязана стоять рядом с любым отношением.
