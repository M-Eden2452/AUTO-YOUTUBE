...
**З:** B3.
**СДЕЛАНО** — `src/config_resolver/`, приёмка через
`channels show --channel <id> --explain`. Резолвер только читает и в пайплайн не
подключён; характеристический тест доказывает совпадение с текущими читателями по
всем 24 полям `VoicePolicy` для каждого канала. Карта конфигурации —
`docs/implementation/config_resolver/CONFIG_MAP.md`; осознанные отклонения (порядок
`template` выше канала — как в коде сегодня; добавленный слой `runtime_override`) —
в `AUTONOMOUS_PROGRESS.md`, раздел «Stage D1».
...
**Ремонтный гейт Q2.1 (добавлен после реального прогона Real Shorts E2E-A).** Q1 и Q2
убрали захардкоженные фразы и запросы, но первый сквозной прогон на настоящей теме
показал, что этого мало: план строился осмысленно, а поиск всё равно давал 0 подходящих
материалов из 8. Q2.1 чинит именно retrieval — маршрутизацию провайдеров по типу сцены,
язык запроса под конкретного провайдера, честную оценку по метаданным провайдера,
`must_avoid`, длительность и локальную инфографику. Гейт: **следующий шаблон не
начинать, пока повторный прогон того же проекта не покажет улучшения подбора.** Карта:
`docs/implementation/visual_retrieval_repair/VISUAL_RETRIEVAL_MAP.md`.
