...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C95 | `corpus_sha256` покрывает поля, которые вычисляет код, поэтому правка отбора отвязывает замороженный корпус от кода, а пересчёт — от слепой разметки | **FACT** | Измерено 2026-08-17 при закрытии `C91`. Правка сдвинула ровно одно поле во всём корпусе: `plan9d_current_capture_v1/scene_010` потеряла категорию `ambiguous_needs_review`; победители не менялись нигде (`changed_winners 0`). Хеш при этом уходит с `bfb4d02437f3c528` на `280e6da190b7fb03`, а `current_annotations_v1.json` записывает хеш, против которого делалась разметка, и `plan9d_ground_truth.py:786` отказывается мерить при несовпадении. Пересчёт корпуса осиротил бы единственную размеченную истину репозитория | **ИСПРАВЛЕНО 2026-08-18.** Разведены два утверждения и два хеша внутри существующего владельца `tests/plan9d_ground_truth.py`. `corpus_sha256` остаётся целостностью файла. Новый `annotation_identity_sha256` — дайджест вопроса, на который отвечал человек: требование сцены (`scene_text`, `semantic_scene`, кадр, длительность), идентичность кандидата (`blind_id`, `asset_id`) и контрольные суммы пикселей (`frames`, `visual_evidence`); привязка разметки идёт по нему. Снаружи намеренно: путь к файлу превью (местоположение, а не картинка) и порядок записей (проекция сортирует). Версия проекции входит в дайджест. Оба поля обязательны: провенанс говорит, из какого файла проход получен, идентичность — чему он принадлежит. Приёмка: корпус v1 пересчитан `finalize` и переподписан `bfb4d02437f3c528` → `280e6da190b7fb03`, дайджест идентичности при этом **не изменился** (`04b33c9bad935d5a`), разметка осталась привязанной, `measure` даёт прежние **4 / 14** и тех же победителей. `KNOWN_DERIVED_DRIFT` пуст: пин снят той же правкой, что убрала его причину. Второй прибор, второй корпус и второй реестр не заводились | Закрыта. Владелец — `tests/plan9d_ground_truth.py`; тот же вопрос для корпуса v2 закрывается тем же дайджестом, потому что доска и шаблон берут его оттуда же |
...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C96 | прогон, который собирается платно озвучивать, не просит класс сети, без которого озвучка теперь отказывает | **FACT** | Пакет **E** (`d7fb378`, строка `C94`) закрыл дыру, поставив `require_network(voice_synthesis)` внутрь `ElevenLabsProvider.synthesize`. Декларация действий прогона за ним не пошла: `src/content_creation/wizard_state.py:163-164` при `voice_provider == "elevenlabs"` добавляет только `NETWORK_ACTION_VOICE_PREFLIGHT`; `voice_synthesis` не появляется ни в одном вызывающем. Мастер показывал список действий без того, на котором прогон и остановится. Симптом: `test_news_voice_adapter.py:138,194` давали `partially_completed`; `d7fb378` заявил targeted по четырём другим модулям, этот в набор не попал | **ИСПРАВЛЕНО 2026-08-17.** `required_network_actions` объявляет оба класса вместе: двухфазный платный поток запоминает `allow_network` в первой фазе и второй раз не спрашивает, поэтому «попросить синтез позже» невозможно. Одобрение сети не одобряет трату — платный гейт второй и независимый. Запрещено: переиспользовать `voice_preflight` вместо нового класса — инвариант `runtime_network` «одобренный класс не открывает соседние» именно это и запрещает; второй сетевой контур; ослабление проверки в провайдере | Закрыта; независимое ревью 2026-08-18 (`docs/audits/REVIEW_C96_S5_2026-08-18.md`) — Scope PASS · Objective PASS, ноль BLOCKER/MAJOR; строка была помечена закрытой автором **до** ревью, и этот разрыв ревью закрывает. Страж — не форма кортежа, а достаточность: тест выдаёт ровно то, что показал мастер, и требует прохождения `voice_synthesis` внутри этого одобрения. Прежний тест замораживал неполный список и был зелёным при сломанном прогоне |
...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C98 | бриф теряет тему видео: `subject` и `provider_queries` сцены не обязаны сохранять предмет, ради которого сцена существует | **FACT** | Измерено 2026-08-18 на замороженном корпусе v2. Из **42** уникальных запросов **15** не содержат ни `solar`, ни `photovoltaic`, ни `panel`, ни русских форм; **13** из них имеют `source: explicit_override`, то есть пришли из `visual_brief.provider_queries` (`src/assets/query_adapter.py:470-486`), **2** — из полей того же брифа, и **ни одного** — из `INTENT_CONTEXT_FALLBACK` детерминированного планировщика. В артефакте прогона `topic_entity` = «панель», при этом `scene_003.visual_brief.subject` = `factory production line`, `scene_004.visual_brief.subject` = `battery pack`; backend — платный `openai`. Следствие в пуле: по `battery pack` пришли аккумуляторы шуруповёрта и батареи Canon, по `factory machines industrial production line` — стеклозавод. В двух сценах (`live_5/scene_004`, `local_after_fix/scene_004`) приемлемого по предмету кандидата в пуле нет вообще, поэтому ни вес, ни порог их не спасают; в третьей сцене этого класса (`local_after_fix/scene_003`) пул засорён, но правильный ролик в нём есть и вылетает по порогу — одного якоря там мало. Граница ожиданий записана числом: в шести сценах из восьми несовпавших предпочтённый владельцем кандидат в пуле был и проиграл ранжированию | **ИСПРАВЛЕНО 2026-08-18** одним слайсом на существующих владельцах. `src/assets/query_adapter.py` получил `TopicAnchor` и `plan_topic_anchor(visual_plan)`: английская форма темы читается либо из самого `topic_entity`, когда он английский, либо из английских `subject`/`exact_entities` сцен **того же плана**, повторяющихся минимум в двух сценах. Для обоих прогонов якорь вышел `solar panel` из русского «панель» — это английское свидетельство плана, а не перевод. Каждый английский запрос, не называющий якорь, получает его в начало (остальная часть запроса — свидетельство сцены — сохраняется), и правка пишется в `notes` вместе с исходной строкой. K9 не ослаблен: русский `topic_entity` не переводится, а русский запрос (локальная библиотека) якорем не склеивается — смешанный скрипт не ищется нигде. Когда тема названа, но английской формы в плане нет, запрос не уходит молча: сцена получает нерассылаемую запись `query_subject_unverified`. Якорь считается один раз на план (`AssetManifestBuilder.topic_anchor`) и передаётся и в `build_scene_queries`, и в `build_slot_queries` — сузить слот до «sunset» этот путь больше не может. Второй orchestrator, планировщик и provider contract не созданы; Vision не задействован | **Закрыта 2026-08-18.** Перепись на тех же замороженных сценах через самого владельца (`tests/test_query_topic_anchor.py`, корпус v2): **15 из 42 → 0**; число «до» осталось воспроизводимым — тот же прогон без якоря по-прежнему даёт 15 из 42. Уникальных запросов 42 → 41: один якорённый совпал с уже имевшимся и шлётся один раз, новых запросов якорь не добавляет; 27 запросов, уже называвших тему, не изменились. Guard «отбор не тронут» зелёный: `blind agreement` 2 / 10 на v2 и 4 / 14 на v1 — до и после. Что **не** заявлено: улучшение отбора. По разбору правка двигает 2–3 сцены из восьми, в шести нужный кандидат и так лежал в пуле и проигрывал ранжированию (`C99`); продуктовый эффект на пуле покажет только новый живой прогон — платное действие и отдельное решение владельца, в этом слайсе не делалось. Что **не** покрыто: локальная библиотека приходит в пул вообще без запроса (пустой `search_query`) и этой правкой не затронута; «подтверждённый эквивалент» из пункта 1 ADR 0022 (`solar panel` ↔ `photovoltaic module`) не реализован — совпадение считается по общей основе слова, ровно как считает перепись; путь `build_slot_queries` заякорен, но переписью не измерен — он проверен модульно
| C99 | ранжирование не различает кадры одинаковой релевантности: план кадра и композиция не участвуют ни в оценке, ни в требовании | **FACT** | Измерено 2026-08-18 там же. `live_5/scene_001`: система взяла крупный план (sem 100), владелец — общий план поля (sem 88.2), при этом слово `establishing` стоит в самом запросе. `local_after_fix/scene_005`: у обоих кандидатов sem 76.5, расходится только крупность. `local_after_fix/scene_002`: три локальных ролика с одинаковым sem 64.7, исход решает порядок подачи. Класс равен по размеру потере предмета — три сцены из восьми несовпавших | **НЕ исправлялось и не назначается этой строкой.** Кандидатов решения два, и выбор между ними — owner decision: признак крупности плана в требовании и оценке (дёшево, офлайн) либо визуальная проверка top-кандидатов (`PD-2`, раздел 8 PRODUCT_PLAN — Vision как последний арбитр). Запрещено: чинить это порогами и весами вслепую — до правки не измерено, что именно различает предпочтение владельца | Закрывается решением владельца о признаке и его измерением на том же приборе |
...
Строка записывает его и не чинит — владелец у него свой.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C100 | пин sha корпуса v1 в тесте гейта отстал от самого корпуса, и тест красный на HEAD | **FACT** | `tests/test_plan9d_retrieval_gate.py:53` держит `EXPECTED_CORPUS_SHA256 = bfb4d02…`, а поле `corpus_sha256` в `tests/data/plan9d/current_corpus_v1.json` равно `280e6da…`. Расхождение внёс `20f02cd` («пересчитать производные поля корпуса v1»): корпус пересчитан, пин рядом не обновлён. `test_gate_runs_with_no_socket_available` падает, остальные 28 тестов самого `tests/test_plan9d_retrieval_gate` зелёные (в прогоне пяти наборов plan9d — 230 из 231). Проверено `git stash push -u` — падает и на чистом HEAD; независимое ревью `7b6a169` перепроверило и подтвердило, что коммит к падению не причастен | **НЕ исправлялось.** Владелец существующий: `tests/test_plan9d_retrieval_gate.py`. Правка на вид однострочная, но пин — это якорь «корпус не дрейфует», и обновлять его можно только вместе с ответом на вопрос, законен ли дрейф, внесённый `20f02cd`. Запрещено: обновить число, не назвав, что именно в корпусе изменилось | Закрывается слайсом, который либо обновляет пин вместе с записью о причине дрейфа, либо возвращает корпус к запинованному состоянию |
