...
- **C79–C81** — findings серии FIRST OWNER SHORT, перенесены 2026-08-14 от HEAD
  ...
  исправлялись, владельца `C79` и `C81` этот перенос не назначает (`C79`
  исправлен позднее, 2026-08-15 — состояние см. в самой строке);
...
| N03 | `assets/`, `manual_assets/`, `music/` | `split` | **изменено ревизией 2 (OWNER).** 287 + 18 + 3 файла; смешанные runtime media и versioned resources | **keep:** `assets/library/metadata/media_index.json` (provenance/rights), versioned SVG в `manual_assets/**`. **delete:** всё медиа, кэши. `assets/voice_samples` — **disposable (OD-3)**; минимально необходимый sample активного voice profile переносится во внешний Workspace с provenance, иначе удаляется. **Частично закрыто 2026-08-13 (owner decision, раньше 14D/14E):** `music/` закрыт — каталог не удалён, а переехал в `assets/music/` (`8200e12`), потому что это исходное медиа, а не runtime-мусор; легаси-медиа `assets/` вынесено в карантин строкой **R05** (`f98cd43`); `manual_assets/` ретайрен строкой **R06**. **Остаток на 14D/14E:** `assets/library` — **изменено 2026-08-14 слайсом CURATED LOCAL LIBRARY:** 85 файлов и 86 записей индекса, из них 72 курируемых (провенанс, права, описание кадра — versioned в `assets/library/metadata/curated_library.json`) и 14 `legacy_unreviewed`; сам `media_index.json` остаётся runtime-артефактом вне Git и пересобирается из манифеста командой `python -m tools.library.curated_index apply`, `assets/cache/retrieval_lab` (evidence EXP-001), `assets/voice_samples` (OD-3) | ~~14D/14E~~ **частично closed → R05/R06**, остаток 14D/14E |
...
| C21 | `assets/broll/.gitkeep` | Git index ∩ ignore | **FACT** | директорное правило `assets/broll/` обесценивает последующее `!assets/broll/.gitkeep`; файл tracked и ignored одновременно | правило заменено на `assets/broll/*` **в PLAN-14F** (единственный slice с `.gitignore` в allowed zones); `git ls-files -i -c` не содержит `.gitkeep`. PLAN-6B — только detector. **Closed 2026-08-13 иначе, чем планировалось:** правка правила в PLAN-14F не понадобилась — вместе с ретайром каталога (**R05**, `f98cd43`) исчезли и `assets/broll/.gitkeep`, и оба правила `assets/broll/` и `!assets/broll/.gitkeep`. Проверено: `git ls-files -i -c --exclude-standard` пуст | ~~PLAN-6B → 14F~~ **closed → R05** |
...
| C24 | hardcoded `G:/` в versioned config | `config/`, `channels/` | **FACT** | `config/video_style.json` — `moss_tts_path`, `vault_path`; `config/channels/psychology/style.json` — `moss_tts_path` | **оба носителя умирают в L3:** `config/video_style.json` и `config/channels/psychology/` ретайрятся вместе с legacy-стеком, поэтому отдельного исправления не требуется. Если после L3 hardcoded drive обнаружится в выжившем versioned config — резолвить через существующий resolver или env | **PLAN-L3**, остаток → PLAN-14B |
...
| C49 | subprocess network-guard measurement | **FACT** | `tests/network_guard.py` живёт внутри test-пакета и дочерним процессом **не наследуется**. На audit HEAD `adcbb19` subprocess-модулей **12** (ранее записано 7). Это **measurement, не invariant**. **Пересчитано 2026-08-17 (пакет T):** Python-child порождают **7** модулей (`grep -l "sys\.executable" tests/*.py`) при **21**, где вообще встречается `subprocess` (`grep -l subprocess tests/*.py`) — записанные выше 12 относятся к `adcbb19` и current fact'ом не читаются. Природа строки не меняется: это то же измерение на другом HEAD, а не закрытие | зафиксировать как измерение; архитектурное решение по kill-switch **сейчас не принимается** — механизм и owner остаются implementation-time решением. **PLAN-6B остаётся report/measurement owner в своей текущей границе** | **PLAN-6B** (measure) + позднее решение owner |
...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C79 | русская морфология: extraction стеммит, matching — нет | **FACT** | Два слоя одной системы расходятся. Extraction имеет реальный стеммер со списком окончаний — `src/content/visual_planning/entities.py:94` (`stem`), группировка по стему и выбор субъекта по salience (`:207-236`: topic `+3.0`, title `+1.5`, claim `+1.0`, сортировка `(-salience, stem)`). Evidence-матчинг имеет только префиксное отношение от пяти символов — `src/assets/semantic_selection/evidence.py:181` (`stem_match`), явно документированное как «spelling allowance rather than a guess». Живое следствие измерено: сцены 002 и 003 солнечного прогона получили один и тот же subject `панель` и разошлись с содержанием сценария | **Owner decision 2026-08-14:** pre-v1 bounded correction после STOCK diagnostic и до M4/PLAN-11, characterization-first. Existing implementation owner — `src/assets/semantic_selection/evidence.py`; он переиспользует существующий `entities.stem`. Запрещено: второй стеммер, второй словарь морфологии, отдельный «RU-путь» отбора. Пороги и веса этой строкой не назначаются. **ИСПРАВЛЕНО ЧАСТИЧНО 2026-08-15** characterization-first, RED получен на production path (`rank_candidates`), не на helper'е; пороги (`MIN_SCORE`, `EXACT_SUBJECT_MIN_SCORE`) и веса не менялись. `evidence.morphological_variant` держит два отношения вместо одного: прежнее префиксное **и** равенство стемов существующего `entities.stem` (импорт ленивый — на уровне модуля он замкнул бы цикл `assets` → `content` → `news` → `assets`). Аддитивность проверена с обеих сторон: `панель`/`панелей` не имеют общего префикса и совпадают по стему, `antarctica`/`antarctic` не имеют общего стема и совпадают по префиксу, `sampling`/`samples` отвергаются обоими. Второй стеммер, второй словарь окончаний и отдельный «RU-путь» не создавались; `entities._ENDINGS` не трогался (для `панель`/`панелей` и `сборка`/`сборки` его хватило). Закрыто ровно то, что спрашивает стемовый примитив: subject-слот перешёл из `missing` в `matched`, `required_slot_missing` снят, verdict стал `complete`, а `conflicting_context` — уже использовавший этот примитив — начал ловить свои словоформы (сторона усиления запрета, зафиксирована тестом). Литеральное осталось литеральным: `must_include`, `must_not_include` и `requirement`-слоты автора не тронуты и закрыты тестами. **Открытая половина вынесена в `C89`, а не объявлена закрытой** (сам `C89` закрыт отдельным слайсом 2026-08-15)**:** исход отбора эта правка не меняет, потому что средневзвешенная оценка, гейтящая `score_below_*`, считается литеральным примитивом. Тесты: `tests/test_metadata_evidence_repair.py::RussianMorphologyMatchingTest` — закрытая половина и открытая закреплены обе. **Не заявляется**, что retrieval улучшен: все числа offline, live-измерение эту правку не проверяло | pre-v1 bounded correction; не `PLAN-10C` (бюджет поиска) и не `C40`/`PLAN-10D` (глобальная локальная библиотека) |
| C80 | платный Vision идёт мимо default-deny сети | **FACT** | `src/runtime_network.py` объявляет default-deny по шести классам (`provider_search`, `asset_download`, `preview_download`, `article_fetch`, `voice_preflight`, `semantic_brief`), и все шесть проходят через `require_network`. Vision — нет: `src/assets/semantic_visual_openai.py:440` создаёт `OpenAI(...)` напрямую, `require_network` в файле не вызывается, класса под Vision в `NETWORK_ACTIONS` не существует. Единственный сторож — paid-гейт `VisionBudgetGuard` (`:170`, фраза `LIVE_VISION_APPROVED`). Практически сейчас не стреляет: Vision выключен четырьмя независимыми гейтами `config/semantic_visual.json` (`enabled:false`, `backend:"mock"`, `semantic_rerank_enabled:false`, `allow_paid_vision:false`). Приписка 2026-08-17: классов теперь **семь** — пакет E добавил `voice_synthesis` (`C94`); сам дефект не изменился, Vision класса по-прежнему не имеет | `правка` в существующем владельце `runtime_network`: Vision получает свой network-класс, paid-гейт остаётся отдельным и вторым. Второй network-контур не создаётся. Смежно `C65` (legacy-стеки мимо `require_network`), но случай **другой**: там legacy-путь, здесь — канонический и ещё не включённый | **PLAN-9E** (латентное расхождение: маршрут обещает раздельные network и paid gates до включения Vision) |
...
**Semantic-brief live limitation (owner decision 2026-08-14, исправлено
2026-08-15).** STOCK repeat остаётся следующим действием, но этот docs-слайс его
не разрешает и не запускает: отдельный execution prompt обязан явно назвать
network/paid scopes. Описание платных границ, данное здесь 2026-08-14, было
неточным и заменено строками `C84`–`C87` ниже: `maximum_calls_per_project` —
жёсткий pre-call предел на число платных запросов **проекта**, точный ровно
настолько, насколько правдива запись на диске (residual-окна — `C86`),
`maximum_budget_usd` — **оценочный** pre-call guard из
`estimated_cost_per_call_usd`, а не гарантия суммы счёта провайдера; SDK
по-прежнему вызывается с `max_retries=0`. Это остаётся зоной существующего
semantic-brief live owner, новый PLAN-ID не создавался.
...
## LIVE-5 findings (C83–C87, C89–C90)

Внесено 2026-08-15 по итогам живого retrieval-прогона LIVE-5
(`docs/audits/LIVE_5_2026-08-15.md`, HEAD `68c46cd`). Новый PLAN-ID не
создавался, номер VA-NEW не выдавался, статусы PLAN-10A/10B/10C/10D не менялись.
`C87` найден там же — независимым review правки `C84`/`C85` от HEAD `3475d5e`.
`C89` найден при исправлении `C79` 2026-08-15 и является его второй половиной;
номер `C88` не выдавался. `C89` исправлен 2026-08-15 отдельным bounded slice;
`C90` — след этого исправления (примитив, оставшийся без вызывающих), внесён тем
же слайсом как запись, а не как разрешение на удаление.
LIVE-5 — owner packaging label, не plan step. `C83` **не исправлялся**. `C84` и
два дефекта, найденные при его разборе (`C85`, `C86`), исправлены 2026-08-15
одним characterization-first слайсом внутри существующего semantic-brief live
owner; строки ниже сохранены как запись дефекта и его закрытия.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C83 | `use_local_library` — мёртвый ключ конфигурации: локальная библиотека не выключается | **FACT** | Ключ `assets.use_local_library` встречается ровно в одном месте репозитория — `config/channels/nature_science_news_ru/channel_config.json:93`; в коде ссылок на него **ноль**. `rank_local_assets` вызывается безусловно (`src/news/asset_manifest_builder.py:389`) и кладёт локальные кандидаты в общий пул до всякой проверки канала. Измерено в LIVE-5 при `use_local_library:false`: локальные кандидаты участвовали в скоринге сцен 003 и 005 (7.5 и 15.0, все отклонены по порогу), при этом 0 provider attempts, отсутствие в per-scene `ordered_providers` и 0 выбранных слотов. Побочно измерено следствие вместе с `C79`: локальная запись «Автоматизированная линия сборки солнечных панелей» — точный предмет сцены 003 — получила 7.5 и была отклонена, а Opel car dashboard получил 72.9 и выиграл  **Приписка 2026-08-17: продуктовая часть этой evidence устарела — см. абзац под таблицей.**| `правка` внутри существующего владельца локальной библиотеки: либо ключ начинает читаться там, где формируется пул кандидатов, либо ключ удаляется как несуществующая возможность. Второй матчер, второй провайдер и отдельный «local off» путь не создаются; выбор между чтением и удалением ключа — owner decision, эта строка его не выдаёт | **PLAN-10D** (canonical matcher/provider boundary глобальной локальной библиотеки, `C40`) |
| C84 | потолок вызовов semantic brief действует на адаптер, а не на проект | **FACT** | `build_semantic_brief_adapter()` вызывается заново внутри `src/news/visual_plan.py:75`, счётчик живёт на экземпляре (`self.calls = 0`, `src/content/semantic_brief_openai.py:219`; проверка `self.calls >= maximum_calls_per_project`, `:229`). Каждый build плана получает полную квоту заново. `_accumulate_semantic_brief_usage` складывает **отчётность** по replan'ам и прямо это документирует («Carry prior spend forward while keeping each adapter's call cap unchanged», `src/news/draft_completion.py:326`). Измерено в LIVE-5: при `maximum_calls_per_project: 12` persisted `calls` = **15** (`localizations/ru/visual/visual_plan.json`) при 5 в master-снимке; учёт честный, имя ключа — нет. Денежный потолок при этом удержан: $0.15 при заявленном $1 | **ЗАКРЫТО 2026-08-15** внутри существующего semantic-brief live owner: счётчик стал project-scoped. Потраченное проектом переносится параметром `prior_usage` (`build_semantic_brief_adapter` → `OpenAISemanticBriefBackend`), и перед каждым запросом проверяется `prior_calls + self.calls + 1 <= maximum_calls_per_project`. Складывается прошлое с текущим ровно в одном месте — `OpenAISemanticBriefBackend.usage_summary`; `_accumulate_semantic_brief_usage` в `draft_completion` удалён как второй сумматор. Второй бюджетный контур и второй owner не создавались, схема `planning_metadata.semantic_brief_usage` не менялась. Тесты: `tests/test_semantic_brief_project_budget.py` (счёт ведётся на stub-клиенте, переживающем все экземпляры backend) | существующий semantic-brief live owner (`PLAN-9B-PRODUCER-M-LIVE`) |
| C85 | `maximum_budget_usd` не ограничивал ничего, кроме собственной положительности | **FACT** | Найдено 2026-08-15 при разборе `C84`. До правки поле читалось ровно в двух местах: `paid_call_blockers` (`> 0`, иначе адаптер не строится) и `activation_diagnostics` (отчёт). Ни одной проверки перед запросом не было, поэтому прогон с `maximum_budget_usd: 0.03` и `estimated_cost_per_call_usd: 0.01` отправлял столько запросов, сколько разрешал call cap | **ЗАКРЫТО 2026-08-15**: перед каждым запросом проверяется `prior_cost + (self.calls + 1) * estimated_cost_per_call_usd <= maximum_budget_usd`, округление согласовано с `usage_summary` (`round(..., 6)`). Это **оценочный** guard: ответ модели читается только через `output_text`, токены не читаются нигде, фактическая цена процессу недоступна — гарантия суммы счёта провайдера **не заявляется** ни в коде, ни в тестах, ни здесь. Pricing/token/billing подсистема не создавалась | существующий semantic-brief live owner (`PLAN-9B-PRODUCER-M-LIVE`) |
| C86 | rerun стадии планирования выдавал новую квоту и уменьшал запись о тратах | **FACT** | Найдено 2026-08-15 при разборе `C84`. Стадия `visual_plan` строила план начисто и перезаписывала `planning_metadata.semantic_brief_usage` счётчиком последнего build'а, поэтому `--force-stage`, resume и restart процесса и выдавали полную квоту заново, и роняли запись ниже фактически потраченного. Отдельно: `src/news/visual_plan.py` писал usage только `if usage:`, из-за чего build без адаптера (оплата или сеть выключены) стирал прошлую запись проекта | **ЗАКРЫТО 2026-08-15**: стадия читает прошлую трату из локализованного плана и передаёт её как `prior_usage`; перенос прошлой записи выполняется безусловно, в том числе когда адаптер не создан (`semantic_brief_project_usage`). Канонический источник — `localizations/<lang>/visual/visual_plan.json`; `master_visual_plan.json` остаётся снимком стадии и как seed не читается, поэтому два зеркала не складываются. **Не закрыто (названо, не исправлено):** (1) два параллельных процесса над одним `project_id` прочитают один и тот же seed и получат по полной оставшейся квоте — блокировки в этом слайсе нет; (2) нечитаемый локализованный план читается как «ничего не потрачено», потому что существующий контракт требует, чтобы повреждённый артефакт стадии перезапускал стадию, а не останавливал прогон (`test_visual_plan_stage_idempotency_missing_and_corrupt_output`); обычным прогоном это не достигается — запись идёт через `atomic_write_json` под project lock; (3) место в бюджете тратится **до** запроса (`src/content/semantic_brief_openai.py:313`), а запись попадает на диск только после того, как build плана вернулся, поэтому исключение, которого нет ни в `_sdk_error_types()`, ни в семействе `OSError` — `RuntimeError`, `KeyboardInterrupt`, kill, OOM — уходит наружу мимо `_semantic_brief` (`src/content/visual_planning/engine.py:290` ловит только `VisualPlannerError`) и мимо `brief_for` (`src/content/visual_planning/semantic_brief.py:386` ловит только `OSError`); план не пишется, и уже отправленные запросы исчезают из записи. Измерено на stub-клиенте при `maximum_calls_per_project: 3`: четыре цикла «упал — перезапустили» отправили 4 запроса при `calls` = 0 на диске, то есть потолок не удержан. Граница проходит ровно здесь и в ту же сторону: обычный отказ провайдера (SDK error, `TimeoutError`, `ConnectionError`) остаётся `VisualPlannerError`, build продолжается, запись сохраняется — те же четыре цикла дали 3 отправленных запроса и `calls` = 3. Это редкое аварийное окно, а не штатная утечка. Тот же класс («чем запись перестаёт быть правдой»), но без падения: `visual-plan build` (`src/ai_youtube/cli/commands/authoring.py:267`) строит план без `brief_adapter`, поэтому платных вызовов не делает, но и записывает план без `semantic_brief_usage`, так что направленный оператором `--out` прямо в живой `localizations/<lang>/visual/visual_plan.json` обнулил бы запись проекта. Отдельный владелец ни на что из перечисленного не открывается | существующий semantic-brief live owner (`PLAN-9B-PRODUCER-M-LIVE`) |
| C87 | USD-guard инертен, пока `estimated_cost_per_call_usd` равен нулю | **FACT** | Найдено 2026-08-15 независимым review правки `C84`/`C85`. `paid_call_blockers` требует `maximum_budget_usd > 0` (`src/content/semantic_brief_openai.py:173`), но `estimated_cost_per_call_usd` не требует ничего — поле по умолчанию `0.0` (`:120`) и равно `0` в `config/semantic_brief.json`. Конфигурация с `maximum_budget_usd: 0.50` и `estimated_cost_per_call_usd: 0` проходит все блокеры, адаптер строится, а pre-call проверка `C85` (`_projected_cost_usd`, `:333`) всегда даёт `prior_cost + 0` и не может превысить потолок — то есть денежный guard не срабатывает никогда. `activation_diagnostics` (`:446`) при этом показывает непустой `maximum_budget_usd` и пустой `paid_blockers`, поэтому отчёт выглядит так, будто бюджет ограничивает. Единственный оставшийся предел в такой конфигурации — `maximum_calls_per_project` | **НЕ исправлялось.** Выбор между «сделать положительный `estimated_cost_per_call_usd` блокером в `paid_call_blockers`» и «оставить документированной оговоркой, что нулевая оценка выключает USD-guard» — owner decision, и эта строка его не выдаёт: первый вариант ломает существующие платные конфиги, у которых оценка не заполнена. Второй бюджетный контур, pricing/token/billing подсистема и новый owner не создаются | существующий semantic-brief live owner (`PLAN-9B-PRODUCER-M-LIVE`) |
| C89 | ранжирующая оценка спрашивает литеральный вопрос о производном описании сцены | **FACT** | Найдено 2026-08-15 при исправлении `C79`; это его вторая половина, и решает исход именно она. `candidate_ranker._field_match` (`:639`) зовёт `evidence.semantic_literal_score` для subject/action/environment/location/camera, тогда как слот-слой для тех же полей зовёт `semantic_stem_score` (`decision.py:316`), а docstring `stem_concept_score` прямо называет производное описание сцены случаем толерантного примитива. Один кандидат судится двумя разными мнениями о том, что такое одно и то же слово. Измерено после закрытия стемовой половины `C79`: слот-слой называет subject совпавшим и verdict'ом `complete`, а `subject_match` остаётся `0.0`, поэтому `semantic_match_status` = `mismatched`, `support_status` = `unsupported`, `score_below_*` остаётся блокирующим и кандидат, чьи метаданные буквально называют предмет сцены, проигрывает записи, разделяющей с ним одно несклонённое слово. Не языковой частный случай: документированная самим модулем пара `antarctica`/`antarctic` на этом пути тоже даёт `0.0` | **ИСПРАВЛЕНО 2026-08-15** characterization-first, внутри того же владельца `src/assets/semantic_selection` (`evidence.py` + одна точка вызова в `candidate_ranker.py`). Пороги (`MIN_SCORE`, `EXACT_SUBJECT_MIN_SCORE`), вес 0.45 и прочие веса не менялись; слот-слой и `entities._ENDINGS` не трогались; второй ранжировщик, второй стеммер и внешний словарь не создавались. Однострочная замена `_field_match` на `semantic_stem_score` остаётся откаченной: она сдвигала замороженный ground truth PLAN-9D (`tests/test_plan9d_ground_truth_baseline.py`, `FrozenBaselineMeasurementTests`) — `scene_009` уходила с предпочтённого аннотатором `C31` (wikimedia, photovoltaic panel) на `C36` (pexels, portable light tower), `preferred_matches` 4 → 3, `unscorable_winner_not_visible` 0 → 1. Причина сдвига перепроверена по токенам 2026-08-15 и оказалась **не** стеммером, а **префиксным отношением**: `C36` получал subject 33.33 → 66.67 за `power` ⊂ `powered` и action 0.00 → 20.00 за `panel` ⊂ `panels`, стем-кредитов в обоих случаях ноль, `C31` не двигался ни по одному полю. Поэтому отношение слов разделено на два именованных: `prefix_variant` (орфографическая поблажка) и `inflected_variant` (равенство стемов существующего `entities.stem`); `morphological_variant` остался их дизъюнкцией и по-прежнему обслуживает слот-слой, а ранжирование зовёт новый `semantic_inflection_score` = литерал + равенство стемов, **без** префикса. Асимметрия названа в docstring `evidence` и `_field_match`: слот-слой отвечает на бинарный вопрос «это вообще нужный объект», где поблажка `antarctica`/`antarctic` уместна и ничего не накапливает; ранжирование отвечает на соревновательный вопрос «кому дать больше очков», где та же поблажка суммируется и делает `powered` доказательством `power plant`. Правка аддитивна на уровне positive matching, но **отсутствие ranking-регрессии доказано отдельно измерением, а не выведено из аддитивности** (наивный вариант тоже был аддитивным): замороженный PLAN-9D бит-в-бит прежний — `scene_009` = `C31`, `preferred_matches` 4, `scorable_scenes` 14, `unscorable_winner_not_visible` 0 — и победители всех 14 сцен сравнены поимённо, не изменился ни один. `must_include`, `must_not_include`, requirement-слоты и conflicts не ослаблены (закрыто тестами). Тесты: `RussianMorphologyMatchingTest::test_the_score_that_gates_selection_reads_the_inflected_subject` (перевёрнут из пина открытого состояния) и новый `EnglishPrefixCreditGuardTest` — guard против ровно откаченной ошибки: зелёный и до, и после правки, красный при наивном stem-aware варианте (проверено прогоном). **Не заявляется**, что retrieval улучшен: замороженный корпус английский и русской морфологии не содержит, а ноль изменившихся победителей означает отсутствие регрессии на измеренном корпусе (14 сцен, английский), а не вообще; улучшение показано на characterization и на форме LIVE-5 | существующий владелец `src/assets/semantic_selection` (тот же, что у `C79`) |
| C90 | `evidence.semantic_literal_score` остался без вызывающих | **FACT** | Найдено 2026-08-15 при закрытии `C89`. Единственным вызывающим был `candidate_ranker._field_match`; он переведён на `semantic_inflection_score`, и после этого ссылок на `semantic_literal_score` в коде **ноль** — остались только определение (`src/assets/semantic_selection/evidence.py`) и упоминания в docstring `evidence` и в docstring одного теста. Метод остаётся в публичном API модуля и читается как действующий режим сопоставления, которым никто не пользуется | **НЕ исправлялось намеренно:** удаление — cleanup, а `AGENTS.md` запрещает смешивать cleanup с функциональным изменением, поэтому оно не делалось в слайсе `C89`. Exit condition: отдельный слайс удаляет метод вместе с его строкой в `__all__` и упоминаниями в docstring — либо строка закрывается решением оставить литеральный режим как явную часть контракта примитивов. Новый примитив, второй матчер и переименование существующих режимов этой строкой не назначаются | существующий владелец `src/assets/semantic_selection/evidence.py` (тот же, что у `C79`/`C89`) |

**Приписка к `C83` 2026-08-17: продуктовая часть evidence устарела, строка
сужена, мёртвые ключи стали видимыми.**

Ту же сцену несёт корпус v2 как `live_5/scene_003`. После правки `C91` (пакет D,
`2b636ab`) победитель в ней сменился с `C5` — Opel car dashboard, wikimedia — на
`C7`, ту самую локальную запись «Автоматизированная линия сборки солнечных
панелей». Число **7.5** в evidence выше было подписью `C91`
(`final = 0.075·quality` при обнулённом `meaning_score`), а не следствием ключа.
От `C83` остаётся узкий факт: `assets.use_local_library` по-прежнему не читает
никто, и решение «читать или удалить» остаётся за владельцем.

`C83` при этом не единичный случай, а класс: в пяти `channel_config.json` **79**
листовых ключей, из них **14** не встречаются ни разу в `src/`, `tools/`,
`scripts/`; после удаления `allow_unknown_rights` — **78** и **13**. Числа
воспроизводятся помощником самого теста
(`tests/test_config_resolver._channel_config_leaf_keys`). Аудит тестов §7.5 назвал
**76**, и эта цифра, перенесённая сюда 2026-08-18 вместе со словами «перепроверен
на этом HEAD», была неверна — нашло независимое ревью пакета (`docs/audits/REVIEW_C96_S5_2026-08-18.md`);
несущее число «14 мёртвых → 13» ревью подтвердило. Слайс
2026-08-17 закрывает не сами ключи, а их невидимость:

- `assets.allow_unknown_rights` **удалён** из `nature_science_news_ru`.
  Выключатель, который обещает ограничить права и ничего не ограничивает, хуже
  отсутствующего: сами права держат `src/projects/rights.py` и
  `src/assets/license_policy.py`, которые этот ключ никогда не читали. Это
  правка поверхности настройки, а не шлюза — открытым он не был.
- Остальные **13** объявлены в `DECLARATIVE_UNREAD_CHANNEL_KEYS`
  (`tests/test_config_resolver.py`), каждый со строкой причины. Они остаются в
  файлах: это заявленный продуктовый замысел владельца, а не ложное обещание
  безопасности, и удалять его молча слайс не вправе.
- Тест требует, чтобы **каждый** листовой ключ канала был либо в
  `keys.SETTINGS`, либо встречался в коде, либо стоял в этом списке — иначе
  пятнадцатый мёртвый ключ появляется молча. Второй тест не даёт списку пережить
  свои ключи, третий запрещает объявлять декларативным булев ключ, чьё имя
  обещает ограничить права, деньги или сеть: список не должен стать способом
  аккуратно хранить ложное обещание.

Новый governance-механизм этим не заводится: проверка живёт внутри существующего
владельца `tests/test_config_resolver.py` и опирается на существующее поле
`consumers` в `keys.SETTINGS` и на существующее состояние `no_consumer_yet`.

## Product audit findings 2026-08-16 (C91)

Внесено 2026-08-16 по итогам аудита продукта
(`docs/audits/PRODUCT_AUDIT_2026-08-16.md`, HEAD `ede8c4b`). Новый PLAN-ID не
создавался, номер VA-NEW не выдавался, статусы шагов не менялись. Аудит назвал
восемнадцать кандидатов в реестр (§10); заведена одна строка — корневая причина
«не по смыслу», у которой уже есть существующий владелец. Остальные заводятся по
мере входа в пакеты, а не списком: строка реестра — запись дефекта, а не
разрешение на его исправление.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C91 | вердикт «доказуемо/нет» выносился по склеенному тексту записи, а не по полю, дающему балл | **FACT** | Аудит продукта 2026-08-16 §6; механизм — `docs/audits/PLAN_9D_H_CORPUS_V2_2026-08-17.md` §11. `CandidateEvidence.is_undecidable` спрашивал `script_mismatch` по склейке `self.text`, тогда как балл читает `self.fields` по отдельности; XOR по кириллице делал английский термин несопоставимым при совпадении 100 по полю, а `candidate_ranker` обнулял за это `meaning_score`. Измерено: 7.5 против 78.3 с английскими метаданными. Слой не покрывали ни `C79`, ни `C89`, ни `C83` | **ИСПРАВЛЕНО 2026-08-17, вариант A.** Две правки в существующих владельцах: `is_undecidable` спрашивает каждое поле, и цикл `must_include` в `candidate_ranker` спрашивает evidence вместо склейки — без второй отказ `semantic_unverified` пережил бы первую. Алиас `_script_mismatch` удалён с последним вызывающим (`C90`). Второй примитив, «RU-путь», пороги и веса не заводились; слот-слой запретов накрыт им же, кроме `must_avoid_match` (`C97`). Приёмка (`K12`) — оба числа: **v1 — `changed_winners 0`** (`measure --baseline`, корпус `bfb4d02437f3c528`; 4/14 не двинулось, v1 языка не видит), **v2 — 2 сцены из 11 поимённо**: `live_5/scene_003` `C5`→`C7`, `local_after_fix/scene_002` воздержание→`C2`. Числа говорят «сдвинулось», а не «стало лучше»: разметки v2 нет, agreement не считается, `4/14` остаётся последним числом | Закрыта; метод замера v2 — в теле коммита. Владелец `src/assets/semantic_selection` |
## Governance finding 2026-08-17 (C92)

Внесено 2026-08-17 пакетом B маршрута внедрения
(`docs/audits/ROLLOUT_PLAN_2026-08-17.md`, HEAD `3619fe1`). Новый PLAN-ID не
создавался, статусы шагов не менялись, ADR этой строкой не пишутся: она
записывает долг, а не разрешает его закрыть.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C92 | долг ADR-бэкфилла нигде не записан как долг | **FACT** | ADR остановились на `0016` (2026-07-29): в `docs/adr/` 16 записей и `README.md`, новых нет с начала программы. Аудит 2026-08-12 назвал это `R9` и предложил бэкфилл 5–6 коротких ADR. В плане записано только **порядковое** ограничение («ADR-бэкфилл выполняется до переноса закрытых секций в архив», блок «WP0-B — placement»), но сам долг не имеет ни строки реестра, ни владельца, ни exit condition. Внешний governance-аудит Antigravity утверждал обратное — что номера `0017`–`0022` зафиксированы в плане как долг; проверка `docs/audits/EXTERNAL_REVIEW_VERIFICATION_2026-08-16.md` показала, что это ложно: `grep` по `docs/current/` на `0017`–`0022` рядом со словом ADR даёт ноль совпадений. То есть долг существует, а запись о нём была только во внешнем документе, который сам в репозитории не сохранён | Существующие владельцы: `docs/adr/` и блок «WP0-B (governance/docs diet) — placement» в плане. Состав бэкфилла подтверждает владелец; номера будущих ADR здесь не резервируются — ссылка на несуществующий ADR роняет `tools/qa/check_agent_docs.py` | Закрывается, когда бэкфилл выполнен либо владелец явно отказался от него с причиной. До этого перенос закрытых секций плана в архив не начинается. Бэкфилл выполнен 2026-08-17: ADR 0019-0021; отказ по 4-му кандидату — детали в блоке «WP0-B — placement» плана. |

## Product decision finding 2026-08-17 (C93)

Внесено 2026-08-17 от HEAD `462c372`. Новый PLAN-ID не создавался, `PRODUCT_PLAN.md`
этой строкой не менялся, решения владельца ею не переносятся и не утверждаются:
она записывает долг и его exit condition. Пакет **P** маршрута
(`docs/audits/ROLLOUT_PLAN_2026-08-17.md`, статус `proposal`) остаётся тем, кто
этот перенос выполняет; строка нужна затем, чтобы долг не зависел от документа,
который сам прав не даёт.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C93 | ответы владельца 2026-08-16 существуют только в evidence-отчёте, а индекс обещает их в `PRODUCT_PLAN.md` | **FACT** | `docs/audits/PRODUCT_INTERVIEW_MERGED_2026-08-16.md` несёт прямые ответы владельца на 10,5 из 12 блокирующих вопросов (§0-B.2), включая правовую модель репурпозера (`Q91`), принцип переключателей per-stage (§0-B.1) и порядок «Content Creator первым» (`Q22`). Строка индекса `docs/audits/README.md` объявляет «где current truth» — `PRODUCT_PLAN.md` §2–§5, §14, §17. Фактически там этого нет: последний коммит `PRODUCT_PLAN.md` — `5979321` от 2026-08-09, старше опросника, и `grep` по нему на `Q91`, `repurposed_personal_use` и per-stage даёт ноль. Сам отчёт это признаёт («перенос принятых решений в canonical документы остаётся отдельным шагом»), но указатель читается как выполненный перенос | **НЕ переносилось.** Owner — `docs/current/PRODUCT_PLAN.md` (§2–§5, §14, §17) плюс пакет **P** маршрута, который называет состав переноса построчно. Запрещено: утверждать решения владельца этой строкой; заводить второй продуктовый документ; переносить `Q31` (пересмотр PD-1) — это отдельное governance-решение с анализом | Закрывается, когда пакет P выполнен и `PRODUCT_PLAN.md` несёт решения с пометкой «ответ владельца 2026-08-16», либо владелец явно отказался с причиной. До этого ни один агент не вправе считать эти ответы канонической продуктовой рамкой |

**Приписка 2026-08-17: перенос выполнен, `C93` закрыта.** Пакет **ADR (+P)**
маршрута исполнен двумя коммитами. Первый — `666c296`: `docs/adr/0017-repurposer-rights-model.md`
(правовая модель репурпозера по `Q91`: не наследует rights-контракт Content
Creator, метка происхождения `repurposed_personal_use` объявляется в манифесте
положительно, `publish_ready` в смысле Content Creator никогда не true) и
`docs/adr/0018-per-stage-automation-switches.md` (переключатели на каждом этапе
в трёх положениях, настраиваемые один раз в шаблоне канала, как расширение
существующего human review boundary, а не третий completion ladder). Второй —
этот коммит: перенос в `docs/current/PRODUCT_PLAN.md` — раздел 2 (формулировка
через человека и границу ответственности), раздел 3 (оговорка про критерии
репурпозера), раздел 4 (`PD-12` — Content Creator первым), раздел 5 (порог
≥ 10/14 назван неизмеренным), раздел 6 (обложка как переключатель и шаблон
канала), раздел 7 и 7.1 (оценка сценария `COMMITTED_LATER`, длительность по
нише), раздел 10 (семь критериев Content Creator и пять — репурпозера),
раздел 11.3 (`Thumbnail generator` → `COMMITTED_NEAR_TERM`), раздел 12 (три
идеи, которые не строятся отдельным владельцем), раздел 14 (правовая метка
ссылкой на ADR 0017), раздел 17 (счёт открытых решений — пятнадцать, закрытых
ноль). Каждый перенос помечен «ответ владельца 2026-08-16, источник —
опросник». **Не переносилось и остаётся вне canonical рамки:** `Q31`
(пересмотр `PD-1` о нескольких сценарных движках) — отдельное
governance-решение класса HIGH, требующее анализа; `Q189` (что раздражает
больше всего) — прямого ответа владельца не было; вторая половина `Q100` (пять
признаков «залетит» и пять «провалится») — дан только структурный признак из
`Q99`. Статус AI-генерации картинок не переносился как существующая
возможность: заявление владельца кодом не подтверждено.

**Приписка 2026-08-19: у переноса был остаток, и он закрыт.** Приписка выше
называла невыполненным только `Q31`, `Q189` и половину `Q100`; read-only сверка
опросника с `PRODUCT_PLAN.md` на HEAD `ca10444` показала, что вне canonical
рамки остались ещё восемь прямых ответов, по которым `grep` по документу давал
ноль: голосовое/аудио семейство провайдеров и очередь фазы после видеоядра
(`Q71`, `Q72`, `Q107`), AI-генерация как переключатель канала (`Q54`), длинные
компиляции репурпозера (`Q137`), определение хорошего момента (`Q99`, `Q91`),
видеоанализ как переключатель (`Q126`), accept/reject как training evidence
(`Q195`), шаблон канала как единица настройки ниши и целевой сквозной поток
(`Q195`), серия роликов из одного материала (`Q82`), вход по промпту и ссылкам
(`Q195`), целевая норма ручной работы (`Q4`, `Q112`). Перенесены ревизией 1.3
`PRODUCT_PLAN.md` (2026-08-19) вместе с тремя исправлениями противоречий,
которые создал первый перенос: раздел 10 объявлял метаданные неподдерживаемыми
при том, что критерий 5 раздела 10.1 их требует; раздел 11.3 держал
`YouTube metadata` в `DEFERRED_IDEA` и шаблон канала — в форме, противоречащей
ADR 0018; раздел 5 утверждал, что слепая разметка не сдана, после того как она
легла и дала `blind agreement` 2/10. `Q31` по-прежнему **не** переносится как
решение: он записан открытым `OD-P-18`, а `PD-1` действует без изменений.
Открыты тем же переносом `OD-P-17` (форма происхождения генерированного
материала) и `OD-P-19` (входит ли обложка в сам v1). `Q189` и вторая половина
`Q100` остаются без ответа владельца.

## Safety finding before the next paid run 2026-08-17 (C94)

Внесено и закрыто 2026-08-17 пакетом **E** маршрута
(`docs/audits/ROLLOUT_PLAN_2026-08-17.md`, статус `proposal`) от HEAD `48266a5`.
Оба решения класса HIGH приняты владельцем **до** работы: имя нового сетевого
класса и выбор по `C87`. Новый PLAN-ID не создавался, второй network-контур и
второй бюджетный контур не заводились, `current_checkpoint` не двигался.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C94 | платная генерация озвучки шла мимо default-deny сети | **FACT** | Найдено 2026-08-17 (опись 82) и подтверждено чтением на `48266a5`. `ElevenLabsProvider.synthesize` делал `self.http.post` без единого вызова `require_network`, тогда как `preflight` и `list_voices` в том же файле были закрыты классом `voice_preflight`. Платный гейт менеджера (`provider_manager.py:29`) дыру не закрывал: `src/audio/voice_cli.py:261` и `src/production_plan/solar_vs_nuclear_render.py:134` зовут метод напрямую. Audition-тест это фиксировал, не называя: реальный платный POST при выданном одном `voice_preflight` | **ИСПРАВЛЕНО этим слайсом.** Решение владельца 2026-08-17: заведён **отдельный** класс `voice_synthesis`, а не переиспользован `voice_preflight` — иначе разрешение на бесплатную проверку аккаунта открывало бы платную генерацию, что запрещает инвариант `runtime_network` «одобренный класс не открывает соседние». Проверка стоит в самом провайдере (не у вызывающих), до `mkdir`, поэтому отказанный прогон не оставляет следа на диске, и после проверки ключа, чтобы ненастроенный ключ сохранил своё сообщение. Платный гейт `VoiceApproval`/`provider_manager` не тронут и остаётся вторым и независимым | Закрыта. Публичное следствие, которое обязан знать владелец: реальный платный прогон теперь требует `--allow-network voice_synthesis` — одного `voice_preflight` недостаточно |

**Приписка к `C87` 2026-08-17: исправлено этим же пакетом, строка закрыта.**
Решение владельца — **блокер**, а не документированная оговорка. В
`paid_call_blockers` добавлена пятая причина `estimated_cost_per_call_usd_required`:
без положительной оценки объявленный потолок в долларах не мог быть превышен ни
при каком числе вызовов, поэтому денежный guard был выключен молча. Отдельно
закрыта вторая половина дефекта — вводящий в заблуждение отчёт:
`activation_diagnostics` печатает теперь `estimated_cost_per_call_usd` и
`usd_budget_enforceable` рядом с `maximum_budget_usd`, поэтому непустой потолок
рядом с пустым `paid_blockers` больше не читается как «бюджет ограничивает».
Второй бюджетный контур, pricing/token/billing подсистема и новый owner не
создавались. Цена решения записана честно: конфигурация с заполненным бюджетом и
незаполненной оценкой раньше проходила и делала вызовы, а теперь отказывает —
**названной** причиной, до первого вызова. В репозитории ломать было нечего
(`config/semantic_brief.json` имеет `enabled: false` и уже блокировался четырьмя
причинами); сломается только локальный конфиг владельца, если он в этом
состоянии. Тест `test_without_a_per_call_estimate_only_the_call_cap_bounds_the_run`
замораживал прежнее поведение (два вызова под потолком, который не мог
сработать) и переписан под новый контракт с сохранением записи о старом.

## Instrument finding at the closure of package D 2026-08-17 (C95)

Найдено пакетом **D** маршрута при закрытии `C91`: правка отбора сдвинула
производное поле замороженного корпуса, а очевидный ответ — пересчитать корпус —
оказался запрещён ценой слепой разметки.

Корпус хранит два разных вида данных: сетевые факты, записанные один раз (пулы,
запросы, кадры), и арифметику над ними (`categories`, статистики), которую
`finalize` пересчитывает **текущим** кодом. Пока хеш покрывает и второй вид,
«замороженный корпус» заморожен только до следующей правки отбора.

Решение владельца 2026-08-17: дрейф назвать, корпус и разметку не трогать. Новый
PLAN-ID не создавался, `current_checkpoint` не двигался.

**2026-08-18: строка закрыта.** Запрет на пересчёт был не самостоятельным
правилом, а следствием привязки разметки к хешу всего файла; правка убрала
следствие вместе с причиной. Порядок именно такой: сначала привязка переехала на
дайджест вопроса, и только после этого корпус был пересчитан — с доказательством,
что дайджест не сдвинулся. Хеш в `current_annotations_v1.json` не переподписан:
владелец размечал файл `bfb4d02437f3c528`, и провенанс продолжает называть его.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C95 | `corpus_sha256` покрывает поля, которые вычисляет код, поэтому правка отбора отвязывает замороженный корпус от кода, а пересчёт — от слепой разметки | **FACT** | Измерено 2026-08-17 при закрытии `C91`: правка сдвинула ровно одно поле корпуса (`plan9d_current_capture_v1/scene_010` теряет `ambiguous_needs_review`, `changed_winners 0`), но хеш при этом уходит с `bfb4d02437f3c528` на `280e6da190b7fb03`, а `current_annotations_v1.json` записывает хеш, против которого делалась разметка, и harness отказывается мерить при несовпадении. Пересчёт корпуса осиротил бы единственную размеченную истину репозитория | **ИСПРАВЛЕНО 2026-08-18** внутри существующего владельца `tests/plan9d_ground_truth.py` (`d49faae` + `20f02cd`). Разведены два утверждения и два хеша: `corpus_sha256` остаётся целостностью файла, а новый `annotation_identity_sha256` — дайджест вопроса, на который отвечал человек (требование сцены, идентичность кандидата, контрольные суммы пикселей), и привязка разметки идёт по нему. Состав дайджеста и намеренные исключения (путь к превью, порядок записей) — в теле `d49faae`. Приёмка числом: корпус v1 пересчитан `finalize` и переподписан, дайджест идентичности **не изменился** (`04b33c9bad935d5a`), `measure` даёт прежние **4 / 14** и тех же победителей. Второй прибор, второй корпус и второй реестр не заводились | Закрыта. Владелец — `tests/plan9d_ground_truth.py`; тот же вопрос для корпуса v2 закрывается тем же дайджестом, потому что доска и шаблон берут его оттуда же |

## Fallout of package E found by package D 2026-08-17 (C96)

Найдено 2026-08-17 полным офлайн-прогоном при закрытии `C91`: два теста ветки
красные, и причина не в правке отбора. Строка **записывает** дефект и его не
исправляет — владелец у него свой, пакет **E**. `current_checkpoint` не двигался.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C96 | прогон, который собирается платно озвучивать, не просит класс сети, без которого озвучка теперь отказывает | **FACT** | Пакет **E** (`d7fb378`, строка `C94`) закрыл дыру, поставив `require_network(voice_synthesis)` внутрь `ElevenLabsProvider.synthesize`. Декларация действий прогона за ним не пошла: `src/content_creation/wizard_state.py:163-164` при `voice_provider == "elevenlabs"` добавляет только `NETWORK_ACTION_VOICE_PREFLIGHT`; `voice_synthesis` не появляется ни в одном вызывающем. Мастер показывал список действий без того, на котором прогон и остановится. Симптом: `test_news_voice_adapter.py:138,194` давали `partially_completed` | **ИСПРАВЛЕНО 2026-08-17** (`91fc97c`). `required_network_actions` объявляет оба класса вместе: двухфазный платный поток запоминает `allow_network` в первой фазе и второй раз не спрашивает, поэтому «попросить синтез позже» невозможно. Одобрение сети не одобряет трату — платный гейт второй и независимый. Запрещено: переиспользовать `voice_preflight` вместо нового класса, второй сетевой контур, ослабление проверки в провайдере | Закрыта; независимое ревью 2026-08-18 (`7e8fded`, `docs/audits/REVIEW_C96_S5_2026-08-18.md`) — Scope PASS · Objective PASS, ноль BLOCKER/MAJOR. Строка была помечена закрытой автором **до** ревью, и этот разрыв ревью закрывает |

## Findings of the first measured blind pass 2026-08-18 (C98–C99)

Внесено 2026-08-18 после первой слепой разметки корпуса v2 и её измерения
(`blind agreement` 2 / 10). Evidence — `docs/audits/PLAN_9D_FAILURE_DIAGNOSIS_2026-08-18.md`.
Новый PLAN-ID не создавался, `current_checkpoint` не менялся. Разбор назвал
четыре класса ошибок; строки заведены на два, у которых есть существующий
владелец и измеренный размер. Остальные заводятся при входе в свой пакет:
строка реестра — запись дефекта, а не разрешение его чинить.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C98 | бриф теряет тему видео: `subject` и `provider_queries` сцены не обязаны сохранять предмет, ради которого сцена существует | **FACT** | Измерено 2026-08-18 на замороженном корпусе v2: из **42** уникальных запросов **15** не называют предмет темы ни по-английски, ни по-русски; 13 из них пришли из `visual_brief.provider_queries` (`src/assets/query_adapter.py:470-486`), 2 — из полей того же брифа, ни одного — из детерминированного планировщика. `topic_entity` = «панель», при этом `scene_003.visual_brief.subject` = `factory production line`, `scene_004` — `battery pack`. В шести сценах из восьми несовпавших нужный кандидат в пуле всё же был и проиграл ранжированию (`C99`) | **ИСПРАВЛЕНО 2026-08-18** одним слайсом на существующих владельцах (`7b6a169`): `query_adapter` получил `TopicAnchor` и `plan_topic_anchor(visual_plan)` — английская форма темы берётся из английского свидетельства того же плана, а не переводом, и встаёт в начало каждого английского запроса, который её не называет. `K9` не ослаблен, немой отказ исключён записью `query_subject_unverified`. Разбор путей и границ — в теле `7b6a169` | **Закрыта 2026-08-18.** Перепись через самого владельца (`tests/test_query_topic_anchor.py`): **15 из 42 → 0** при воспроизводимом «до», guard отбора не двинулся (2 / 10 на v2, 4 / 14 на v1). Улучшение отбора **не** заявлено. Не покрыто: локальная библиотека приходит в пул без запроса, «эквивалент» ADR 0022 не реализован, `build_slot_queries` проверен модульно |
| C99 | ранжирование не различает кадры одинаковой релевантности: план кадра и композиция не участвуют ни в оценке, ни в требовании | **FACT** | Измерено 2026-08-18. `live_5/scene_001`: система взяла крупный план (sem 100), владелец — общий план (sem 88.2), слово `establishing` стоит в самом запросе. `local_after_fix/scene_005`: у кандидатов sem 76.5, расходится только крупность. `local_after_fix/scene_002`: три ролика с sem 64.7, исход решает порядок подачи | **ИСПРАВЛЕНО ЧАСТИЧНО 2026-08-18.** Owner decision: офлайн-признак крупности плана внутри существующего `candidate_ranker`, привязанный к уже существующему `visual_brief.shot_type` (не Vision). Смещение вне `meaning_score` — бонус/штраф `±8` к `final_score` только для `establishing`/`context`→wide и `detail`→close, и (`C103`) только внутри `SHOT_SCALE_TIE_EPSILON` от лучшего по смыслу в пуле — смысл структурно не перевешивается. `payoff`→close **измерена и отклонена** — 1 из 2 на v2, не правило; в код не попала. `action`/`evidence` не размечены — второй сигнал вслепую запрещён этой же строкой | Частично: `measure --baseline` v1 **4/14 → 6/14** (`scene_006`, `scene_010`), регрессий нет; v2 agreement не сдвинулся (2/10) — `scene_001` перестал брать крупный план, но берёт другой общий (`C7`), не ровно предпочтённый (`C1`) — метрика по конкретному кандидату это не засчитывает. `scene_002` (`action`) остаётся неразличимой тройной связкой. Остаток — `PD-2`/Vision отдельным шагом, если офлайн-признаков не хватит |

## Independent review finding on package D 2026-08-17 (C97)

Найдено независимым ревью коммита `2b636ab` (MINOR) и подтверждено чтением кода:
запрет автора проверяется двумя разными путями с разными правилами. Строка
записывает несогласованность и её не исправляет — ревью прямо говорит «назвать,
а не чинить сейчас». `current_checkpoint` не двигался.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C97 | `must_not_include` проверяется двумя путями, и дисквалифицирующий из них доказуемость не спрашивает | **FACT** | Слот-слой (`decision.py:363`) спрашивает `evidence.is_undecidable(text)` до `evidence.contains(text)`. Сам отказ выносит не он: `candidate_ranker.py:315` считает `negative_matches` через `_contains_concept` по склеенному тексту и без вопроса о доказуемости, `:526` превращает это в `must_avoid_match:<термины>`. Разделение объяснено docstring `declared_conflict_terms` (`decision.py:232-237`): повтор дал бы одному факту две причины отказа | **Разделение зафиксировано намеренным 2026-08-18** решением владельца, числом, а не мнением. Разойтись пути могут ровно на одной форме записи — поле, написанное в двух алфавитах сразу: оно вне скрипта термина и всё же содержит его дословно. Измерено до решения: выравнивание в любую сторону меняет **0 из 1928** кандидатов в 212 сохранённых сценах и 0 из 599 (v1) и 5 (v2) корпусных триплетов, а доступно только ослабляющее — `is_undecidable` может лишь погасить сработавший запрет. Тест: `test_visual_retrieval_repair::test_the_disqualifying_ban_does_not_ask_whether_the_term_was_provable` | Закрыта фиксацией разделения, а не выравниванием. Переоткрывается числом: записью, на которой два пути дают разный исход отбора. «Один примитив на весь слой запретов» по-прежнему неверно; перенос отказа между слоями этой строкой не разрешён |
...
| R05 | легаси-медиа из `assets/`: `broll/` (35 файлов, 1 557 749 972 B) · `cache/videos/` (95, 8 978 764 090 B) · `images/generated/` (26, 6 760 925 B) — итого **156 файлов, 10 543 274 987 B**; плюс tracked `assets/broll/.gitkeep` и оба его правила `.gitignore` | `retired/legacy-media-manual-assets-2026-08-13` (bundle того же имени, `verify` — complete). **Медиа в bundle не входит и не могло войти: оно никогда не было versioned.** Физическая обратимость — карантинный путь `G:\Projects\AI-YouTube_attic\2026-08-13\`, счётчики файлов и байт сверены после перемещения и совпали | `b52f456` — anchor (последний commit с `.gitkeep` и правилами); удаление — `f98cd43` | `assets/broll` — **ноль ссылок в коде**: все совпадения «broll» оказались именами семантических классов (`generic_broll`, `supporting_broll`), а содержимое — `scene_01..35.mp4` одного легаси-прогона 10.05.2026. `assets/cache/videos` — осиротевший кэш **без единого производителя**: легаси `video_asset_engine.py:61` пишет в `assets/library/videos`, последняя запись здесь 17.05.2026. `assets/images/generated` — легаси `asset_finder.py:75` | `projects/<id>/assets/downloaded/` — per-project store канонического пути | уникального knowledge нет: проверено, что **ни одна** из 64 записей `media_index.json` не ссылалась на ретайренные каталоги (63 — в `assets/library`, 1 — музыка). Сохранены `assets/library`, `assets/cache/retrieval_lab` (artifact directory EXP-001, evidence), `assets/cache/images`, `assets/voice_samples` | 2026-08-13 (owner decision, раньше PLAN-14E) |
| R06 | `manual_assets/psychology/overloaded_mind_001/**` — 3 versioned SVG-диаграммы + 4 `.gitkeep` | `retired/legacy-media-manual-assets-2026-08-13` (bundle того же имени) — файлы versioned, поэтому bundle их действительно держит | `b52f456` — anchor | **отклонение от `Preserved runtime corpus`, зафиксировано намеренно.** Корпус требовал сохранять «versioned SVG в `manual_assets/**`»; владелец удалил их в рабочем дереве. Допустимо потому, что брифы того же ролика (`content/psychology/overloaded_mind_001/**`, включая diagram-плейсхолдеры) уже ретайрены **R03**, и SVG — их пара. Строка корпуса исправлена этим же слайсом, иначе документ продолжил бы лгать | — | читателей нет: единственные упоминания `overloaded_mind_001` в тестах строят собственное tmp-дерево `manual_assets` и реальные файлы не открывают | 2026-08-13 (owner decision) |

Строка добавляется **в том же commit**, что и удаление. Ретайр без tag, без
bundle и без строки здесь считается незавершённым.

**Частичный возврат из карантина R05 (owner decision 2026-08-14, слайс CURATED
LOCAL LIBRARY).** Карантин — механизм обратимости, и здесь он сработал в обе
стороны: 25 файлов вернулись в `assets/library/videos` (24 копии в 4K, которые
в библиотеке лежали в HD, + `pixabay:178732`), вытесненные HD-копии ушли в
`G:\Projects\AI-YouTube_attic\2026-08-14_displaced_by_curation` (24 файла).
Итог: карантин `2026-08-13` — **124 файла** вместо 156 (`broll` 35 файлов и
`images/generated` 26 не тронуты, `cache/videos` 95 → 63), общий карантин с
новой папкой — 148 файлов, 6.6 GB. Ничего не удалено, счётчики сверены после
перемещения. Провенанс возвращённых файлов и причина отбора — в
[docs/audits/CURATED_LIBRARY_2026-08-14.md](../audits/CURATED_LIBRARY_2026-08-14.md).
Семь файлов, у которых источник не был записан нигде, сначала остались в
карантине; после отдельно разрешённой владельцем сетевой проверки страниц
провайдера (только чтение, без скачивания) они подтверждены и тоже возвращены —
таблица проверки в том же документе.
...
| ~~versioned SVG в `manual_assets/**`~~ — **снято 2026-08-13 (R06)** | пункт больше не действует: каталог `manual_assets/` ретайрен целиком вместе с брифами того же ролика (R03). Обратимость — bundle `legacy-media-manual-assets-2026-08-13`. Строка сохранена зачёркнутой, а не удалена, чтобы отклонение от корпуса осталось видимым | N03 → R06 |
| `config/` кроме `video_style.json`; активные каналы `config/channels/nature_science_news_ru`, `config/channels/nature_pulse` | активная versioned-конфигурация: 8–21 caller на файл. **Путь изменён 2026-08-13** (`b52f456`): каталог каналов переехал из корня в `config/channels/`, содержимое файлов не менялось | N04, T.9 |
...
## Red test found while closing C98 2026-08-18 (C100)

Найдено 2026-08-18 targeted-прогоном соседних наборов при закрытии `C98`.
Проверено `git stash`: тест красный и **без** правки C98, то есть дефект не её.
Найдено и записано без починки; закрыто отдельным слайсом в тот же день.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C100 | пин sha корпуса v1 в тесте гейта отстал от самого корпуса, и тест красный на HEAD | **FACT** | `tests/test_plan9d_retrieval_gate.py:53` держал `EXPECTED_CORPUS_SHA256 = bfb4d02…`, а поле `corpus_sha256` в `tests/data/plan9d/current_corpus_v1.json` равно `280e6da…`. Расхождение внёс `20f02cd` («пересчитать производные поля корпуса v1»): корпус пересчитан, пин рядом не обновлён. Падал `test_gate_runs_with_no_socket_available`, остальные 28 тестов набора зелёные. Проверено `git stash push -u` — падает и на чистом HEAD; независимое ревью `7b6a169` подтвердило, что тот коммит к падению не причастен | **ИСПРАВЛЕНО 2026-08-18.** Пин в существующем владельце `tests/test_plan9d_retrieval_gate.py` переведён на `280e6da…` вместе с записью причины дрейфа рядом с самой константой. Дрейф законен и проверен по diff'у, а не по телу коммита: `20f02cd` изменил в корпусе ровно три строки — `scene_010` (выбранный кандидат `C33`) потеряла категорию `ambiguous_needs_review`, агрегат `technical_categories.ambiguous_needs_review` ушёл 2 → 1, файл переподписан. Ни одно из чисел, которые пинует этот же тест (14 сцен, 1064 наблюдения, 56 превью, 64 кадра, 745/319 прав), не двигалось, дайджест идентичности остался `04b33c9…`. Корпус к запинованному состоянию не возвращался: это откатило бы вторую половину `C95` | Закрыта. Владелец — `tests/test_plan9d_retrieval_gate.py`; пин остаётся якорем «корпус не дрейфует молча» и двигается только вместе с названной причиной |

## Finding of the independent review of C98 2026-08-18 (C101)

Найдено независимым `review-change` коммита `7b6a169` как residual risk, а не как
дефект того коммита: ревью дало Scope PASS · Objective PASS и ноль
BLOCKER/MAJOR/MINOR. Строка записывает мёртвый обход и его не чинит.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C101 | два неиспользуемых wrapper'а в `asset_manager` обходят якорь темы, если их когда-нибудь подключат | **FACT** | `src/news/asset_manager.py:189` (`_complete_scene_assembly`) и `:236` (`_targeted_slot_search`) вызывают `complete_scene_assembly` / `targeted_slot_search` без `topic_anchor`, то есть с `None` — запросы строились бы как до `C98`. Вызывающих нет ни одного: `grep` по `src`, `tests`, `apps`, `tools` даёт только упоминание в комментарии `tests/test_slot_aware_retrieval.py:488`. Оба существовали до `7b6a169` и в его diff не входят | **НЕ исправлялось.** Сегодня код недостижим, поэтому дыры в поведении нет и правка ради правки запрещена. Владелец существующий: `src/news/asset_manager.py`. Выбор между «пробросить `topic_anchor`» и «удалить wrapper» — часть отдельного решения о судьбе compatibility patch-points этого модуля, а не хвост слайса запросов | Закрывается вместе с решением о `asset_manager` patch-points: либо wrapper удалён, либо он проносит `topic_anchor` и это закреплено тестом |


## Finding of the independent review of C99 2026-08-18 (C103)

Независимый `review-change` коммита `cbec0f4` в тот же день, что и сам слайс —
Scope PASS, Objective PASS с одним MAJOR. Строка записывает находку и закрывается
правкой того же дня.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C103 | сигнал C99 нарушал собственное условие владельца «смысл всегда приоритетнее крупности»: магнитуда `±8` не гарантирует этого при реалистичном частичном совпадении предмета, только при полном разрыве | **FACT** | Найдено независимым `review-change` коммита `cbec0f4` (тот же день). Воспроизведено сценой с составным предметом из трёх слов: кандидат с двумя из трёх слов и верной крупностью (`sem 82.353`) обгонял кандидата со всеми тремя словами и неверной крупностью (`sem 100.0`) — разрыв `final_score` без сигнала ≈15 очков, меньше сырого качания `±8` (16 очков). Тест `test_meaning_still_outranks_shot_scale` проверял только разрыв по несвязанному предмету, далеко от границы переворота | **ИСПРАВЛЕНО 2026-08-18, тем же днём.** `SHOT_SCALE_TIE_EPSILON = 12.0` и `_apply_shot_scale_within_meaning_tier` внутри существующего `rank_candidates`: кандидат, отстающий от лучшего по смыслу `final_score` пула больше чем на эпсилон, сигнала не получает вообще — структурно, не по вероятности. Порог не подобран под контрпример: измерены реальные разрывы трёх исправленных сцен (9.4/3.3/1.3) и эпсилон поставлен выше них и ниже опасного разрыва ревью (≈15) с запасом | Закрыта. `measure --baseline` на обоих корпусах — те же числа, что до правки (v1 4/14→6/14, v2 2/10 без сдвига), регрессий нет; новый тест `test_a_partial_subject_match_cannot_be_lifted_past_a_full_match` фиксирует ровно случай ревью, плюс 5 тестов на границу эпсилон напрямую |

## Registry rows over their own ceiling 2026-08-18 (C102)

Вторая краснота, найденная тем же полным прогоном, что и `C100`, но с другим
владельцем: не тест отстал от данных, а сам реестр перерос собственный потолок.
Строка записывает это и закрывается тем же слайсом.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C102 | реестр красил ветку сам: три строки, написанные после C94, стояли выше потолка длины, и страж реестра был красным на том же HEAD | **FACT** | Найдено 2026-08-18 полным offline suite при закрытии `C100` — вторая краснота того же прогона, отдельная от первой. `test_stage2_agent_onboarding.test_registry_rows_stay_a_trail_and_not_a_journal` называл `C95` (1957), `C96` (1682) и `C98` (3763) при потолке 1500 для новых строк. Все три были над потолком уже на чистом HEAD — измерено по `git show HEAD:docs/current/CLEANUP_REGISTRY.md`. Почему это дожило до полного прогона: `scripts/gates.py` тестов не запускает намеренно, а targeted-набор слайса, который писал строку, страж реестра не затрагивал | **ИСПРАВЛЕНО 2026-08-18** тем же слайсом. Три строки укорочены до потолка тем способом, который тест и называет: находка, вердикт и указатель остаются в строке, а рассказ уходит в тело коммита (`d49faae`, `91fc97c`, `7b6a169`) и в раздел PLAN-9D плана — ни одно число из строк не потеряно. `REGISTRY_ROW_LENGTH_BASELINE` не пополнялся: он для шести строк, написанных до правила, и новые строки получают 1500 сразу. Потолок, страж и второй реестр не заводились | Закрыта. Владелец — `docs/current/CLEANUP_REGISTRY.md`; страж — существующий `tests/test_stage2_agent_onboarding.py` |

## Language audit findings never recorded 2026-08-18 (C104–C110)

Внесено 2026-08-18 от HEAD `6d815c7`, ветка `governance-reset`. Семь находок
языкового аудита
([SEMANTIC_BRIEF_PROMPT_AND_LANGUAGE_AUDIT_2026-08-16.md](../audits/SEMANTIC_BRIEF_PROMPT_AND_LANGUAGE_AUDIT_2026-08-16.md),
`K7`, `K2`, `K4`, `K5`, `K6`, `K8`, `K11`) существовали только в отчёте и в
сводке; строки реестра у них не было, то есть для следующей сессии их не было.

Отчёту три дня, и с тех пор прошли `C91`, `C97`, `C98`, `C99` и `C103`, поэтому
каждая находка перепроверена по сегодняшнему коду заново, офлайн, на уже
существующих данных: 44 сохранённых `visual_plan.json` (273 сцены),
`assets/library/metadata/media_index.json` (86 записей) и замороженные корпуса
`tests/data/plan9d`. Сеть, провайдеры, Vision, TTS и render не запускались.
Числа в строках — свои, измеренные сегодня; там, где число отчёта не
воспроизведено или получено другим способом, это сказано в самой строке.

Строки записывают дефекты и их не исправляют. Новый PLAN-ID не создавался,
`current_checkpoint` не двигался, новых владельцев и второго реестра нет.
`C104` — единственная класса **HIGH**; она соотносится с открытой `C97` (обе про
`must_avoid`) и третьего пути запрета не назначает.

| ID | Предмет | Класс | Фактическое evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C104 | русский `must_avoid` не сравним ни с чем в лестнице запросов и в приёмке брифа | **FACT** (структура и охват); исход отбора прибором не измерим | Перепроверено 2026-08-18 на `ee253b9`: `_avoided_match_tokens` (`expansion.py:290`) токенизировал обе стороны через `_PROVIDER_TOKEN_RE` (`:54`, `[A-Za-z]`), поэтому кириллический запрет давал пустой список и не отклонял ни одного запроса. Охват: 186 запретов автора в 42 сценах 43 планов (280 сцен), кириллических **26**, все в 5 сценах `2026-08-13_polog-dozhdevogo-lesa`, где в прогоне 10 кандидатов на сцену и срабатываний 0. Примитив отбора юникоден (`evidence.py:175`), но судит только русскую запись | **ИСПРАВЛЕНО 2026-08-18** внутри владельца `expansion.py`: `_avoided_match_tokens` получил свой `_AVOIDED_TOKEN_RE` — та же форма токена без алфавита, `_PROVIDER_TOKEN_RE` не тронут, запросы строятся латиницей как прежде. ru/ru False→**True**, латиница не изменилась. Цена на 43 планах: запросов адаптера 348→348, отклонено 0→0, сцен без запроса 0→0. Второй примитив, третий путь и «RU-путь» не заводились (`C97`, `C115`). Тест `MustAvoidIsPartOfTheQueryTest` | **Не покрыто:** кросс-скрипт — `parse_response` требует ответа на языке провайдера, поэтому русский запрет не откажет английскому ответу ни при каком токенизаторе (`C105`). Число прежней редакции исправлено: в v1 запретов автора **7** (кириллических 0), а не «0 из 21»; в v2 — 1. Прибор слеп, поэтому `measure --baseline` здесь сторож: v1 6/14, v2 2/10, `changed_winners` 0 |
| C105 | между русской сценой и английскими индексами нет моста: каждая пятая сцена не отправляет провайдеру ни одного запроса | **FACT** | Измерено 2026-08-18 вызовом `build_scene_queries` (`src/assets/query_adapter.py:475`) на всех 44 планах (273 сцены) против `pexels`: **59 из 273** сцен получают `query_translation_required` и не отправляют ничего. Якорь темы `C98` этого не двигает — он правит уже выбранные запросы: 655 запросов без якоря, 638 с якорем, 59 молчащих сцен в обоих случаях. Единственный мост — `GLOSSARY` (`:105`), 51 запись; из **193** различных кириллических слов, которые планы кладут в `visual_intents`/`semantic`, глоссарий знает **2** (`пластик`, `учёные`). Отбрасывает построчный `_query_language` (`:996`): один кириллический символ объявляет `ru` всю строку | **Мост не строился.** Владелец: `src/assets/query_adapter.py`. Запрещено: второй генератор запросов и машинный перевод как «мост» без отдельного решения владельца — это меняет платный контракт. Половина запрета: 2026-08-19 непроверяемый запрет назван полем записи, а не исполнен ([ADR 0023](../adr/0023-unanswerable-ban-recorded.md)). Доли отчёта (5 из 124 терминов, 19 из 51 записей глоссария) **не** воспроизведены: другая выборка полей | Закрывается, когда 59 из 273 сдвигаются измеренным слайсом на тех же 44 планах, либо владелец фиксирует отсутствие моста как намеренное с причиной; слайс 19.08 её не двигает. До этого утверждение «движок ищет и по-русски» неверно и в документах так писать нельзя |
| C106 | провайдер, объявляющий двуязычный поиск, каноническим реестром не создаётся — объявление возможностей мертво | **FACT** | Проверено 2026-08-18 по коду. `create_default_stock_providers` (`src/providers/registry.py:56`) возвращает Wikimedia, NASA, Internet Archive, Pexels и Pixabay; `LocalLibraryStockProvider` не входит. `grep` по `src`, `tests`, `tools`, `apps` даёт его только в `src/assets/provider_diagnostics.py:125` (пустой индекс, ради `capabilities`) и в двух тестах. Значит `query_languages=["en","ru"]` (`src/providers/local_library_provider.py:37`) и `PROVIDER_QUERY_LANGUAGES["local_library"]` (`query_adapter.py:96`) в реальном прогоне ни на что не влияют. Работает другой матчер — `rank_local_assets` → `search_local_assets` → `_score_asset` (`src/media_library.py:536`), который сравнивает `keywords`, `mood`, `channel_tags`, `scene_tags` и `title`/`description` не читает | **НЕ исправлялось.** Владельцы существующие: `src/providers/registry.py` и `src/providers/local_library_provider.py`. Запрещено: второй provider contract, второй реестр провайдеров и «включить на всякий случай» без решения владельца — локальный путь несёт права и попал бы в отбор целиком | Закрывается, когда объявление и факт совпадают: либо провайдер входит в канонический набор, либо объявление возможностей снято или помечено как неавтоматическое. `C40` и `C83` — соседи по тому же коду, но их гейт **PLAN-10D** шире этой строки и ею не открывается |
| C107 | единственная оценка, которая читает русскую курацию, в выбор не входит: `relevance_score` не участвует в `final_score` | **FACT** | Измерено 2026-08-18 офлайн на `assets/library/metadata/media_index.json` (86 записей) и сцене `scene_003` плана `2026-08-15_solnechnaya-panel-lovit-svet-tolko-dnem-nochyu`: `relevance` 15.0 → `final` 51.667, `relevance` 9.0 → 57.5, 6.0 и 5.0 → одинаковые 35.0 | **Закрыто owner decision 2026-08-18: `relevance_score` в `final_score` не входит, и это решение, а не пропуск.** Основания числами: прибор поле не видит — `tests/plan9d_corpus_builder.py:145` стирает его как ranker output, 0 из 1064 (v1) и 0 из 78 (v2) кандидатов его несут при 30 локальных записях в v2, поэтому веса от 0.075 до заведомо огромного 2.0 дали ровно baseline 6/14 и 2/10; чтобы что-то изменить на `scene_003`, весу нужно >0.972 за балл (обгон) и >0.556 (перевал гейта 60), а 20 251 из 23 800 пар (сцена, запись) на 280 планах стоят ровно на 3 — это тип, 16:9 и длительность, ноль смысла; курацию ранжировщик и так читает через `build_evidence` (`title`/`description`). Решение записано комментарием в `candidate_ranker.py` у самого `final_score`. Не покрыто: измерить исход веса нечем, пока корпус не несёт поле — переоткрытие требует корпуса v3. Третья оценка не заведена, `meaning_score` не тронут | Закрыта. Оставшийся вопрос «может ли локальная запись выиграть сцену» принадлежит `C108` и гейтам 60/75, не этой строке |
| C108 | порог допуска локального шортлиста пропускает весь индекс: смысловое совпадение для попадания в десятку не нужно | **FACT** | Измерено 2026-08-18 на реальном индексе (85 видеозаписей): при `min_score=1` для `['солнечная','панель']` проходят 85 из 85, из них 80 без совпадения; для `['градирня']` и `['кит','океан']` — 85 из 85, весь шортлист без совпадений | **Исправлено 2026-08-18** внутри существующих владельцев: `search_local_assets` получил правило допуска `require_lexical_match` (запись обязана повторить слово сцены), `rank_local_assets` перестал перекрывать дефолт владельца `min_score=4`, а `_score_asset` читает теперь и `title`/`description` (п. 49 описи), считая слово один раз на все три поля. Числа после: 85→5 (`['солнечная','панель']`), 85→0 (`['градирня']`, `['кит','океан']`), допущенных без общего слова 0; `measure --baseline` на обоих замороженных корпусах без сдвига — v1 6/14, v2 2/10, `changed_winners 0` на каждом. Цена принята владельцем, её число **исправлено по ревью** (`C111`): на 217 сценах, которые этот путь маршрутизирует в video, пустой шортлист 0 → 77, средний размер 10.00 → 4.53. **Не покрыто:** совпадение по служебному слову склеенного запроса — на реальном `primary_query` сцены `scene_003` порог проходят 10 из 85, двое из них держатся на словах `on` и `line`; это класс `C79`/`C110` и своё решение | Закрыта вместе с `C107`. `C40`/`C47`/`C83` — соседи, их гейт **PLAN-10D** этой строкой не открывается |
| C109 | `shot_type` уходит в текст поискового запроса как обычное слово | **FACT** | Измерено 2026-08-18 на всех 44 планах: из **655** запросов, которые `build_scene_queries` отдал бы `pexels`, **42** заканчиваются собственным `shot_type` сцены (`establishing`, `action`, `detail`, `context`, `evidence`, `payoff`); с якорем темы `C98` — **39 из 638**. Источник — одна строка `_english_queries` (`src/assets/query_adapter.py:781`), склеивающая `exact_english + [subject, action, place, shot]`. Все шесть имён — обычные английские слова, и на стоке они сужают выдачу не по смыслу сцены | **НЕ исправлялось.** LOW. Владелец существующий: `src/assets/query_adapter.py`. Важное соседство: тот же `shot_type` после `C99`/`C103` несёт офлайн-сигнал крупности в ранжировании, поэтому убрать поле из текста запроса можно только не трогая сигнал; обратное сломало бы уже принятое решение владельца | Закрывается, когда `shot_type` либо перестаёт быть поисковым словом, либо владелец фиксирует это как намеренное с причиной. Приёмка — то же число на тех же 44 планах плюс `measure --baseline` на обоих корпусах, если правка задевает отбор |
| C110 | предметом кадра становится слово, которое предмета не называет: прилагательные, числительные, наречия и инфинитивы проходят как сущности | **FACT** (числа и механизм); доля «не-предметов» — не перепроверена | Измерено 2026-08-18 на всех 44 планах: **551** `visual_intent`, **156** различных субъектов, из них **62** различных однословных кириллических (**270** вхождений). Механизм проверен прямыми вызовами: `looks_like_verb` = False для `выпрыгивать`, `зависнуть`, `нашли` (`src/content/visual_planning/entities.py:107`; `_VERB_SUFFIXES:46` не содержит инфинитивных окончаний); `is_filmable` = True для `быстрее`, `главное`, `солнечная`, `одна`, `двух`, `миллионы` (закрытый список `_NON_VISUAL:60`) и False для `лёд`, `мох`, `пар` — их срезает `MIN_ENTITY_CHARS = 4` (`:89`) и тот же порог `len(word) < 4` в `src/content/script_engine/text_analysis.py:96`. Оценка отчёта «≥29 из 62 и 83 из 270» — ручная вычитка его автора; здесь списки заново не размечались | **НЕ исправлялось.** Владельцы существующие: `src/content/visual_planning/entities.py` и общий `STOPWORDS` (`src/content/script_engine/text_analysis.py:39`). Запрещено: второй извлекатель сущностей и порог длины «по языку строки» вместо разбора слова. `C79` — про сопоставление уже выбранного слова, не про его выбор | Закрывается, когда порог и списки перестают быть однобокими по языку и это подтверждено числом на тех же 44 планах; если правка задевает отбор — плюс `measure --baseline` на обоих замороженных корпусах |
| C111 | принятая цена порога измерена не тем путём, которым ходит прод: маршрутизация сцены по `visual_type` в замере не участвовала | **FACT** | Найдено независимым `review-change` над `741885f..41632ca` и перепроверено 2026-08-18. Замер, попавший в тело коммита `0115816` и в первую редакцию `C108` (0 → 83 из 280, среднее 10.00 → 4.96), звал `search_local_assets` с `media_type="video"` для всех 280 сохранённых сцен, тогда как `rank_local_assets` (`src/news/asset_manifest_builder.py:1631`) спрашивает `image` для сцен с `visual_type` `image`/`animated_image`. По фактическому маршруту: 217 сцен video и 63 image, пустой локальный шортлист 63 → 140 из 280, среднее 7.75 → 3.51; собственный вклад слайса — 0 → 77 на 217 video-сценах (среднее 10.00 → 4.53), остальные 63 были пусты и до него (`C112`). Направление вывода не изменилось, число изменилось | **Исправлено 2026-08-18** в той же строке `C108` и в комментарии на вызове; коммит `0115816` не переписывается — исправление живёт строкой, а не задним числом. Запрещено: заявлять цену слайса числом, снятым в обход `rank_local_assets`, — маршрут по `visual_type` часть измеряемого пути | Закрыта. Урок общий: число приёмки снимается тем же вызовом, который делает прод, а не удобной его частью |
| C112 | локальная библиотека не содержит ни одной картинки, а каждая пятая сохранённая сцена просит именно её | **FACT** | Измерено 2026-08-18 на `assets/library/metadata/media_index.json`: 85 записей `video`, 1 `music`, `image` — **0**; каталог `assets/library/images/` пуст. Из 280 сцен 43 сохранённых планов **63** имеют `visual_type` `image`/`animated_image`, и `rank_local_assets` спрашивает для них `media_type="image"`, то есть локальный шортлист этих сцен пуст независимо от любого порога. Обнаружено при разборе `C111`; к правилу допуска `C108` отношения не имеет | **НЕ исправлялось.** Владельцы существующие: `tools/library/curated_index.py` (наполнение) и `src/media_library.py` (индекс). Запрещено: второй индекс, второй матчер и «подставить видео вместо картинки» на уровне отбора — это меняет смысл слота и требует своего решения | Закрывается, когда владелец либо наполняет библиотеку картинками и это подтверждено числом на тех же 63 сценах, либо фиксирует видео-только библиотеку как намеренную с причиной. Приёмка — то же число, снятое через `rank_local_assets` |
| C113 | дедупликация шортлиста сохраняла первую копию, а первой приходит не самый прямой запрос | **FACT** | Найдено собственной перепроверкой после ревью, измерено 2026-08-18: `build_scene_queries` отдаёт запросы провайдера **не** по возрастанию `fallback_level` — на сохранённом плане `2026-08-15_solnechnaya-panel-...` порядок отправки `[1, 2, 3, 4, 1, 3]`. Поэтому правило «оставляем первое вхождение» (`6be3d35`) могло сохранить копию, найденную запросом уровня 4, когда тот же ассет позже вернул запрос уровня 1, и манифест сообщал бы менее прямое происхождение. На отбор это не влияло: решающий `fallback_level` ранжировщик считает из свидетельств (`candidate_ranker._candidate_fallback_level`), а не из запроса | **Исправлено 2026-08-18** в том же владельце `_deduplicate_candidates`: позиция берётся у первого вхождения (порядок пула — тай-брейк ранжирования и не должен зависеть от того, какая копия пришла), а выживает копия с наименьшим записанным `fallback_level`. Доказано тестом `test_the_surviving_copy_names_the_most_direct_query`; заодно исправлено ложное обоснование в docstring | Закрыта. Оговорка «первый запрос — самый прямой» неверна и в документах так писать нельзя |
| C114 | число в обосновании `C107` описывало прежнюю формулу счёта, а не ту, рядом с которой стоит | **FACT** | Найдено собственной перепроверкой (совпало с residual-замечанием ревью) и измерено 2026-08-18: распределение баллов локального матчера на 23 800 парах (сцена, запись) 280 сохранённых сцен даёт на «бесплатных» трёх баллах **20 251** пару при текущем коде, а записанные 20 366 были сняты до того, как матчер начал читать `title`/`description`. Максимум 24 в обоих замерах, вывод тот же | **Исправлено 2026-08-18** в строке `C107` и в комментарии `candidate_ranker.py`. Запрещено: оставлять в коде число, снятое на прежней версии того же кода, — оно перестаёт воспроизводиться первой же проверкой | Закрыта. Правило то же, что у `C111`: число берётся из того кода, рядом с которым оно записано |


## Species name wired into the general ranker 2026-08-18 (C115)

Найдено при разборе `C104`: единственный запрет автора, который ранжировщик
понимал не буквально, был вычеркнут по имени вида. Строка записывает находку и
закрывается тем же слайсом.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C115 | общий ранжировщик знал имя одного вида: запрет `dolphin` вычёркивался из `negative_matches` на orca-сцене | **FACT** | Найдено при разборе `C104` и измерено 2026-08-18: `candidate_ranker.py:341-344` снимал `dolphin` с `negative_matches`, если сцена просит orca/killer whale, текст даёт орка-свидетельство, а `title`/`description` не говорят `dolphin` — единственное место, где заявленный запрет автора значил не то, что написано. Причина была настоящей: Commons кладёт orca-клип в категорию «People with dolphins». Охват: 18 сцен с exact-orca субъектом, исключение спасает **1** запись | **Исправлено 2026-08-18** удалением исключения внутри существующего владельца, решением владельца в слайсе `C104`. `exact_orca_evidence`, `ambiguous_whale_for_orca` и `missing_orca_evidence_for_orca_scene` не тронуты: клип отказывается по запрету автора, а не за недоказанную орку. Цена измерена до правки: `must_avoid_match` 4→**5** из 1928 кандидатов в 212 сценах, победитель сцены не меняется (`pixabay_image_74081` и с исключением, и без), в обоих корпусах запрета `dolphin` нет вовсе. Тест `test_a_banned_word_in_the_metadata_disqualifies_whatever_the_species` перевёрнут из пина прежнего поведения | Закрыта. Потеря названа: настоящий orca-клип, чью таксономию провайдер подписал «dolphins», теперь отказывается, и автор, которому он нужен, не должен запрещать это слово. Правило «первичный текст перекрывает широкий тег» — новый механизм и отдельное решение владельца |


## Finding of the independent review of C104 2026-08-18 (C116)

Независимый `review-change` диапазона `ee253b9..1a513b5` — scope PASS, objective
PASS, ноль BLOCKING и две находки класса MINOR. Обе верны, обе закрыты тем же
слайсом; строка записывает их и правило, без которого число не повторяется.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C116 | цена слайса `C104` записана без правила сборки пула, и вторая сборка дала другой знаменатель | **FACT** | Найдено независимым `review-change` над `ee253b9..1a513b5`: числа «1928 кандидатов» и `must_avoid_match` 4→5 из `C97`/`C115` у ревьюера не воспроизвелись — он собрал пул как `candidates`+`rejected_candidates` без дедупа и получил 1878 и 1→2. Перепроверено своим замером 2026-08-18: сцен с пулом 212 в обеих сборках, поимённо совпали и единственный перевернувшийся кандидат `wikimedia_44588420`, и неизменность победителя во всех 212 сценах | **Исправлено 2026-08-18** записью правила, а не числа: пул сцены — объединение `ranked_candidates`, `rejected_candidates` и `candidates` с дедупом по `asset_id`, а для 50 записей без него — по `provider`+url+`title`. При этом правиле 3620 сырых записей дают **1928**; наивный дедуп, схлопывающий безымянные записи в одну, даёт 1888, сборка ревью — 1878. Числа в `C97` и `C115` остаются; коммиты `96c2deb` и `1a513b5` не переписываются. Вторая находка (MINOR) — устаревшая фраза в docstring `_mentions_avoided` про токенизацию правилом построения запросов — исправлена тем же слайсом | Закрыта. Заодно снята двусмысленность формулировки `C104`: «сцен без запроса 0→0» — это сцены, замолчавшие из-за запрета; сцен, которым лестница и так не даёт ни одного запроса, **187 из 280**, и это `C105`, а не цена этого слайса. Урок тот же, что у `C111`: число приёмки не воспроизводится, пока не названы вызов и вход, на которых оно снято |

## Cause named for a number that has two causes 2026-08-18 (C117)

Перепроверка утверждения из тела коммита `1945437`. Число верное, причина —
только для 39% случаев; строка записывает разложение и запрещает использовать
общий знаменатель как охват `C105`.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C117 | молчание сцены записано одной причиной, хотя из 187 молчащих сцен 114 молчат из-за формата плана, а не из-за языка | **FACT** | Измерено 2026-08-18 на тех же 43 планах (280 сцен) двумя путями. Лестница (`legacy_format.semantic_scene_queries` над блоком `semantic`) даёт **348** запросов и **187** сцен без единого — число тела `1945437` воспроизводится. Разложение: **114** сцен не несут `semantic` вовсе (18 проектов формата до Q2, 18–24.07), **73** несут только кириллические поля, которые срезает `provider_language_text`; из-за запрета, порога в два термина или локатора не молчит **ни одна**. Все 187 несут непустой `primary_query` (72 — отставной широкий `nature science wildlife observation`). Прод-путь `build_scene_queries` против `pexels` на тех же планах: **656** запросов ok, молчащих **59** — знаменатель самой `C105` | **Исправлено 2026-08-18** этой строкой: «187 из 280 не получают ни одного запроса, и это `C105`» верно числом и неверно причиной для 114 из 187. `1945437` не переписывается. Запрещено: приводить 187/280 как охват `C105` (её охват 59/280) и заводить вторую строку на 114 сцен устаревшего формата — это возраст планов, не дефект отбора | Закрыта. Правило то же, что у `C111`, `C114` и `C116`: рядом с числом стоит путь, которым оно снято |


## Guard quoted where there is nothing to guard 2026-08-19 (C118)

Перепроверка того, что за 18.08 сдвинулось в отборе. Числа в строке снимались
одним и тем же прибором на HEAD `ca10444` и на дереве `7e8fded`; путь назван
рядом с каждым, как требуют `C111`, `C116` и `C117`.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C118 | `blind agreement` v2 приводится как сторож у слайсов, которых прибор не исполняет: замер начинается после построения запроса и после допуска в пул | **FACT** | Измерено 2026-08-19 счётчиком вызовов по code-object (`sys.setprofile`) вокруг того же `run_metadata_baseline`, который зовёт `measure`. На v2: `plan_topic_anchor`, `_anchored_to_topic`, `build_slot_queries`, `_avoided_match_tokens`, `search_local_assets`, `rank_local_assets` — **0** вызовов; `rank_candidates` — **11** (на v1 те же нули и **14**). Корпус не несёт и полей: `topic_entity` — 0 вхождений, `relevance_score` — 0 при 78 карточках (v1: 0 при 1064). Достижимый слайс `C99`/`C103` применим к **2 сценам v2 из 11** против **9 из 14** на v1. Все четыре слайса 18.08 против `7e8fded` дают `changed_winners` **2** на v1 (4/14 → 6/14) и **1** на v2 (`live_5/scene_001` `C5`→`C7`, miss→miss) | **Записано строкой, а не механизмом**; прибор и корпус не меняются. Запрещено: приводить `2/10` или `changed_winners 0` как сторож для слайса, чьего владельца прибор не вызывает, без строки рядом — какой вызов не исполняется. Достаточно: у числа приёмки стоит счётчик вызовов. Снимается корпусом v3 (платный capture, owner decision). Evidence — [PLAN_9D_SLICE_REACHABILITY_2026-08-19.md](../audits/PLAN_9D_SLICE_REACHABILITY_2026-08-19.md) | Открыта до capture v3. Заодно исправлено число отчёта 18.08: локальных записей в пуле `local_after_fix/scene_001` **пять**, а не шесть; вывод §4 не меняется |
| C119 | число охвата слайса `C105` названо правилом сборки `C116`, но популяция у него другая и в тексте не названа | **FACT** | Найдено независимым `review-change` над `3ccf04a`: ревьюер собрал пул по правилу `C116` и получил 212 сцен и 3620 сырых записей вместо заявленных 262 и 3447, а сцен с запретом 42 вместо 61. Перепроверено своим разбором: 262/3447 — это все записи под `projects/`, где в одном объекте лежат `semantic_scene` и непустой пул (`assets_manifest.json` 183/1803, `sources.json` 65/580, `capture_raw.json` 14/1064); популяция `C116` — только `assets_manifest.json` и без требования `semantic_scene`. Числа верны каждое в своей популяции и несравнимы | **Исправлено 2026-08-19** поправкой в разделе Verification [ADR 0023](../adr/0023-unanswerable-ban-recorded.md): популяция названа файлами и условием, отличие от `C116` записано явно. `3ccf04a` не переписывается. Проверяемые числа слайса (5 сцен, 50 карточек, 15 терминов, `changed_winners` 0) ревью воспроизвело независимо | Закрыта. Тот же урок, что у `C111`, `C114`, `C116` и `C117`: имя правила дедупа не заменяет имени популяции |
| C120 | новое поле ранкера не попало в список, которым строитель корпуса срезает вердикты ранкера | **FACT** | Найдено тем же ревью `3ccf04a`. `RANKER_OUTPUT_KEYS` (`tests/plan9d_corpus_builder.py:134`) объявляет, что срезается **каждый** ключ, который производит ранкер, и перечисляет `must_include_unverifiable` и `negative_matches`; `must_avoid_unverifiable` в списке не было. Сегодня ущерб нулевой — ни один сохранённый манифест поля не несёт, оно младше их, — но первая же пересборка корпуса с проектов, прогнанных новым кодом, внесла бы вердикт ранкера в запись, объявленную «только то, что дал провайдер» | **Исправлено 2026-08-19**: ключ добавлен в `RANKER_OUTPUT_KEYS`. Замороженные корпуса `tests/data/plan9d/*` не пересобирались и не менялись | Закрыта |


## Meaning does not separate the survivors 2026-08-19 (C121–C122)

Пятый класс отказов PLAN-9D, которого не было в разборе 18.08 как класса. Числа
сняты одним и тем же production-путём (`rank_candidates` через загрузчик
`tests.plan9d_ground_truth`) на обоих замороженных корпусах, офлайн; рядом с
каждым числом стоит популяция и вызов, как требуют `C111`, `C114`, `C116`,
`C117` и `C119`.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C121 | у выживших совпадает не только `semantic_score`, а вся прочитанная оценка требования, и запись не говорила, что победителя выбрал не смысл | **FACT** | Измерено 2026-08-19 `rank_candidates` на обоих замороженных корпусах, офлайн. Популяция — сцены с ≥2 отбираемыми кандидатами после гейтов: **9 из 11** на v2, **8 из 14** на v1. `semantic_score` первых двух совпадает у **5** и **7**. Совпадает и весь вектор (`subject`/`action`/`environment`/`camera`, `support_status`, `slot_verdict`, `fallback_level`, поправка крупности) — у **5 из 5** на v2 и **6 из 7** на v1; группы 2/4/2/3/4 и 6/2/2/2/**12**/2, кроме v1 `scene_006`, где различает `C99`. Исход решает `0.075 * vertical_score` либо, в **3 сценах** (`scene_010`, `scene_011`, `local_after_fix/scene_002`), **ничто**: `final_score` равны, `sorted` стабильна, побеждает порядок манифеста | **Исправлено отчётом, не предпочтением**: `meaning_tie_peers`/`meaning_tie_broken_by` у `candidate_ranker.rank_candidates`, оба в `RANKER_OUTPUT_KEYS`. Решение, два измеренных в отказ правила и запрет подгонки под разметку — [ADR 0024](../adr/0024-meaning-tie-named-not-broken.md). Запрещено: второй ранкер, порог или вес без owner decision | Открыта как класс: равенство названо, не разрешено. Приёмка: `blind agreement` v1 **6/14**, v2 **2/10** не сдвинулись, `changed_winners` **0**. Заодно исправлено: вторая запись `live_5/scene_004` — `C7` 3840×5760, а не 2208×3312 (`C1`, третья); зазор 0.001 и вывод §4 верны |
| C122 | три ключа `shot_scale_*`, которые производит ранкер, не попали в список, которым строитель корпуса срезает вердикты ранкера | **FACT** | Найдено при добавлении полей `C121`. `RANKER_OUTPUT_KEYS` (`tests/plan9d_corpus_builder.py`) объявляет, что срезается **каждый** ключ, который производит ранкер; `shot_scale_intent`, `shot_scale_observed` и `shot_scale_adjustment` (`C99`, 18.08) в нём отсутствуют. Ущерб сегодня нулевой и это проверено: в замороженных корпусах литерал `shot_scale_adjustment` встречается **0** раз в обоих файлах — поля младше сохранённых манифестов. Первая же пересборка корпуса с проектов, прогнанных кодом после `C99`, внесла бы вердикт ранкера в запись, объявленную «только то, что дал провайдер» | **Записано строкой, не исправлено в этом слайсе**: правка чужого слайса (`C99`) вместе с функциональным изменением смешала бы два изменения. Владелец тот же — `RANKER_OUTPUT_KEYS`. Ровно тот же дефект, что `C120`, и третье его повторение подряд: список ведётся вручную и не проверяется ничем | Открыта. Закрывается одной строкой в списке; достаточным закрытием считается тест, который сверяет `RANKER_OUTPUT_KEYS` с ключами, которые `_score_candidate` фактически добавляет к записи провайдера, — иначе четвёртое повторение неизбежно |


## One identity for two different records 2026-08-19 (C123)

Единственный красный тест full offline suite на `e517c69` (2455 тестов, 1 отказ).
Дефект — в offline-фикстуре провайдера, не в отборе; прод-путь в отказе не
участвует.

Источник назван бисекцией: `test_fake_provider_no_network_pipeline_reaches_final_render_with_local_paths`
зелёный на `0115816` и красный начиная с `6be3d35` — коммита, который ввёл
`_deduplicate_candidates`. Механизм на `scene_006` того же прогона: пул до дедупа
11 записей, после — 3; из пяти формулировок под одним `fake_scene_006_video_001`
выживала английская `Why whales swim ocean`, русская `историю появляются данные`
отбрасывалась. Против русской сцены (`visual_priority` `exact_subject`, порог 75)
английские метаданные undecidable, поэтому `meaning_score` 0 и отказ
`fallback_level_not_allowed;score_below_75` — при том что `fallback_level` самого
запроса был 1, а не 4: ранкер считает его из свидетельств. Тот же приём, что во
внесённой правке, уже стоит у наследника фикстуры
(`tests/test_slot_aware_retrieval.py:221`): меняешь метаданные — меняй и id.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C123 | offline-фикстура провайдера давала двум разным записям один `asset_id`, и дедуп шортлиста молча оставлял от сцены одну формулировку из пяти | **FACT** | Измерено 2026-08-19 на прогоне, где этот тест — единственный красный: `FakeStockProvider.search` писал `title`/`description` из `request.query`, а `provider_asset_id` — как `{scene_id}_{media_type}_001`, одинаковый для всех запросов лестницы. Коммит-источник, пул `scene_006` (11 → 3) и причина отказа — в абзаце над таблицей | **Исправлено 2026-08-19 в фикстуре, а не в отборе**: `provider_asset_id` получил 8 символов sha1 от нормализованного запроса — личность следует содержанию. `_deduplicate_candidates` не трогался: дедуп по `asset_id` верен, дефект был в записи, объявлявшей два разных документа одним ассетом. Запрещено: чинить это порогом, лестницей или правилом дедупа — прод в этом отказе не участвует | Закрыта. Пин — `test_fake_provider_gives_two_differently_worded_answers_two_identities`: две формулировки дают две личности, повтор одного запроса — одну |

**Что этот отказ не закрывает.** `scene_006` — boilerplate-концовка `legacy_template`
(«…что мы упускаем в этой истории?»); анализатор извлекает из неё `subject`
`историю` и `action` `появляются`. Сцена без визуального предмета, помеченная
`exact_subject` с порогом 75, — вопрос к шаблону сценария, а не к фикстуре, и
этой строкой он не решается.


## Query anchor after the first paid end-to-end run 2026-08-19 (C124)

Причина 2 отчёта `docs/audits/CONTENT_CREATOR_E2E_2026-08-19.md`. Замер по всем 13
сценам сохранённого прогона, офлайн, тем же владельцем: сцен, не способных искать
собственный предмет, 6 из 13 → 0 из 13; запросов с чужим предметом впереди
43 из 63 → 18 из 63; число отправляемых запросов не изменилось (63 → 63). На
замороженном корпусе сцена со своим предметом: 8 из 11 → 11 из 11. Совпадение
проверяется по стеблям, а не по смыслу: синонимичная ступень (`brain neural
network` при предмете `neurons`) якорь сохраняет, и это названо, а не закрыто.
Плата названа:
10 из 41 уникальных запросов корпуса уходят без предмета темы — это дрейф самого
брифа (`C98`: «бриф теряет тему видео»), который переписывание запроса прятало.
Закрывать его надо в планировании брифа, а не в строке запроса.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C124 | якорь темы `C98` дописывался в **каждый** английский запрос **каждой** сцены и переписывал предмет, объявленный самой сценой | **FACT** | Измерено 2026-08-19 на сохранённом плане первого платного end-to-end прогона (тема «тело», якорь `sleeping person`): **6 сцен из 13** не могли искать собственный предмет ни на одной ступени — `human brain` уходил как `sleeping person human brain anatomy animation`, `neurons` и `brain scan` так же. Заполнились ровно три сцены, и все три те, где предмет сцены совпадал с предметом ролика. Приёмка `C98` была переписью запросов и улучшения отбора не заявляла | **Исправлено 2026-08-19** у существующего владельца `src/assets/query_adapter.py`: якорь применяется к ступени, которая не называет ни тему, ни предмет сцены (`_scene_subject_stems` из тех же двух полей, что и вывод якоря); одно правило на `build_scene_queries` и `build_slot_queries`. Решение — [ADR 0025](../adr/0025-topic-anchor-does-not-overwrite-scene-subject.md). `K9`, запись `query_subject_unverified` и запрет смешанного скрипта не тронуты | Закрыта. Приёмка `C98` уточнена, а не отменена: 15 из 42 → **10 из 41** вместо 0, и все десять называют предмет своей сцены. Пин — `tests/test_query_topic_anchor.py` (22 теста) |


## Owner decision on scientific animation 2026-08-20 (C125)

Причина 4 отчёта `docs/audits/CONTENT_CREATOR_E2E_2026-08-19.md` и прямое следствие
замера `E2E_ASSET_SEARCH_RECHECK_2026-08-20.md`. На сохранённом плане в новый класс
попадают 4 сцены из 13 (`scene_004`, `scene_006`, `scene_007`, `scene_008`), остальные
девять класс не меняют. Выигрыш ожидается на `scene_008`; `scene_004` этим решением не
открывается — её потолок 58.8 ниже порога 60, и это отдельная причина.

Remotion и моушн-дизайн этим решением **не** вводятся: это второй движок рендера, и
он не приблизил бы MP4 — три из пяти недостающих сцен блокирует потолок балла.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C125 | запрет «не живая съёмка» был один на все сцены и оставлял пустой навсегда сцену, чей предмет камера снять не может | **FACT** | Измерено 2026-08-20 на сохранённом прогоне: `scene_008` нашла `abstract neural network connections animation` с баллом **64.7** при пороге 60, и единственной причиной отказа было `non_real_video_footage:animation`. Сцена осталась пустой, а `preview_render` заблокирован гейтом «нет пустых сцен». Для нейронов, мозга, клеток и молекул живой съёмки не существует, то есть запрет был вечным | **Исправлено 2026-08-20** по owner decision ([ADR 0026](../adr/0026-scientific-visualization-source-class.md)): новый класс `scientific_visualization` в существующем каталоге `scene_strategy` со своей строкой `PROVIDER_PRIORITY` и `REQUIRED_SLOT_KINDS`; запрет расщеплён на `ENTERTAINMENT_VIDEO_TERMS` (отказ для всех классов) и `SYNTHETIC_VIDEO_TERMS` (не отказ только здесь). Класс читается из объявленного предмета, а не из narration. Порог, права, длительность, `must_avoid`, кадрирование и слоты не ослаблены | Закрыта частично. Пин — `tests/test_scientific_visualization_class.py` (8 тестов). **Residual:** класс назначает только словарь или явный бриф — `RESPONSE_CONTRACT` смысловой модели поля `source_class` не имеет, и модель этот класс назвать не может |


## Catalogue page read as a naming 2026-08-20 (C126)

Второй независимый пример класса `C121` из `E2E_ASSET_SEARCH_RECHECK_2026-08-20.md`,
доведённый до механизма. Владелец тот же, что у `C79`/`C89` —
`src/assets/semantic_selection/evidence.py`. Потолок `58.824` этой строкой **не**
закрывается и остаётся следующим блокером.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C126 | качество поля читалось по имени поля и никогда по его размеру, поэтому фраза внутри каталожной страницы делала то же утверждение, что заголовок, который предмет называет | **FACT** | Измерено 2026-08-20 офлайн из сохранённых манифестов, без обращений к провайдерам. Фраза один раз в описании на 604 токена = `100.0`, столько же, сколько у заголовка из 12 токенов, называющего предмет: четыре кандидата `scene_004` вернули один `semantic_score = 58.824`. Высший балл прогона **90.588** (`scene_010`, `closed eyes`) пришёл из **1370-токенного** дампа ключевых слов; `scene_003` (`sleeping man`) выбирала страницу коллекции `Erotic Art Gallery` с баллом **94.231** | **Исправлено 2026-08-20** у владельца `evidence.py` ([ADR 0027](../adr/0027-catalogue-page-supports-a-claim-it-does-not-make-one.md)): поле длиннее `MAX_NAMING_FIELD_TOKENS = 120` подтверждает утверждение и не делает его — вклад ограничен сверху `SUPPORTING_EVIDENCE_SCORE` (переименованный `DESCRIPTION_SINGLE_WORD_SCORE`, то же 75.0), константа в измеренном промежутке 101..152. Веса, `MIN_SCORE`, `EXACT_SUBJECT_MIN_SCORE` и `_ranking_key` не тронуты; `must_avoid`, `conflicting_context` и `must_include` `semantic_concept_score` не вызывают | Закрыта частично. Пин — `tests/test_naming_field_evidence.py` (12 тестов). `blind agreement` v1 **6/14**, v2 **2/10** не сдвинулись, `changed_winners` **0**; `C121` впервые сдвинут смыслом — групп равенства на v1 6 → **5**. **Residual:** потолок `58.824` не закрыт |


## Silence spent as a mismatch 2026-08-20 (C127)

Owner decision 2026-08-20: порог `MIN_SCORE = 60` и веса `0.45/0.20/0.15/0.05` не
трогать, под `58.824` не подстраиваться. Замер **до** кода поправил постановку: из
четырёх сцен на потолке `UNKNOWN` блокирует **одну** (`scene_004`); в `009`/`012`/`013`
предмет не назван ни одним кандидатом, и наверху там мультфильмы. Тогда эта причина была
названа «recall»; замер 2026-08-20 после слайса это опроверг — см. `C128`/`C129`/`C130`.
Строкой `C127` она в любом случае не закрывается.

| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C127 | молчание каталога тратилось как доказательство несоответствия, и отказ был циклическим: низкое среднее само себя объявляло дисквалификацией | **FACT** | Измерено 2026-08-20 офлайн, 1419 кандидатов, без обращений к провайдерам. `meaning_score < MIN_SCORE` → `mismatched` → `semantically_disqualified` → `support = unsupported` → `∉ SOFT_REJECT_ELIGIBLE` → `score_below_60` остаётся блокирующим. `SOFT_REJECT_PREFIXES` написаны ровно для того, чтобы вердикт слотов перевешивал среднее, и цикл не давал им сработать. `scene_004`: предмет `MATCH 100` в заголовке из 12 токенов, действие и место `UNKNOWN silent`, единственная причина отказа `score_below_60`, сцена пустая | **Исправлено 2026-08-20** ([ADR 0028](../adr/0028-silence-is-not-a-mismatch.md)): низкое среднее не дисквалифицирует, если предмет назван (`subject_match >= SLOT_MATCH_SCORE`, что после `C126` приходит только из поля про один ассет), ничто не противоречит и каждое остальное требование — ровно ноль. `EXACTING_CLASSES` исключён целиком: такой класс определён отказом угадывать. Ни поля схемы, ни формулы, ни второго движка; веса, `MIN_SCORE` и `semantic_score` не тронуты | Закрыта частично. Пин — `tests/test_unanswered_is_not_mismatch.py` (16 тестов; мутации: выключено 5 RED, безусловное 2 RED). `blind agreement` v1 **6/14**, v2 **2/10** не сдвинулись, `changed_winners` **0**, отказано заново **0**, заполнена 1 сцена. **Residual:** названный предмет может быть не тем (омоним `панель`); `009`/`012`/`013` — recall |

| C128 | оценка совпадения считает долю слов, и служебные слова входят в неё наравне со знаменательными | **FACT** | Измерено 2026-08-20 офлайн на сохранённом `sleep-anchor-recheck`, без сети ([отчёт](../audits/RECALL_BLOCKER_009_012_013_2026-08-20.md)). `stem_concept_score` — `matched / len(words)` ([evidence.py:342](../../src/assets/semantic_selection/evidence.py)), веса у слов равные. `scene_012`, предмет `morning light on a bed`: честный ролик `pexels_video_7240845` даёт `-morning -light +on +a +bed` = `3/5` = **60.0** — все 60 из служебных слов и `bed`, оба смысловых слова по нулю. Ровно **60.0** получает мультсериал `blues-clues-notebook-drawings`, у которого `title`/`tags`/`keywords` дают `0/5`, а все 60 приходят из длинной расшифровки эпизода. Итог сцены: мультфильм **54.31**, честный ролик **41.57** | **Не исправлено.** Owner decision до работы: это правка примитива сопоставления, то есть HIGH (отбор). Кандидат на исправление — считать долю **знаменательных** слов, а не всех; порог и веса при этом не трогать | Открыта. Пина нет. `C126` уже ограничил длинное поле потолком `SUPPORTING_EVIDENCE_SCORE`, и это работает, но 75 в длинном описании продолжает бить короткий заголовок, который отвечает только на служебные слова |
| C129 | морфология английского не связывает герундий с исходной формой: `waking` не совпадает с `wake` | **FACT** | Измерено 2026-08-20 офлайн, там же. `morphological_variant` ([evidence.py:314](../../src/assets/semantic_selection/evidence.py)) возвращает False для пары `waking`/`wake`. `scene_013`, предмет `tired person waking`, порог **75**: в пуле лежит `pixabay_video_44647` с ключевыми словами `yawn, woman, sleep, night, fatigue, sleepy, face, morning, tired, wake, rest` — ровно тот материал; пословно `+tired +person -waking` = `2/3` = **66.7**, `semantic_score` **53.92**. При `subject = 100` было бы **71.57**, всё ещё ниже 75: действию нужно `>= 31.25`, а оно `16.7`, потому что объявлено предложением из шести слов | **Не исправлено.** Owner decision до работы: HIGH (отбор). Две причины в одной сцене — морфология и длина заявленной фразы; чинить их по отдельности и замерять по отдельности | Открыта. Пина нет. Одна морфология сцену `013` не открывает — это измерено, а не предположено |
| C130 | у визуального брифа два словаря, и оценка читает абстрактный, а конкретный до неё не доходит | **FACT** | Измерено 2026-08-20 офлайн, там же. `visual_brief.provider_queries` конкретны и стоковы (`student studying desk lamp evening`), `semantic_scene.subject` абстрактен (`person practicing a skill`). Поиск читает первый, оценка — второй; `provider_queries` читают только [query_adapter.py:784](../../src/assets/query_adapter.py) и [completion/report.py:95](../../src/assets/completion/report.py). `scene_009`: годный кандидат `pexels_video_8550149` (`person, studying, on, paper`) получает `+person -practicing -a -skill` = `1/4` = **25.0**, а сцену выигрывает **46.08** у записи студенческого телевидения с музыкальным кавером. **Подтверждено полным пулом** (`012`, 240 кандидатов, бесплатный `provider_search` по разрешению владельца): **0 заполняемых**; с называемым предметом `bed in the morning` — **4**, первый `pexels_video_9786285` с **82.4**. Пул, код, пороги и веса не менялись | **Не исправлено.** Owner decision до работы. Сам запрос обратно в evidence не возвращать: это уже было и делало совпадающим любого кандидата (отказ назван в докстринге `_field_match`). Речь о наблюдаемом предмете сцены, а не о тексте запроса | Открыта. Пина нет. Второй движок оценки при этом не создаётся: владелец формулировки — `src/content/visual_planning/brief.py`. **Ограничение:** предмет должен стать **называемым**, а не **коротким** — на укороченном тоже проходит музыкальный альбом |
