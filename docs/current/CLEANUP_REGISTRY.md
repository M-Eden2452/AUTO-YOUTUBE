...
| C130 | у визуального брифа два словаря, и оценка читает абстрактный, а конкретный до неё не доходит | **FACT** | Измерено 2026-08-20 офлайн, там же. `visual_brief.provider_queries` конкретны (`student studying desk lamp evening`), `semantic_scene.subject` абстрактен (`person practicing a skill`). Поиск читает первый, оценка — второй; `provider_queries` читают только [query_adapter.py:784](../../src/assets/query_adapter.py) и [completion/report.py:95](../../src/assets/completion/report.py). `scene_009`: годный кандидат `pexels_video_8550149` (`person, studying, on, paper`) получает `1/4` = **25.0**, а сцену выигрывает **46.08** у записи студенческого телевидения. **Подтверждено полным пулом** (`012`, 240 кандидатов, бесплатный `provider_search` по разрешению владельца): **0 заполняемых**; однофакторно, при неизменных `action`/`environment`, с называемым `bed in the morning` — **1**, и это нужный ролик `pexels_video_9786285` (**66.7**, subject 100). Пул, код, пороги и веса не менялись | **Не исправлено.** Owner decision до работы. Сам запрос обратно в evidence не возвращать: это уже было и делало совпадающим любого кандидата (отказ назван в докстринге `_field_match`). Речь о наблюдаемом предмете сцены, а не о тексте запроса | Открыта. Пина нет. Второй движок оценки при этом не создаётся: владелец формулировки — `src/content/visual_planning/brief.py`. **Ограничение:** предмет должен стать **называемым**, а не **коротким**: `bed` даёт 73 заполняемых из 240 |
