...
## LIVE-5 findings (C83–C84)

Внесено 2026-08-15 по итогам живого retrieval-прогона LIVE-5
(`docs/audits/LIVE_5_2026-08-15.md`, HEAD `68c46cd`). Оба дефекта **не
исправлялись**, новый PLAN-ID не создавался, номер VA-NEW не выдавался, статусы
PLAN-10A/10B/10C/10D не менялись. LIVE-5 — owner packaging label, не plan step.
...
| C84 | потолок вызовов semantic brief действует на адаптер, а не на проект | **FACT** | `build_semantic_brief_adapter()` вызывается заново внутри `src/news/visual_plan.py:75`, счётчик живёт на экземпляре (`self.calls = 0`, `src/content/semantic_brief_openai.py:219`; проверка `self.calls >= maximum_calls_per_project`, `:229`). Каждый build плана получает полную квоту заново. `_accumulate_semantic_brief_usage` складывает **отчётность** по replan'ам и прямо это документирует («Carry prior spend forward while keeping each adapter's call cap unchanged», `src/news/draft_completion.py:326`). Измерено в LIVE-5: при `maximum_calls_per_project: 12` persisted `calls` = **15** (`localizations/ru/visual/visual_plan.json`) при 5 в master-снимке; учёт честный, имя ключа — нет. Денежный потолок при этом удержан: $0.15 при заявленном $1 | `правка` внутри существующего semantic-brief live owner: либо счётчик становится project-scoped, либо ключ переименовывается в честный per-build. Второй бюджетный контур и второй owner не создаются. Уточняет ранее записанную ноту «Semantic-brief live limitation», которая называет `maximum_calls_per_project` hard runtime bound | существующий semantic-brief live owner (`PLAN-9B-PRODUCER-M-LIVE`) |
