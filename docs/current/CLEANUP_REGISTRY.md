...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C97 | `must_not_include` проверяется двумя путями, и дисквалифицирующий из них доказуемость не спрашивает | **FACT** | Слот-слой (`decision.py:363`) спрашивает `evidence.is_undecidable(text)` перед `evidence.contains(text)` — это одна из пяти точек, которую пакет **D** перевёл на пофайловое чтение. Но сам отказ выносит не он: `candidate_ranker.py:233` считает `negative_matches` через `_contains_concept(word, all_tokens, text)` по склеенному тексту и без единого вопроса о доказуемости, а `:402` превращает это в `must_avoid_match:<термины>`. Разделение намеренное — docstring `declared_conflict_terms` (`decision.py:232-237`) объясняет, что повтор дал бы одному факту две причины отказа, — но означает, что несущий путь запрета живёт по своим правилам. Живого дефекта не найдено: сравнение литеральное, поэтому термин вне алфавита не совпадает и так; расхождение вывода на входах ревью не воспроизвело | **НЕ исправлялось.** Owner — существующие `candidate_ranker` и `decision` (тот же, что у `C79`/`C89`/`C91`). Запрещено: заводить третий путь запрета; ослаблять литеральное сравнение `must_avoid`; переносить отказ в слот-слой этой строкой — это меняет число причин отказа и требует своего решения владельца | Закрывается, когда оба пути задают один вопрос одному примитиву, либо владелец фиксирует разделение как намеренное с причиной. До этого утверждение «один примитив на весь слой запретов» неверно и в документах так писать нельзя |
...
|---|---|---|---|---|---|
| C104 | русский `must_avoid` не защищает ролик: запрет на кириллице не сравним ни с чем в лестнице запросов и в приёмке брифа | **FACT** (структура и охват); исход отбора прибором не измерим — см. гейт | Перепроверено 2026-08-18: `_avoided_match_tokens` (`src/content/visual_planning/expansion.py:290`) токенизирует обе стороны через `_PROVIDER_TOKEN_RE` (`:54`, `[A-Za-z]…`), поэтому кириллический запрет даёт пустой список: `mentions_avoided('дельфин выпрыгивает из воды', ['дельфин'])` → False, а тот же смысл на латинице → True. Охват: из **186** запретов автора (`semantic.must_not_include`) в 42 сценах **26 кириллических**, и все 26 — в 5 сценах проекта `2026-08-13_polog-dozhdevogo-lesa`, где мертвы **все** запреты сцены. Примитив отбора юникодный (`semantic_selection/evidence.py:175`): на кириллице он срабатывает, но только против записи с русскими метаданными | **НЕ исправлялось.** Владельцы существующие: `expansion.py` (лестница `:246`, приёмка ответа модели `semantic_brief.py:312`) и `candidate_ranker`/`decision` — те же, что у открытой `C97`. Запрещено: третий путь запрета (`C97` — про два уже существующих), ослабление литерального сравнения, перенос отказа между слоями | **HIGH** (запреты и права): owner decision до работы — один вместе с `C97`, — независимый `review-change` после, findings — тестами. Исход измерить нечем: арм `metadata_only` прибора `tests/plan9d_ground_truth.py` не импортирует `expansion`, кириллических запретов в корпусах 0 из 21 (v1) и 1 (v2) |
