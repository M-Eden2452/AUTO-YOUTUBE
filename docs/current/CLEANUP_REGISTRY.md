...
`C83` при этом не единичный случай, а класс: в пяти `channel_config.json` **76**
листовых ключей, из них **14** не встречаются ни разу в `src/`, `tools/`,
`scripts/` (замер аудита тестов §7.5, перепроверен на этом HEAD). Слайс
