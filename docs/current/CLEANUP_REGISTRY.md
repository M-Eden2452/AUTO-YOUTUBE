...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C91 | вердикт «доказуемо/нет» выносится по склеенному тексту записи, а не по полю, дающему балл | **FACT** | Аудит продукта 2026-08-16 §6; подробности и обе приписки 2026-08-17 — `docs/audits/PLAN_9D_H_CORPUS_V2_2026-08-17.md` §11. `CandidateEvidence.is_undecidable` (`src/assets/semantic_selection/evidence.py:413-417`) спрашивает `script_mismatch` по склейке `self.text`, тогда как балл читает `self.fields` по отдельности (`:428-436`); XOR по кириллице (`:189-191`) делает английский термин несопоставимым при совпадении 100 по полю, а затем `candidate_ranker.py:307-320` обнуляет `meaning_score`, оставляя `final = 0.075·quality`. Измерено: 7.5 против 78.3 с английскими метаданными. Слой не покрывают ни `C79`, ни `C89`, ни `C83` | **НЕ исправлялось.** Owner — `evidence.py` (как у `C79`/`C89`/`C90`). Запрещено: второй примитив доказуемости, отдельный «RU-путь» отбора, изменение порогов и весов этой строкой. **Приёмка, уточнена 2026-08-17 (`K12`):** на корпусе v1 она язык не проверяет вообще (14/14 английских субъектов, 2 кириллических записи из 1064) и прошла бы при любой правке, поэтому нужны два числа — ноль изменившихся победителей v1 через `measure --baseline` и названный сдвиг на v2. **Вариант A** (пофайловый `is_undecidable`) шире прежней формулировки: тот же примитив чинит и слой запретов (`decision.py`, 3 вызова), поэтому строка накрывает и слот-слой — второй примитив не заводится | pre-v1 bounded correction; владелец `src/assets/semantic_selection` |
