...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C96 | прогон, который собирается платно озвучивать, не просит класс сети, без которого озвучка теперь отказывает | **FACT** | Пакет **E** (`d7fb378`, строка `C94`) закрыл дыру, поставив `require_network(voice_synthesis)` внутрь `ElevenLabsProvider.synthesize`. Декларация действий прогона за ним не пошла: `src/content_creation/wizard_state.py:163-164` при `voice_provider == "elevenlabs"` добавляет только `NETWORK_ACTION_VOICE_PREFLIGHT`; `voice_synthesis` не появляется ни в одном вызывающем. То есть мастер показывает владельцу список нужных действий, в котором не хватает того, на котором прогон и остановится. Симптом уже на ветке: `tests/test_news_voice_adapter.py:138,194` дают `partially_completed` вместо `completed`; коммит `d7fb378` заявил targeted-прогон по четырём другим модулям, этот в набор не попал. Обе падают и при восстановленном прежнем теле `is_undecidable`, то есть к отбору отношения не имеют | **НЕ исправлялось этой строкой.** Owner — `src/content_creation/wizard_state.py` (объявить `voice_synthesis` там же, где прогон объявляет намерение генерировать озвучку) и тесты голосового адаптера. Запрещено: переиспользовать `voice_preflight` вместо нового класса — инвариант `runtime_network` «одобренный класс не открывает соседние» именно это и запрещает; второй сетевой контур; ослабление проверки в провайдере | Закрывается слайсом пакета **E**: одобренный платный прогон проходит целиком, а список действий прогона называет `voice_synthesis` до старта |
