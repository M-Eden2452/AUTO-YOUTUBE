...
| C89 | ранжирующая оценка спрашивает литеральный вопрос о производном описании сцены | **FACT** | Найдено 2026-08-15 при исправлении `C79`; это его вторая половина, и решает исход именно она. `candidate_ranker._field_match` (`:639`) зовёт `evidence.semantic_literal_score` для subject/action/environment/location/camera, тогда как слот-слой для тех же полей зовёт `semantic_stem_score` (`decision.py:316`), а docstring `stem_concept_score` прямо называет производное описание сцены случаем толерантного примитива. Один кандидат судится двумя разными мнениями о том, что такое одно и то же слово. Измерено после закрытия стемовой половины `C79`: слот-слой называет subject совпавшим и verdict'ом `complete`, а `subject_match` остаётся `0.0`, поэтому `semantic_match_status` = `mismatched`, `support_status` = `unsupported`, `score_below_*` остаётся блокирующим и кандидат, чьи метаданные буквально называют предмет сцены, проигрывает записи, разделяющей с ним одно несклонённое слово. Не языковой частный случай: документированная самим модулем пара `antarctica`/`antarctic` на этом пути тоже даёт `0.0` | **НЕ исправлялось, и однострочная замена на `semantic_stem_score` в этом слайсе сознательно откачена.** Она измерена против замороженного ground truth PLAN-9D (`tests/test_plan9d_ground_truth_baseline.py`, `FrozenBaselineMeasurementTests`) и сдвинула его: `scene_009` ушла с предпочтённого аннотатором `C31` (wikimedia, photovoltaic panel) на `C36` (pexels, portable light tower), `preferred_matches` 4 → 3, `scorable_scenes` 14 → 13, `unscorable_winner_not_visible` 0 → 1 — то есть новый победитель вне размеченного набора и оценить его нечем. Стемовая половина `C79` отдельно проверена: она этот baseline **не** двигает (54 теста зелёные). Поэтому это не однострочная правка, а bounded slice со своим owner decision: чем ранжирующая оценка должна судить производное описание, и что делать с замороженным baseline — переизмерить и переутвердить или сохранить. Второй ранжировщик, новые пороги и веса не создаются | существующий владелец `src/assets/semantic_selection` (тот же, что у `C79`); переизмерение — `PLAN-9D` |
