...
| C108 | порог допуска локального шортлиста пропускает весь индекс: смысловое совпадение для попадания в десятку не нужно | **FACT** | Измерено 2026-08-18 на реальном индексе (85 видеозаписей): при `min_score=1` для `['солнечная','панель']` проходят 85 из 85, из них 80 без совпадения; для `['градирня']` и `['кит','океан']` — 85 из 85 и весь шортлист без совпадений | **Исправлено 2026-08-18** внутри существующих владельцев: `search_local_assets` получил правило допуска `require_lexical_match` (запись обязана повторить слово сцены), `rank_local_assets` перестал перекрывать дефолт владельца `min_score=4`, а `_score_asset` читает теперь и `title`/`description` (п. 49 описи), считая слово один раз на все три поля. Числа после: 85→5 (`['солнечная','панель']`), 85→0 (`['градирня']`, `['кит','океан']`), допущенных без общего слова 0; `measure --baseline` на обоих замороженных корпусах без сдвига — v1 6/14, v2 2/10, `changed_winners 0` на каждом. Цена принята владельцем и записана числом: пустой локальный шортлист 0 → 83 из 280 сохранённых сцен, средний размер 10.00 → 4.96. **Не покрыто:** мусорное совпадение по служебному слову склеенного запроса — на реальном `primary_query` сцены `scene_003` порог проходят 10 из 85, и двое из них держатся на словах `on` и `line`; это класс `C79`/`C110` и своё решение | Закрыта вместе с `C107`. `C40`/`C47`/`C83` — соседи, их гейт **PLAN-10D** этой строкой не открывается |
