...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C91 | вердикт «доказуемо/нет» выносится по склеенному тексту записи, а не по полю, дающему балл | **FACT** | Аудит продукта 2026-08-16 §6 (`docs/audits/PRODUCT_AUDIT_2026-08-16.md`). `CandidateEvidence.is_undecidable` (`src/assets/semantic_selection/evidence.py:413-417`) спрашивает `script_mismatch(concept, self.text)` — склейку всех полей записи, тогда как балл берёт `semantic_inflection_score`, читающий `self.fields` по отдельности (`:428-436`). `script_mismatch` (`:189-191`) — XOR по наличию кириллицы: один русский символ в заголовке делает английский термин сцены несопоставимым при совпадении 100 по полю `keywords`, а `candidate_ranker.py:307-320` затем обнуляет `meaning_score`, оставляя `final = 0.075·quality`. Измерено офлайн: запись курированной библиотеки даёт 7.5 при `semantic_inflection_score('factory assembly line') = 100.0`, а с английскими title/description — 78.3, обходя кадр Opel 73.0, выбранный движком в LIVE-5 scene_003. Слой не покрывают ни `C79`, ни `C89`, ни `C83` | **НЕ исправлялось.** Строка заведена аудитом и права на действие не даёт. Existing implementation owner — `evidence.py` (тот же, что у `C79`/`C89`/`C90`). Запрещено: второй примитив доказуемости, отдельный «RU-путь» отбора, изменение порогов и весов этой строкой. Приёмка правки: characterization-first, 7.5 → ≥78 и ноль изменившихся победителей замороженного корпуса PLAN-9D, сравнённых поимённо | pre-v1 bounded correction; владелец `src/assets/semantic_selection` |
