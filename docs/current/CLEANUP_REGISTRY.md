...
| `channels/*/prompts/`, `channels/*/templates/` | **FACT.** Обе директории во всех четырёх legacy-каналах содержат только `.gitkeep`. Промптов и шаблонов там нет — вывод сделан по содержимому, а не по именам директорий | `DELETE` | — | Восстанавливать нечего: пустые placeholder-директории |
