...
| ID | Предмет | Класс | Evidence | Action / disposition | Gate |
|---|---|---|---|---|---|
| C95 | `corpus_sha256` покрывает поля, которые вычисляет код, поэтому правка отбора отвязывает замороженный корпус от кода, а пересчёт — от слепой разметки | **FACT** | Измерено 2026-08-17 при закрытии `C91`. Правка сдвинула ровно одно поле во всём корпусе: `plan9d_current_capture_v1/scene_010` потеряла категорию `ambiguous_needs_review`; победители не менялись нигде (`changed_winners 0`). Хеш при этом уходит с `bfb4d02437f3c528` на `280e6da190b7fb03`, а `current_annotations_v1.json` записывает хеш, против которого делалась разметка, и `plan9d_ground_truth.py:786` отказывается мерить при несовпадении. Пересчёт корпуса осиротил бы единственную размеченную истину репозитория | **Пин, не разрешение.** `KNOWN_DERIVED_DRIFT` в `tests/test_plan9d_current_capture.py` называет сцену, потерянную категорию и правку-причину; тесты требуют, чтобы дрейф был **ровно** таким — шире краснеет, уже тоже краснеет. Тест `finalize_is_idempotent` разделён на два разных утверждения: идемпотентность функции и совпадение файла с кодом. Запрещено: пересчитывать корпус и переподписывать `corpus_sha256` в файле разметки; расширять пин без строки причины | Закрывается, когда хеш перестанет покрывать вычисляемые кодом поля; владелец — прибор PLAN-9D-H (пакет **C**), где тот же вопрос встанет сразу после разметки v2 |
