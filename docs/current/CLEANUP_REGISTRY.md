...
| C107 | единственная оценка, которая читает русскую курацию, в выбор не входит: `relevance_score` не участвует в `final_score` | **FACT** | Измерено 2026-08-18 офлайн на `assets/library/metadata/media_index.json` (86 записей) и сцене `scene_003` плана `2026-08-15_solnechnaya-panel-lovit-svet-tolko-dnem-nochyu`. `rank_local_assets` (`src/news/asset_manifest_builder.py:1598`) кладёт оценку локального матчера как `relevance_score` (`:1674`), а `final_score` (`src/assets/semantic_selection/candidate_ranker.py:431`) считается как `0.85·meaning + 0.075·quality + 0.075·vertical` минус штрафы — без неё. Числа сцены: `relevance` 15.0 → `final` 51.667, `relevance` 9.0 → `final` 57.5, то есть лучшая локально запись проигрывает; 6.0 и 5.0 дают одинаковые 35.0. Поле читает только `src/assets/review_bundle.py:92,269` и только как запасное, когда `final_score` нет. Пара отчёта «18.0 и 6.0 → 7.5» не воспроизводилась: ранжирование сменилось `C99`/`C103`, числа выше свои | **НЕ исправлялось.** Владельцы существующие: `rank_local_assets` и `candidate_ranker`. Запрещено: складывать две шкалы без приёмки числом (`relevance_score` — целые баллы совпадений, `final_score` — 0–100), заводить третью оценку и трогать `meaning_score` этой строкой | Слайс `K5`/`K6` из [ROLLOUT_PLAN_2026-08-17.md](../audits/ROLLOUT_PLAN_2026-08-17.md) — MEDIUM, офлайн, вместе с `C108`. Приёмка — `measure --baseline` существующим прибором на **обоих** замороженных корпусах: сдвиг заявляется числом, регрессий нет |
| C108 | порог допуска локального шортлиста пропускает весь индекс: смысловое совпадение для попадания в десятку не нужно | **FACT** | Измерено 2026-08-18 на реальном индексе (85 видеозаписей). `rank_local_assets` зовёт `search_local_assets` с `min_score=1` (`src/news/asset_manifest_builder.py:1618`) при дефолте самого владельца `min_score=4` (`src/media_library.py:81`). Для запроса `['солнечная','панель']` порог проходят **85 из 85**, из них **80** — без единого совпадения по ключевым словам; при `min_score=4` проходят 5 из 85. Для `['градирня']` и для `['кит','океан']` — снова 85 из 85, и все 10 записей шортлиста без совпадений, то есть шортлист вырождается в «десять самых длинных клипов индекса». Причина видна в `_score_asset` (`src/media_library.py:536`): совпадение типа, 16:9 и длительности дают по `+1` каждое, и одного уже достаточно | **НЕ исправлялось.** Владельцы существующие: `src/news/asset_manifest_builder.py` (значение на вызове) и `src/media_library.py` (дефолт и веса). Запрещено: двигать порог без измерения на обоих корпусах — это ровно тот путь, где вместо плохого кандидата появляется пустой слот, и цена ошибки несимметрична | Тот же слайс, что у `C107`, и та же приёмка: `measure --baseline` на обоих корпусах плюс число «сколько записей проходит порог» на том же индексе. Закрывается вместе с `C107`; `C40`/`C47` — соседи, их гейты этой строкой не открываются |
