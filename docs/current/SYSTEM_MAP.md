...
Provider normalizers, rights owner, media-selection owner и persisted schema не менялись. Текущий checkpoint — **PLAN-9D**. Mini plan reconciliation 2026-08-11 отменила прежнюю формулировку «следующий шаг — LIVE-5»: закрыты WP0-A machine gates (`98e58fe`, `a9bfc11`), VA-NEW-01 (`15cb20d`), VA-NEW-03 (`37ca498`) и VA-NEW-02 / M1-B (этот commit). M1-B не создал новую boundary или persisted schema: `src/assets/visual_preview.py` остаётся cache owner, local key v2 добавляет source SHA-256 и фактический transform. До LIVE-5 остаются VA-NEW-04, 05, 06, 08, 09 и минимальные budget guards 10/12 внутри существующих owners PLAN-9A, PLAN-9E, PLAN-10B и PLAN-10C; новых owner-границ и новых PLAN-ID это не создаёт. Следующее точное действие — отдельный owner decision для M1-C / VA-NEW-04+05; полная сводка маршрута — блок «Mini plan reconciliation 2026-08-11» в [PROJECT_EXECUTION_PLAN.md](PROJECT_EXECUTION_PLAN.md).
