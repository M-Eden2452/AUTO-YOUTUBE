...
- **сделано 2026-08-18: слайс C100 — ветка перестала быть красной.** Причина
  слайса ровно одна: этой же веткой измеряется качество отбора, и пока в ней есть
  чужая краснота, любой следующий прогон читается через неё. Оказалось две
  красноты, обе на чистом HEAD и обе не от `C98`. **Первая (`C100`):**
  `test_plan9d_retrieval_gate` держал `EXPECTED_CORPUS_SHA256 = bfb4d02…`, а корпус
  v1 подписан `280e6da…` с `20f02cd`. Пин переведён на текущее значение вместе с
  причиной дрейфа, записанной рядом с константой, и причина проверена по diff'у, а
  не по телу коммита: `20f02cd` изменил в корпусе ровно три строки —
  `plan9d_current_capture_v1/scene_010` (выбранный кандидат `C33` /
  `pexels_video_31767897`) потеряла категорию `ambiguous_needs_review`, агрегат
  `technical_categories.ambiguous_needs_review` ушёл 2 → 1, файл переподписан. Ни
  одно из остальных чисел, которые пинует тот же тест (14 сцен, 1064 наблюдения,
  56 превью, 64 кадра, 745 / 319 прав), в том коммите не двигалось, дайджест
  идентичности разметки остался `04b33c9…` — то есть дрейф производный, а не
  потеря захвата. Возврат корпуса к запинованному состоянию отвергнут: он откатил
  бы вторую половину `C95`. **Вторая (`C102`), найденная только полным прогоном:**
  страж реестра `test_registry_rows_stay_a_trail_and_not_a_journal` называл три
  строки выше потолка 1500 — `C95` (1957), `C96` (1682) и `C98` (3763), все три уже
  на HEAD. Строки укорочены тем способом, который тест и требует: находка, вердикт
  и указатель остаются, рассказ уходит в тело коммита (`d49faae`, `91fc97c`,
  `7b6a169`); `REGISTRY_ROW_LENGTH_BASELINE` не пополнялся — он для шести строк,
  написанных до правила. **Приёмка:** полный offline suite **2407 тестов**, до
  правок реестра — 1 падение (тот самый страж), после — зелёный целиком (863 с и
  992 с двух прогонов); targeted четырёх наборов plan9d — 227 тестов OK (409 с);
  `GATES OK`. **Найдено попутно и не чинилось:** `scripts/gates.py` тестов не
  запускает намеренно, поэтому обе красноты дожили до полного прогона; порядок
  «gates + targeted» этого класса дефектов не ловит, и цена вопроса — те самые
  ~16 минут.
- **решение владельца 2026-08-18: gates получают repo-state guard-тесты.** Принято
  сразу после C100/C102 и названо тем, что предотвращено по факту, а не гипотезой:
  краснота реестра — тест на 1.4 с, который пережил и gates, и targeted-наборы
  нескольких слайсов подряд, потому что gates не запускали ни одного теста, а
  targeted слайса не имел повода его трогать. `scripts/gates.py` получил гейт
  `repo-state-guards` — пять существующих наборов, которые читают сам репозиторий
  (`test_stage2_agent_onboarding`, `test_docs_routing_and_freshness`,
  `test_capability_consistency`, `test_asset_import_boundaries`,
  `test_apps_structure`): **56 тестов, 10 с**. Полный suite в gates по-прежнему не
  входит и остаётся в CI; `test_claude_permission_contract` намеренно снаружи —
  вопрос у него того же класса, но 20 с одним набором вдвое дороже всего списка, а
  гейт, которого ждут, обходят. Новый механизм, реестр и вторая команда не
  заводились: это шестая строка в существующем `GATES`. Проверено обратным ходом —
  в реестр подсажена строка длиннее потолка, `GATES FAIL repo-state-guards`, строка
  убрана, `GATES OK`. Подпроцессам гейтов задан UTF-8: жалоба стража цитирует
  кириллические строки реестра, и на cp1252 она превратилась бы в ошибку кодировки
  вместо самой жалобы.
...
- **сделано 2026-08-18: слайс C99 — офлайн-признак крупности плана.** После
  разметки владелец выбрал вариант 1 из двух: детерминированный офлайн-сигнал
  внутри существующего `candidate_ranker`, привязанный к уже существующему
  `visual_brief.shot_type` (establishing/action/detail/evidence/payoff/context —
  та же дуальная схема, что и у остальных полей `analyze_scene`), а не Vision.
  Условие владельца выполнено буквально: крупный план не считается лучше общего
  по умолчанию — маппинг `establishing`/`context` → wide и `detail` → close взят
  из определения самого словаря ролей («что кадр должен сделать драматургически»),
  а не подобран под три измеренные сцены; смещение — бонус/штраф `±8` к
  `final_score` **вне** `meaning_score` (0.85 веса), поэтому смысл невозможно
  перевесить этим сигналом — залочено тестом `test_meaning_still_outranks_shot_scale`.
  **`payoff`→close испытан и отклонён по числу, а не по мнению:** на замороженном
  v2 предсказал верно `local_after_fix/scene_005` и неверно `live_5/scene_005` — та
  же дуга «payoff», два разных решения владельца о крупности одного и того же
  предмета. Один из двух — это «неразличимо», а не правило, и в итоговый код не
  вошёл; тест `test_payoff_action_and_evidence_carry_no_scale_preference` не даёт
  ему тихо вернуться. **Приёмка (`measure --baseline`):** v1 — **4/14 → 6/14**
  (`scene_006`, `scene_010`), `changed_winners 2`, регрессий нет; v2 agreement не
  сдвинулся (2/10) — `live_5/scene_001` перестал брать крупный план (`C5`), но
  берёт другой общий план (`C7`), не ровно предпочтённый владельцем `C1` —
  метрика по конкретному кандидату это не засчитывает, залочено тестом с
  комментарием, что это остаточный пробел, а не победа. `local_after_fix/scene_002`
  (`action`, признак не размечен) остаётся неразличимой тройной связкой — этот
  слайс её не адресует. Frozen-приёмка PLAN-9D-E/9D-H (`tests/test_plan9d_ground_
  truth_baseline.py`) обновлена вместе с числами, а не отдельно от них.
  **required verification:** `tests.test_shot_scale_signal` (12 OK, новый файл) +
  `tests.test_plan9d_ground_truth_baseline` (102 OK) + widened targeted 487 OK +
  `GATES OK`. Сеть, платные вызовы, Vision не использовались.
- **остаток для владельца:** если после этого слайса остающиеся ошибки нельзя
  надёжно различить metadata/framing/crop-сигналами (как `local_after_fix/
  scene_002`, где все кандидаты — один и тот же общий план), следующий шаг —
  `PD-2`/Vision как арбитр top-кандидатов, отдельным owner-approved действием.
- **сделано 2026-08-18: независимое ревью C99 нашло MAJOR, исправлено тем же
  днём (C103).** Ревью коммита `cbec0f4` не поверило магнитуде `±8` на слово:
  воспроизвело сценой с составным предметом из трёх слов реальный переворот —
  кандидат с двумя словами из трёх и верной крупностью обгонял кандидата со
  всеми тремя и неверной, разрыв `final_score` без сигнала ≈15 очков против
  ±8's 16-очкового качания. Условие владельца «смысл всегда приоритетнее»
  не выполнялось при частичном совпадении предмета, только при полном
  разрыве — ровно то, что залоченный тест не проверял. Исправлено структурно,
  а не смягчением формулировки: `SHOT_SCALE_TIE_EPSILON = 12.0` внутри
  `rank_candidates` — кандидат, отстающий от лучшего по смыслу `final_score`
  сцены больше чем на эпсилон, сигнала не получает вообще, независимо от
  текста. Порог взят из измерения, не из контрпримера ревью: три уже
  исправленные сцены нуждались в 9.4 / 3.3 / 1.3 очка запаса, порог 12.0
  оставляет им место и остаётся ниже опасного разрыва ревью (≈15) с запасом.
  Приёмка не изменилась (v1 4/14→6/14, v2 2/10, оба без сдвига) — правка
  только сужает применимость сигнала, не меняет ни одного уже принятого
  решения. Новый тест `test_a_partial_subject_match_cannot_be_lifted_past_a_
  full_match` фиксирует ровно случай ревью, плюс 5 тестов на границу эпсилон.
  `tests.test_shot_scale_signal` — 18 OK; `GATES OK`.
