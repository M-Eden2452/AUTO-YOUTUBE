...
next_exact_action: PLAN-9C-2 ПЕРВЫЙ SUB-SLICE ВЫПОЛНЕН 2026-08-10 (unified media-selection policy foundation, этот implementation commit, trailer Plan-Step PLAN-9C-2). Канонический owner решения о виде медиа — src/assets/semantic_selection/media_policy.py (select_with_media_policy) — ранжирует по-прежнему select_best_candidate, политика поверх применяет hard whitelist существующего allowed_media_kinds сцены (VIDEO_ONLY/IMAGE_ONLY без допустимого кандидата → честный abstain, не скрытая подмена) и bounded video-предпочтение только среди конкурентных кандидатов (тот же SUPPORT_RANK-класс, что у лучшего; права/review не хуже; mode-independent blocking_reasons молчит; только внутри review-окна shortlist_size). Безусловная подмена «первое не-отклонённое видео любого ранга» удалена — select_best_with_video и facade-дубль asset_manager._select_best_candidate стали thin-делегациями к политике (имена сохранены для существующих callers/patch-points), закрепляющий тест дубля перенацелен, post-Vision reselection идёт через ту же политику с тем же окном. Числовой score-gap не вводился, новых persisted/config полей нет — trace пишет существующее поле selected_by (media_policy_video_preference / _fallback). RED-сначала на HEAD bbe5147 — CASE 1, CASE 5 и оба LIVE-4 кейса падали (partial-видео поверх full-кадра; видео вне превью-окна); после реализации tests/test_media_selection_policy.py 21 OK, owning radius (news-to-short assets+contract 30, semantic selection 84, ladder 51, PLAN-9D harness 167, provider/preview/slot 67, retrieval regression+query truth 18, manual+rights 89) OK, полный offline suite 2117 OK. Retrieval symmetry в этот commit НЕ входит. Ladder draft-ветка prefer_video сознательно не тронута (унификация — её retirement row, audit §31); download walk — PLAN-9A; shortlist/evidence-set identity — PLAN-10C; metadata evidence — PLAN-9C-3. THE NEXT EXACT ACTION — отдельный owner-issued ВТОРОЙ bounded sub-slice PLAN-9C-2 (retrieval symmetry — search_provider, src/news/asset_provider_adapters.py:98, preferred=image не запрашивает видео вовсе) в новом чате строго по контракту секции PLAN-9C-2; затем PLAN-9C-3 (metadata evidence repair); acceptance gate обоих прежний — повтор real Russian diagnostic против LIVE-4 baseline (рабочий ярлык LIVE-5) отдельным owner-issued действием. current_checkpoint остаётся PLAN-9D; PLAN-9D-D остаётся NOT STARTED / blocked. Earlier context - PLAN RECONCILIATION 2026-08-10 (docs-only slice). LIVE-3 и LIVE-4 выполнены отдельными owner-issued действиями (projects/2026-08-09_diagnostic-ru-semantic-live-3, projects/2026-08-09_diagnostic-ru-semantic-live-4), role-contract repair закоммичен (19d2c94), merged comparative audit принят как evidence — docs/audits/VISUAL_ASSETS_COMPARATIVE_AUDIT_2026-08-10.md. Подтверждённый bottleneck — visual media-selection correctness (select_best_with_video, src/news/asset_manifest_builder.py:1239, подменяет выбор select_best_candidate первым не-отклонённым видео ЛЮБОГО ранга — 9D-C 7 из 12 сцен, LIVE-4 — scene_003 видео 65.75 вместо кадра 80.72 ранга 1 и scene_004 zoo-видео 70.0 вместо snow-кадров 80.0; продуктового мандата у безусловного prefer_video нет) и metadata evidence (каталожная проза Internet Archive + bag-of-words concept_score — LIVE-4 scene_001 lava subject_match=100 для unusual animals, scene_003 Life On Earth hummingbird=100). THE NEXT EXACT ACTION — отдельный owner-issued implementation slice PLAN-9C-2 (unified media-selection policy foundation, рабочий ярлык R-2a) в новом чате строго по его контракту (секция ниже); затем PLAN-9C-3 (metadata evidence repair, рабочий ярлык R-2b); acceptance gate обоих — повтор того же real Russian diagnostic input против LIVE-4 baseline (рабочий ярлык LIVE-5, PLAN-ID не является) отдельным owner-issued действием со своими network/paid разрешениями. Существующие owners сохраняются без дублей — shortlist/dedup/evidence-set identity за PLAN-10C (включая записанный F2), best-so-far/replacement persistence за PLAN-9A, LocalLibrary convergence и diversity reserve за PLAN-10D, Vision activation за PLAN-9E; Motion (MOTION-CS1..CS4, Remotion/HyperFrames) в critical path не входит. current_checkpoint остаётся PLAN-9D; PLAN-9D-D остаётся NOT STARTED / blocked (его два owner decision из секции PLAN-9D-C) и идёт после acceptance gate. Earlier context - Two live runs (live-1, live-2) were done and both findings closed. The second finding, from the read-only diagnostic projects/2026-08-09_diagnostic-ru-semantic-live-2 - 5 of 6 scenes were classified exact_location and routed to NASA/Wikimedia because _named_place in src/assets/scene_strategy.py tested only that visual_brief.place was non-empty, so open ocean, nature outdoors, snowy icy ground, outdoor natural setting and indoor glass wall all counted as a named place - and every one of those decisions also recorded classified_from=glossary while _LOCATION_TERMS matched 0 times. A bounded repair commit on 2026-08-09, carrying the same Plan-Step trailer PLAN-9B-PRODUCER-M-LIVE and no new PLAN-ID, rests the exact_location branch on the existing _LOCATION_TERMS glossary alone, removes _named_place (its semantic.location half is the same scene.place mirrored by legacy_format.semantic_block, not a second source), and makes the recorded reason and classified_from name the rule that actually fired. PROVIDER_PRIORITY, REQUIRED_SLOT_KINDS, requires_provider_metadata, candidate_ranker and all scoring, rights and Vision (OFF) are UNCHANGED - generic scenes reach the existing generic stock route because the class was fixed, not the table. Residual observation recorded and deliberately NOT fixed - _LOCATION_TERMS still contains generic landform words (landscape, terrain, ландшафт, местност). Full offline suite 2083 OK. The first live run's finding is closed too - gpt-4.1 understood every scene, but 3 of 6 semantically usable answers were refused whole by parser limits the model-facing prompt and schema never stated - a too-long context item (scene_001), a too-long action (scene_004), and the on-screen text it had been shown copied back into context in the scene language (scene_006). A bounded repair commit on 2026-08-09, carrying the same Plan-Step trailer PLAN-9B-PRODUCER-M-LIVE and no new PLAN-ID, aligned the asking side only - build_prompt now states the provider language, the per-field word ceiling, the context item ceiling, that values are short search phrases, and that quoting narration, copying on-screen text or explaining anything voids the whole answer; RESPONSE_CONTRACT is now written from MAX_FIELD_TERMS/MAX_CONTEXT_ITEMS instead of beside them, and response_schema() carries those same descriptions. The parser was NOT weakened - MAX_FIELD_TERMS=8, MAX_CONTEXT_ITEMS=4, provider-language validation and all-or-nothing parsing are unchanged, and the three refused live answers stay refused under test. Selection is UNCHANGED (required slots, candidate_ranker, action/context blocking, fallback_level, metadata thresholds, rights); action was deliberately NOT downgraded to advisory, because metadata-only inability to prove an action - orca swimming offered instead of orca breaching, gecko standing instead of gecko running - must be measured separately rather than hidden by loosening the selector. Vision stays OFF and 0 real model, network, provider, paid, TTS or render calls were made by the repair. False-fallback repair is deliberately NOT attempted - with the brief refused, scene_004 degraded to the query snow and picked a winter forest with no penguins, and the re-run must show whether that failure disappears on its own once the brief is accepted. No independent review was scheduled for the repair - it changes no network, paid gate, selection authority, persisted schema or rights. PLAN-9D-D stays NOT STARTED / blocked either way. INDEPENDENT READ-ONLY REVIEW of the PLAN-9B-PRODUCER-M-LIVE activation commit (live semantic-model activation), closed 2026-08-09. It touches network permission, paid behaviour and secret handling, so exactly one independent review comes before anything else; with no BLOCKING finding the next owner-issued action after that is the real diagnostic Short, and PLAN-9D-D stays NOT STARTED / blocked either way. The live backend IS implemented and IS reachable from production, and its default activation state is OFF - config/semantic_brief.json ships every paid condition disabled, and the network class semantic_brief is denied by default like every other. No real model call, network call, provider call, Vision call, TTS call, paid call or render was executed by that slice. PLAN-9B-PRODUCER-M (model-assisted semantic VisualBrief adapter) is CLOSED 2026-08-09 by an owner-issued implementation slice of its own. current_checkpoint stays PLAN-9D because that step is still open, and this is the capability prerequisite that unblocks it, not a new checkpoint and not a PLAN-9D evidence sub-slice. The diagnostic Short after PLAN-9D-C was blocked because an ordinary prepared Russian script carries no provider-language evidence at all - deterministic extraction ranked grammatical objects as subjects and produce_brief correctly returned nothing, so most scenes reached the query adapter as query_translation_required. Root cause was the missing semantic capability, not broken producer wiring, and the deterministic fail-closed contract stays valid. The adapter lives inside src/content/visual_planning, fills the existing VisualBrief, and every query string is still built by the existing expansion ladder; no second planner, no second query owner, no TranslatorService, no schema change. It is injection-only and no production caller passes one, so nothing is activated by that slice. A real model call additionally needs a network action class for text models, which src.runtime_network.NETWORK_ACTIONS did not have, and a separate paid approval if the model costs money - two different owner decisions, neither taken there; both were taken and implemented afterwards by PLAN-9B-PRODUCER-M-LIVE, and both still default to denied. No real model, network, provider, Vision, TTS, paid call or render was executed by this slice. C63 remains open and untouched, so author-brief parity is not claimed for topic/article paths. That implementation commit was independently reviewed, verdict ACCEPT WITH NON-BLOCKING FINDINGS, and its architecture accepted; three MAJOR findings were found before any model was wired and closed by a bounded repair slice under the same PLAN-ID on 2026-08-09 - semantic assistance is now offered when the deterministic brief does not name a provider-language subject rather than only when it is empty, the semantic overlay can no longer drop a must_include or must_avoid the model was never offered, and a defect in the injected callable is no longer disguised as an unavailable model. That repair commit was independently reviewed in turn, and the two obstacles it left in front of live activation were closed by a second bounded repair under the same PLAN-ID on 2026-08-09 - a provider-language subject no longer suppresses semantic assistance merely by being provider-language, because an ordinary English sentence hands deterministic extraction the place or the preposition (reproduced offline as antarctic, breaches, across) and those scenes were refusing the only help that could correct them; sufficiency is now decided by the planner's own existing entity provenance, the topic stated for the video, and not by any lexical quality score, classifier or word list; and a backend that really ran and failed permanently now leaves the same visible semantic_brief_unavailable warning as one that may recover, because retryable answers whether asking again would help and was being read as whether anything was asked at all. Третий bounded repair того же PLAN-ID выполнен 2026-08-09 и закрыл последний repository-confirmed blocker перед дизайном live activation. Первое — `deterministic._plan_scene` больше не пишет `must_include`. Эвристически извлечённый subject/place получал авторитет автора только потому, что слово написано латиницей, тогда как `candidate_ranker` трактует `must_include` как hard requirement и отказывает любому кандидату, чьи provider metadata не содержат термин целиком. Из-за этого semantic correction не доводила работу до конца — модель исправляет предмет сцены, но прав на constraints не имеет, поэтому устаревшая догадка переживала исправление и отклоняла именно того кандидата, ради которого исправление делалось. Offline-репродукция на русской сцене про пингвинов со словом `NASA` дала `subject_match=100.0` при `blocking_reject_reasons=['must_include_missing:NASA']`. Явный авторский `must_include` остаётся hard и blocking — изменился источник авторитета, а не его сила, и извлечённые subject/place по-прежнему едут в своих слотах и в intents, где оцениваются частичным совпадением вместо буквального требования. Второе — отозван topic-provenance bypass, потому что слово внутри глобальной строки `topic` больше не считается scene-level основанием пропустить semantic model. `Antarctic penguin colonies` помечает `antarctic` и `penguin` одинаково и не может сказать, о чём именно эта сцена. Единственный scene-level источник с реальным авторитетом — авторский `visual_brief`, и он по-прежнему пропускает модель; другого структурированного источника в репозитории нет, и он не выдумывался. Ни quality classifier, ни lexical blacklist, ни confidence-поле, ни новая persisted provenance-схема не вводились. `ENTITY_KIND_TOPIC`, `VisualPlanResult.topic_entity` и `topic_stems` сохраняют прежний смысл — отозвано только их использование как bypass evidence. Стоимостное следствие принято сознательно, верхняя граница при будущей активации — один вызов модели на сцену без авторского брифа. Semantic model по-прежнему не получает права записывать `must_include`/`must_avoid`, attempted/backend observability не регрессировала. Третий repair commit прошёл independent read-only review, verdict ACCEPT WITH NON-BLOCKING FINDINGS, и на этом линия PLAN-9B-PRODUCER-M закрыта. Оба owner decision, которых не хватало, приняты и исполнены слайсом **PLAN-9B-PRODUCER-M-LIVE** 2026-08-09 — `src.runtime_network.NETWORK_ACTIONS` получил ровно один класс `semantic_brief`, платное разрешение осталось отдельным gate в `config/semantic_brief.json`, и `src/news/visual_plan.py` теперь действительно конструирует адаптер — но только когда оба разрешения выданы. По умолчанию не выдано ни одно, поэтому обычный прогон остаётся детерминированным. Подтверждённый MAJOR-A того же review закрыт там же — частичный авторский бриф из одних constraints больше не считается ответом на вопрос «что показывать». Diagnostic Short по-прежнему **не запускался**, и следующее точное действие — independent read-only review именно activation commit. PLAN-9D-D is NOT STARTED and stays blocked. PLAN-9D-C (retrieval quality gate) is CLOSED 2026-08-09 by this implementation slice (trailer Plan-Step PLAN-9D). It is an offline gate over the frozen current corpus - no provider call, no download, no Vision, no paid call of any kind, and the gate itself is exercised under the repository socket guard. The corpus was re-verified by this slice rather than taken from the PLAN-9D-B report - validate_corpus, validate_current_capture and assert_current_benchmark_input all pass, the recomputed corpus_digest equals the recorded corpus_sha256 da8e50a968afc72fcc427ffeb9b0e58fe264119f9d191d17849ce2265fa89b35, and all 64 frames are present on disk with matching SHA256. The gate owner is tests/plan9d_retrieval_gate.py with tests/test_plan9d_retrieval_gate.py as its lock; no second selector, score or ranking layer was created, and src, config, schemas, projects/ and the corpus itself are unchanged. RAW RETRIEVAL verdict PASS - none of the 220 provider strings actually sent carries non-provider script (CRITICAL-1 does not recur) and none is a retired broad literal (C35/C36 do not recur); every one of the 14 scenes sent at least one query carrying its declared subject verbatim, and every pool holds at least one candidate whose own provider metadata carries that subject - 396 of 1064 by phrase and 697 of 1064 by token, reported separately and never blended. The query we sent is excluded from the matched text on purpose, because including it would let every candidate confirm its own query. 35 of the 220 attempts carry no subject at all (the rungs forest, bay, Antarctica, open ocean, low Earth orbit, savanna, laboratory, river waterfall salmon, tundra white winter coat) and returned 301 results, but not one candidate first returned by such a rung reached the previewed shortlist - ranking removes them, so this is query cost, not decision pollution. PREVIEW SHORTLIST verdict PASS with one structural defect - all 56 previewed candidates are licensed and allowed_for_render and all 56 carry the subject in their own metadata, but _prepare_visual_review previews state.candidates[:5] while that list still contains repeated observations, so a repeat inside the window consumes a preview slot; 14 of 70 slots across 9 scenes have no preview at all, leaving only 2 visible candidates in scene_004 (orca) and scene_007 (pangolin) and 3 in scene_014. SELECTED-CANDIDATE verdict WEAK, and the weakness is not retrieval - 12 of 14 scenes have a selection and the two that do not (cheetah_not_leopard, arctic_fox_winter_coat) correctly returned no acceptable candidate instead of forcing one; in 7 of those 12 the selected asset is exactly the first non-rejected video in the ranked list, because select_best_with_video (src/news/asset_manifest_builder.py:1239) replaces select_best_candidate's choice with the first video at any rank - which sent the selection outside the previewed window in 3 scenes (gecko rank 14, solar_farm rank 6, laboratory_pipetting rank 5, none of them previewed), picked a perched hummingbird over two hovering ones at ranks 0-1, picked the LIFE ON EARTH title card over a real pangolin at rank 1, and picked the conspiracy video "Nuke Van Allen Belt Nazi Freemasons Man On Moon" over a real Saturn V on the pad at rank 2. The agent's own visual read of all 56 previewed frames is recorded in the PLAN-9D-C section as evidence and is explicitly not owner ground truth and not a substitute for PLAN-9D-D - it found three clearly good outcomes (tiger walking through forest, brown bear with a salmon at the falls including its must_include, real ISS footage rather than a rendering) and three clearly bad ones (the two above plus suspension_bridge, where a cantilever truss bridge was selected over a real suspension bridge at rank 1), plus a visible photographer watermark on the selected assets of scene_004 and scene_014 and two explicitly labelled "Artist Rendering" candidates surviving into the scene_011 shortlist even though conflicting_context is declared - the declared-conflict gate held at selection but not at shortlist. No must_avoid violation exists in any selected candidate, checked both by metadata and by eye. The honest boundary is stated rather than blurred - every visual claim covers only the 56 previewed candidates and nothing is claimed about the remaining 1008, so it is explicitly NOT claimed that no better asset existed in the pool; for cheetah 32 pool candidates carry the subject in metadata and for arctic fox 14, and whether any of them shows a running cheetah or a white winter coat is a question the frozen data cannot answer. The PLAN-9D-C stop condition did NOT trigger - current retrieval does produce a reasonable candidate pool, so PLAN-9D-D through PLAN-9D-G are not blocked by this step - and nothing was repaired here - PLAN-10B pagination, PLAN-10C shortlist/budget (including the already recorded F2 follow-up), Vision activation and any revision of select_best_with_video stay with their own steps and are not pulled forward. Two owner decisions are handed to PLAN-9D-D and were deliberately not taken here - whether a blind annotation is acceptable when the annotator sees 2 to 5 candidates instead of 5, and what PLAN-9D-E should do for the 3 scenes whose selected candidate was never previewed, which makes a decision-versus-annotation comparison undefined for them. Verification actually run - tests.test_plan9d_retrieval_gate plus tests.test_plan9d_current_capture, tests.test_plan9d_ground_truth_baseline and tests.test_plan9d_historical_evidence, 167 OK; the full offline suite was not run and is not required (Execution protocol item 10) because no production contract, test discovery or runner changed. current_checkpoint stays PLAN-9D; PLAN-9D as a whole remains in progress and this closure asserts nothing about decision quality - it asserts the opposite for several scenes. THE NEXT EXACT ACTION is a separate owner-issued implementation slice for PLAN-9D-D (human ground truth), in a fresh chat, strictly under its own contract, after the owner has taken the two decisions above; the diagnostic vertical Short remains a separate owner-issued run with its own network, paid-call and render approvals and is not started here. Earlier context - PLAN-9D-B (current-HEAD retrieval capture) is CLOSED 2026-08-08 by implementation commit 69af3ca7387fa9fe649fabf0fd464ec519f76400 (trailer Plan-Step PLAN-9D). One bounded run of the current production retrieval path, driven through the existing production owners with no query written by hand and no src/config/schemas change, is frozen as tests/data/plan9d/current_corpus_v1.json - 14 scenes, 1064 observations, 1052 unique assets, 64 frames, corpus_sha256 da8e50a968afc72fcc427ffeb9b0e58fe264119f9d191d17849ce2265fa89b35 (value read from the corpus file itself and independently re-verified by this docs-only closure, not taken from the implementation report). capture_head_sha d01914d77822057569a491216cfecf21b08f5d0c is confirmed by this closure to mean the production HEAD whose behaviour was captured, not a separate harness revision - git diff --stat d01914d 69af3ca -- src/ config/ schemas/ is empty, so production behaviour did not move between the two commits. Two capture attempts occurred before the commit, and this closure independently re-verified their relationship rather than accepting the implementation report at face value. The first attempt reached all 14 scenes and was stopped by its own freeze-time secret scan, a false positive on the word "authorization" in a provider description and the approval note; no corpus was written and nothing was selected from that attempt, and no artifact of it survives anywhere in the untracked capture workspace or in Git. The second attempt is the only one that persisted anything - the raw capture file (projects/plan9d_current_capture_v1/capture_raw.json, untracked) carries capture_timestamp_utc 2026-08-08T15:57:57+00:00 and scene/observation counts (14/1064) identical to the frozen corpus, so the corpus is derived from exactly that one raw capture and no cross-run metadata mixing is possible. Preview-cache integrity was independently checked over all 56 previewed candidates - 56 distinct cache_key values, zero collisions or sharing across different asset_id values, all 56 preview file SHA256 digests match their declared values, and every preview_record.json is present - a stale or wrong-asset preview is structurally impossible because compute_preview_cache_key (src/assets/visual_preview.py) hashes the candidate's own identity fields (provider, provider_asset_id, asset_id, preview_source_url, media_type, rendition, target_aspect_ratio, sample_count, top_k), not a run sequence number. Capture-integrity verdict - VALID_CAPTURE - the retry is documented non-quality recovery from an infrastructure false positive, not a quality-motivated re-run - the capture module's own contract forbids a second run "to get a better pool" (tests/plan9d_current_capture.py), and no on-disk evidence contradicts that for this capture. Baseline CI for the exact implementation SHA was independently checked by this closure - GitHub Actions check-run "unittest" on 69af3ca7387fa9fe649fabf0fd464ec519f76400 is completed/success. Targeted tests re-run by this closure - tests.test_plan9d_current_capture plus tests.test_plan9d_ground_truth_baseline, 100 OK; full offline suite and any provider/network test were not re-run (Execution protocol item 10, docs-only closure). Coverage semantics for PLAN-9D-C are stated here to remove an ambiguity in the prior wording, not to change scope - the corpus carries three distinct levels - raw provider retrieval (1064 candidate observations, metadata only), the ranked/rights-filtered candidate pool (the same 1064 after rank_provider_results/apply_policy_to_candidate, 745 licensed / 319 review_required), and the visually previewed shortlist (56 of 1064, the production shortlist_size=5-per-scene that the decision owner actually sees). PLAN-9D-C may judge query integrity and raw retrieval evidence across all 1064 observations by metadata, and shortlist quality visually only over the 56 previewed candidates; it must not draw a visual-quality conclusion about the remaining 1008 candidates, which were never rendered to a preview. No quality claim is made by this closure - not that free providers are adequate, not that Envato is unnecessary, not that retrieval succeeded, not that candidate selection improved; only that subjectful current queries were built, the provider capture executed, and a frozen current corpus now exists for PLAN-9D-C to evaluate. No production file changed by this docs-only closure - src, config, schemas, tests and the corpus itself are exactly as commit 69af3ca left them. THE NEXT EXACT ACTION is a separate owner-issued implementation slice for PLAN-9D-C (retrieval quality gate), in a fresh chat, strictly under its own contract - an offline gate over the frozen current corpus with no provider network call, evaluating query integrity and raw retrieval evidence across all candidates and shortlist quality visually only over the 56 previewed candidates, and is not started by this docs-only closure; that stated next action has since been issued and executed, see above. current_checkpoint stays PLAN-9D; PLAN-9D as a whole remains in progress and this closure asserts nothing about retrieval quality. Earlier context - PLAN-9D-A (historical evidence curation) is CLOSED 2026-08-08 by implementation commit 2bae6f6d23d8cbf874fcf71883334a7ea4d8619d (trailer Plan-Step PLAN-9D). corpus_v1.json (9777 lines, 16 scenes, benchmark-shaped data harvested from projects/) and its empty annotations_v1.json template are removed, not renamed - the exact content stays reachable at 04fe035e6ac07dbbe4a80257c3ed9d971976457e and the fixture records that anchor. The evidence is now tests/data/plan9d/historical_failure_evidence_v1.json - 7 cases, 31 candidates, one representative frame per candidate, kept explicitly as historical failure and compatibility evidence, never as a current-quality benchmark - gecko, hummingbird and penguin (no visual brief, the retired subject-free literal registry C36), CRITICAL-1 (a Russian query sent verbatim to latin stock indexes), a usable primary query followed by every provider asked the single word close, and the retired one-topic orca hardcode (registry C35). Nine of the sixteen source scenes are dropped, each recorded in dropped_source_scenes with its reason. Historical and current are separated by contract, not naming - payloads declare generation_class and fixture_kind, assert_current_benchmark_input is the single gate, and an unstamped payload is read as historical, never assumed current, so restoring the old corpus from Git does not reopen the gate. historical_runtime_paths() over the committed fixture was independently re-run for this closure and returns exactly 45 paths - 14 manifests and 31 frames, 33.47 MB across 7 projects, all present on disk - down from the prior 107 frame paths across the whole untracked projects/ tree; nothing under projects/ was deleted, and this reduction is not a cleanup authorization - historical runtime cleanup stays owned by PLAN-14 and registry rows N02/C32/D04/C20 and needs its own gate. No production file changed - src, config, schemas, projects, assets and outputs are untouched, and no network, provider search, download, Vision, model, TTS or paid call was made. Verification independently re-run for this closure - targeted tests.test_plan9d_historical_evidence plus tests.test_plan9d_ground_truth_baseline, 80 OK. Full offline suite 1875 OK, check_agent_docs exit 0, check_task_scope OK and git diff --check clean are the implementation commit's own claims, owner-provided, not re-run by this docs-only closure under Execution protocol item 10. current_checkpoint stays PLAN-9D (offline visual-quality evidence) - PLAN-9D-A is one of seven bounded sub-slices, its closure does not close PLAN-9D, and it asserts nothing about decision quality. THE NEXT EXACT ACTION is a separate owner-issued implementation slice for PLAN-9D-B (current-HEAD retrieval capture), in a fresh chat, strictly under its own contract - it needs its own explicit per-action owner network approval for the provider_search and preview_download classes of src/runtime_network.py, and is not started by this docs-only closure. Earlier context - PLAN-9D (offline visual-quality evidence) stayed the current checkpoint and its status moved from pending to in progress, because commit 04fe035e6ac07dbbe4a80257c3ed9d971976457e already landed under the Plan-Step PLAN-9D trailer and delivered a benchmark harness plus a frozen corpus without closing the step. An owner-issued plan reconciliation on 2026-08-08, following a read-only architecture, data-hygiene and stock-pipeline audit, reformulated what PLAN-9D measures - decision quality must be measured on candidate pools that represent CURRENT retrieval behaviour, and historical projects keep their value as historical failure and compatibility evidence but stop being the primary current-quality benchmark. No production logic changed in that reconciliation - src, tests, config, schemas, projects, assets, outputs, legacy, apps and the workflows were untouched, nothing was deleted, moved or archived, and no network, provider, Vision, model, TTS or render call was made. The audit evidence behind the reformulation, all re-derived from the code and the filesystem at clean HEAD 04fe035 rather than from documentation, is this. No runtime project on disk is a product of the current query stack - the newest file anywhere under projects/ is dated 2026-07-28, while PLAN-9B-1 landed 2026-08-01 in 141beae and PLAN-9B-2, PLAN-9B-3 and PLAN-9C landed 2026-08-07 and 2026-08-08, and the count of files under projects/ newer than 2026-08-01 is zero. The frozen corpus tests/data/plan9d/corpus_v1.json is built entirely from those projects, so none of its 16 scenes was retrieved by the current query stack - 6 of 16 scenes, which is 30 of 75 candidate-in-scene observations, were retrieved by the retired subject-free literal "nature science wildlife observation" that still sits in the _LEGACY_BROAD_QUERIES compatibility guard in src/assets/query_adapter.py and is recorded as registry C36; 2 more came from the retired orca topic hardcode recorded as C35; 4 more came from the degenerate pools "orca", "close", "scientists" and "ocean aerial"; and one carries the raw Russian query "пластик воздуху оставили" straight to a provider, which is the CRITICAL-1 failure that PLAN-9B-1 repaired. The sharpest single piece of evidence is that the gecko, hummingbird and penguin scenes all declare an empty subject and share one identical five-candidate pool, in which no candidate depicts the declared subject at all - on that corpus the benchmark measures which of five wrong videos is least wrong, not decision quality. The owner ground truth does not exist yet either - annotations_v1.json is still status WAITING_FOR_OWNER_ANNOTATION with an empty annotator and every per-candidate field blank - so nothing has been frozen against the polluted corpus and no measurement is invalidated by the reformulation. The stock architecture was re-verified as single-owner in all three roles that matter here - the only provider-facing query owner is build_scene_queries and build_slot_queries in src/assets/query_adapter.py, seeded upstream by the PLAN-9B-2 expansion ladder in src/content/visual_planning/expansion.py, which is one chain with two roles and not two competing owners; the only candidate decision owner is rank_candidates and select_best_candidate in src/assets/semantic_selection/candidate_ranker.py, with the copy in src/news/asset_manager.py still the documented compatibility facade outside the production path; and the only rights owner is apply_policy_to_candidate in src/assets/license_policy.py. PLAN-10A, PLAN-10B, PLAN-10C, PLAN-10D and PLAN-9E are confirmed not started - each carries a blocked status with an empty commit field and Git contains no commit with a Plan-Step trailer for any of them. Historical runtime is confirmed to be needed almost entirely as evidence rather than as a production source - projects/ holds 7.2 GB, is fully untracked, and is referenced from no file under src, tests or config, its only live dependency being the 107 frame paths inside the frozen corpus. PLAN-9D is therefore split into the seven bounded sub-slices PLAN-9D-A through PLAN-9D-G under Execution protocol item 5, each needing its own owner-issued implementation slice. PLAN-9D-A curates the historical evidence - the harness delivered by 04fe035 is kept and reused, while corpus_v1.json is demoted from current benchmark to an explicitly named historical failure fixture showing retired subject-free retrieval, and compacted; this sub-slice also defines the identifier PLAN-9D-A that the corpus data files already declared while the plan did not. PLAN-9D-B captures a fresh candidate corpus in one bounded run of the current production path - visual planning, current query planning, current free providers, current candidate pools, previews and frames - which is DATA CAPTURE and EVALUATION PREPARATION and explicitly not an implementation of PLAN-10A, PLAN-10B, PLAN-10C or PLAN-10D, tunes nothing, and needs its own explicit per-action owner network approval for the provider_search and preview_download classes of the default-deny owner src/runtime_network.py. PLAN-9D-C is a retrieval quality gate that must pass before any Vision evaluation - simple subject scenes have to receive subject-relevant candidate pools, the evaluation set must include at least gecko, hummingbird, penguin and orca plus several harder environment, must_avoid and declared-context scenes, and if current retrieval cannot produce a reasonable candidate pool then PLAN-9D-D through PLAN-9D-G do not begin, because a Vision comparison must never mask a retrieval failure as a decision-quality result. PLAN-9D-D takes the blind owner annotation exactly once and then freezes and hashes it, with the agent never filling it in. PLAN-9D-E measures the current metadata-only decision against that frozen human ground truth through the existing decision owner with empty vision_tags. PLAN-9D-F saves real Vision evidence and stays a separate explicit owner-approved paid action with the PLAN-9E implementation-time model verification applied and fail-closed behaviour on an unknown model. PLAN-9D-G runs the offline A/B of metadata-only against metadata plus saved real Vision evidence through that one decision owner. The PLAN-9D principle is unchanged in substance and now stated explicitly - the final evaluation is offline, the bounded provider capture exists only to prepare the current candidate corpus, the corpus is frozen by its corpus_sha256 immediately afterwards, and PLAN-9D must not become a live mutable benchmark. Plan order is preserved exactly - the route PLAN-9D then PLAN-9A then PLAN-10A then PLAN-10B then PLAN-10C then PLAN-9E, recorded in this plan and in PRODUCT_PLAN.md section 11.7, is unchanged, PLAN-10A, PLAN-10B, PLAN-10C, PLAN-10D and PLAN-9E are not pulled forward, and the outcome of the PLAN-9D-C gate may justify a future owner decision about ordering but not before the current baseline evidence exists. Cleanup sequencing is fixed as a principle without performing any deletion here - historical runtime data must not remain an implicit current evaluation source, but the order is curate compact historical failure evidence, then capture the fresh current corpus, then freeze corpus and ground truth, then and only then allow historical runtime cleanup through PLAN-14 and registry rows N02, C32, D04 and C20, with legacy production code retired only through PLAN-L and its existing retirement gates; a single old_everything dumping directory inside the active repo is forbidden, and historical heavy runtime is either deleted, or archived outside the active runtime paths, or turned into a compact versioned fixture. One ordering conflict found by the audit is closed inside the Safety boundaries section rather than by a new identifier or a new Preserved runtime corpus row - that section classifies jpeg files and caches under projects/ as disposable while the frozen corpus addresses 107 such preview frames, so a proviso now protects exactly those frames while PLAN-9D is open and lifts itself at cleanup sequencing step four. Early hygiene was checked separately and needs no pull-forward micro-slice - the stale bytecode is 2573 __pycache__ directories and 18938 pyc files of which 2521 directories sit inside the vendored MOSS_TTS_Nano tree, all of them already covered by the .gitignore rules and invisible to Git status, so they stay with their existing PLAN-14 owner and registry row D04, and tmp/ is indeed not ignored but contains zero files with only an empty tmp/pdfs directory while output/ does not exist at all, so registry row C20 stays with its existing PLAN-14 owner too. PRODUCT_PLAN.md and CLEANUP_REGISTRY.md were deliberately not modified - the product direction did not change and the registry already names sufficient owners for every item touched here, the missing piece being ordering alone, which this file owns. That paragraph's own stated next action - a separate owner-issued implementation slice for PLAN-9D-A - has since been issued and executed, see above. Earlier context - PLAN-9C (semantic decision wiring) is CLOSED 2026-08-08 by three immutable implementation commits, all carrying the Plan-Step PLAN-9C trailer. Implementation commit 8932957ec2e1acb800d04bbbc7aa9d1e50212c9d wires the existing paid Vision/semantic evidence producer into the scene-selection loop itself - semantic_visual_service is now also asked about the bounded shortlist while the previews and sampled frames already exist inside that loop, not only afterward from _write_reviews as before, and what it reports is handed to the existing decision owner in the field that layer already reads - semantic_selection.evidence consults vision_tags, and candidate_ranker already refuses a candidate on must_avoid_match and vision_mismatch - so select_best_with_video is asked again with the evidence present and the choice stays with select_best_candidate; no second selector, Vision stack, scoring system, budget mechanism or manifest was created, and asset_manifest_builder gained no suitability criterion of its own. The selection pass and the post-selection review pass share one loop and build the candidate in the same request shape, so they share one cache key and a candidate is paid for once. The seam stays off unless semantic_visual.enabled and semantic_rerank_enabled are both set, and both stay false by default, so default behaviour and every existing enabled configuration are unchanged; activation remains PLAN-9E's own gate. A user's own asset and a generated figure are never re-ranked, and a failed, offline or low-confidence analysis contributes no evidence at all. This commit also closes the already-recorded reporting defect that _semantic_visual_summary hardcoded semantic_rerank_enabled to False regardless of the real config. Repair commit 668ff10a245082b81383ba3cb64a53c72d179fae closed a blocking independent-review finding - the shipped default MockSemanticVisualBackend built its answer out of the request it was handed, so it confirmed every requirement it was asked about, and those confirmations became vision_tags that could clear must_include_missing on a candidate whose real provider metadata never mentioned the term - a project that only flipped the two config flags on could turn publish_ready from false to true on fixture evidence with no real image analysis. _apply_semantic_visual_evidence now resolves the configured backend name once and declines to run at all when that name is a fixture backend, checked by name rather than by paid_backend, because the mock already reports paid_backend=False and a future free local model would report the same while looking at real frames. The post-selection review pass is untouched, so the mock keeps its reporting role and loses only its vote on selection; the wiring test's scripted stand-in was renamed to identify itself as "scripted" so it keeps proving the wiring without being mistaken for the guarded default. Test-lock commit 8c1186fd529c761cfbf180541c1159323facd95b closed a second review finding - the guard regression test's "on" branch was vacuously green, because it reused the wiring test's _WIRED fixture, whose backend name "scripted" resolves through create_semantic_visual_backend to ExternalSemanticVisualBackend, which returns configuration_error with no vision_tags whenever allow_paid_vision is unset, so the mock guard added by 668ff10 was never actually exercised by that branch. It now uses a selector that omits the backend key so the real factory resolves the shipped default and the test drives the actual MockSemanticVisualBackend; the owner mutation-tested this by temporarily emptying FIXTURE_SEMANTIC_BACKENDS at runtime outside the commit and confirmed the test fails without the 668ff10 guard, and passes unmodified on HEAD. No production file changed in this commit. A final independent review of 8c1186f returned verdict ACCEPT, blocking findings 0 - owner-provided review evidence, the same pattern as PLAN-STAB-1/2/3, and it leaves no separate review commit in Git. GitHub Actions run 31250693048 (headSha 8c1186f, workflow offline-tests) is recorded as conclusion success as owner-provided pre-closure evidence; this docs-only closure commit performs its own single read-only CI existence/status check against its own headSha after push, with no polling, no rerun and no workflow_dispatch. Two properties stay explicit for any reader of this record - default configuration is unchanged - semantic_visual.enabled and semantic_rerank_enabled both stay false out of the box, and turning semantic decision wiring on for any project remains PLAN-9E's own gate - and the shipped MockSemanticVisualBackend keeps only its pre-existing reporting/test role, never a production reselection authority; candidate_ranker (rank_candidates / select_best_candidate) remains the single decision owner and no second selector was created. The review's one confirmed non-blocking finding, F2, stays owned rather than lost - after a semantic demotion the bounded shortlist/review-window composition can drift, so the pre-rerank preview set is not guaranteed to equal the post-rerank review bundle set - a candidate the evidence demoted can drop out of the late review artifact, a candidate promoted only by that evidence can enter the review bundle without ever having gone through preview analysis, and either can mean an extra semantic/backend call with its own cost and review-evidence drift. The review rated F2 MAJOR but explicitly NON-BLOCKING for PLAN-9C, because PLAN-9C's acceptance criterion is the wiring and its order, while shortlist size and budget are named as PLAN-10C's own contract inside this very PLAN-9C section ("Размер shortlist и бюджет принадлежат PLAN-10C"), so F2 is recorded as a bounded follow-up note inside the existing PLAN-10C section below rather than repaired here or given a new PLAN-ID. The review's other findings, referenced only by label (R2-R5) in the owner-issued closure instruction, are recorded as non-blocking per that same final verdict; their content was not supplied to this closure slice and no review artifact exists anywhere in Git history to derive it from, so none beyond F2 is itemized, repaired or invented here. Nothing outside documentation changed by this closure slice itself - production code, tests, schemas, config and runtime are exactly as the three implementation commits above left them; no new owner, selector, service, abstraction, persisted field or PLAN-ID was created by this docs-only slice. current_checkpoint therefore advances to PLAN-9D (offline visual-quality evidence), whose two blocking prerequisites are both now down - the PLAN-9B family via its closed PLAN-9B-2 (66fd2431/8c60295) and PLAN-9B-3 (72221e1) sub-slices, and PLAN-9C via the three commits above - and which stays pending / not started; this docs-only closure slice does not begin its implementation. THE NEXT EXACT ACTION is a separate owner-issued implementation slice for PLAN-9D, in a fresh chat, strictly under its existing PLAN-9D contract - proving an improvement to the decision path on already-available data (the existing live-eval dataset, already-saved frames, already-saved results of the prior Vision run, manually labelled fixtures), with no new paid calls and with a mock never admissible as proof of the improvement. Earlier context - PLAN-1C′ (capability owner gate - asset/semantic) is CLOSED 2026-08-07 by a single docs-only commit, and C01-SEM is closed with it. The deliverable is the section "C01-SEM - ownership inventory asset/semantic (PLAN-1C′)" in docs/current/CLEANUP_REGISTRY.md - 22 ownership rows covering the whole declared scope (src/assets/semantic_selection/*, src/assets/semantic_visual*, src/assets/completion/*, src/news/asset_manifest_builder.py, src/news/asset_scene_completion.py, src/news/project_store.py, schemas/), each with its canonical owner, its actual production callers, its decision authority, its persisted contract, its owning tests and its duplicate/overlap verdict, all re-derived from the code at clean HEAD b0e99a7 rather than from file names. The three contract questions are answered by fact. Who decides a candidate is suitable - one owner only, rank_candidates / select_best_candidate in src/assets/semantic_selection/candidate_ranker.py, judging on provider metadata alone; src/news/asset_manifest_builder.py is the orchestration owner and adds only the video preference and the user-asset priority, having no suitability criterion of its own; the single other point that can change an already selected candidate is select_candidate_after_review inside _prepare_visual_review, reached only when technical_rerank_enabled is true (default false) and judging technical properties, not meaning. The four roles are distinct and are recorded as such - evidence producers (scene_analyzer, evidence, visual_preview, semantic_visual_service), decision owner (candidate_ranker, plus completion/ladder for the fallback rung and completion/modes for admissibility), orchestration owner (asset_manifest_builder), persistence owner (news/project_store together with news/pipeline). Where the shared service ends and workflow policy begins - the boundary runs along src/assets/* against src/news/* - the shared services know nothing about stages, job.json or provider ordering, while completion mode, provider order, download attempts, draft_complete and missing_scenes belong to src/news; the two reverse crossings (completion/replacement.py:250 importing src.news.asset_manager, and completion/replacement.py plus assets/visual_preview.py reading assets_manifest.json) are recorded as INFERENCE-class layering evidence and were NOT repaired. The role of the vision_validator stub and of the connected but non-deciding semantic_visual_service - vision_validator has zero callers in production and in tests and no owning test at all, and the vision_validation_enabled flag it mirrors is read by nobody; semantic_visual_service is wired but cannot affect selection, because analyse_semantic_visual_for_project is called from _write_reviews after every scene has already been selected, downloaded and recorded, and it writes its evidence only into assets/review/visual_review_manifest.json, while _selection_fingerprint is a protective self-check raising selection_warning rather than a veto. The already recorded reporting defect - _semantic_visual_summary writing semantic_rerank_enabled False regardless of the real config - was re-confirmed and deliberately left to PLAN-9C. Persisted contracts are recorded, including the three writers of assets/assets_manifest.json (news/pipeline.py:437 through the atomic NewsProjectStore, news/draft_completion.py:256, assets/completion/replacement.py:294). The duplicate/overlap verdict is that no second live owner exists - news/asset_manager is a compatibility facade, its _select_best_candidate is a line-for-line copy of asset_manifest_builder.select_best_with_video kept as a patch point and pinned by test_news_asset_manager_contract.py while the production path does not call it, semantic_score means one thing in candidate_ranker and another in semantic_visual_models, semantic_decision_policy is a real but entirely unwired calibration layer, and vision_validator is an unused stub. C31 was re-verified and is NOT resolved - the production dependency on docs/implementation/openai_live_evaluation still exists at src/assets/semantic_visual_evaluation_tooling.py lines 26, 38 and 695 plus the evaluation tests - so it is recorded as a confirmed defect while no file was moved, no import was changed and no target owner was chosen; the physical target remains PLAN-13 work under OD-8/OD-9 and the per-file classification of docs/implementation remains the documentation-classification work inside PLAN-12. Nothing outside documentation changed - production code, tests, schemas, config, manifests, runtime and user data are untouched, no file was moved or deleted, and no new owner, selector, semantic service, abstraction, persisted field, PLAN-ID or finding ID was created; every defect found was recorded as evidence and none was repaired. Verification of that closure slice - check_agent_docs exit 0, git diff --check clean, check_task_scope OK against the exact docs allowlist, and git diff --name-only listing exactly the five documentation files; the full offline suite was not run and is not required under Execution protocol item 10 because no production contract, test discovery or runner changed. The three routing mirrors were touched only in their current-checkpoint sentence, which is the mechanical consequence of the PLAN-STAB-7 current-routing-integrity contract that check_agent_docs - this slice's own required verification - enforces. current_checkpoint therefore advances to PLAN-9C (semantic decision wiring), which stays pending / not started; this docs-only closure slice does not begin its implementation. Both of PLAN-9C's gates are now down - PLAN-6E completed 2026-08-02 and PLAN-1C′ closed 2026-08-07 - and PLAN-9A's prerequisite chain (PLAN-9B-2 + PLAN-1C′ + PLAN-6E) is likewise complete, but the route recorded in this plan and in PRODUCT_PLAN.md section 11.7 is PLAN-1C′ then PLAN-9C then PLAN-9D then PLAN-9A then PLAN-10A, PLAN-10B, PLAN-10C and PLAN-9E, so PLAN-9C and not PLAN-9A becomes the checkpoint. THE NEXT EXACT ACTION is a separate owner-issued implementation slice for PLAN-9C, in a fresh chat, strictly under its existing PLAN-9C contract - producer to existing semantic consumer wiring in the order provider-ready query, candidates, bounded shortlist, Vision/semantic evidence, the existing semantic ranking, selection - with evidence reaching the existing decision layer BEFORE the choice is made, with no second visual planner, Vision stack, asset pipeline, candidate selector, completion ladder, project state or semantic manifest, with the default configuration unchanged by that slice, and with a mock semantic backend admissible only in wiring tests and never as proof of visual quality; PLAN-9C is now closed, see above. Earlier context - PLAN-9B-3 (query-path cleanup) is CLOSED 2026-08-07. Implementation commit 72221e1861f7c62de01aa09056cfaf6f56ef99a7 retired the superseded query-generation paths after the PLAN-9B-2 expansion ladder was proven and every live caller was migrated to it. All five retirement candidates named in the PLAN-9B-3 contract are now closed - the obsolete GLOSSARY substring matcher (registry C34) whose harmful substring implementation was already retired earlier in commit 141beae under PLAN-9B-1, leaving the current Unicode-aware token/phrase matcher and the GLOSSARY seed vocabulary as the deliberate replacement; the orca topic hardcode _apply_video_first_topic_briefs (C35); legacy_broad_query (C36); the deprecated make_stock_query (C37); and the superseded semantic_selection/query_generator.py (C38) - the last four all retired by this commit and recorded as registry row R01. The wording "four of five closed" is factually wrong and must not be used. The exclusion list _LEGACY_BROAD_QUERIES in src/assets/query_adapter.py is NOT a sixth retirement candidate and was never one - it is a persisted-compatibility guard created by PLAN-9B-1 itself in commit 141beae, it produces no query and only filters the four retired literals out of plans written before the slice, and it is deliberately kept with the exit condition that it is removed once pre-slice persisted plans are no longer read. The reversible retirement mechanism is complete and verified - annotated tag retired/query-paths-2026-08-07 on 1bbfcad (the last commit in which the retired code still existed), a commit body carrying Retired/Reason/Replaced-by/Recovered-from/Salvaged/Exit, registry row R01 added in the same commit, and the external bundle query-paths-2026-08-07.bundle written outside the worktree. The Envato manual query source is preserved - the manual_request path in src/news/asset_manifest_builder.py was migrated from ordered_queries to semantic_scene_queries rather than losing its query source. That implementation commit received an independent review - verdict ACCEPT WITH MINOR, blocking findings 0 - and a green CI run 31195789804 (headSha 72221e1861f7c62de01aa09056cfaf6f56ef99a7, conclusion success); implementer verification recorded targeted 243 OK, expanded radius 209 OK, full offline suite 1780 OK, check_agent_docs exit 0, check_task_scope OK and git diff --check clean. The four review findings are all non-blocking and none is a close blocker - F1 (the semantic_queries assertion in tests/test_youtube_shorts_production_plan.py passes vacuously on an empty list) is a follow-up in the already recorded src/production_plan/** backlog; F2 (semantic_queries is empty for legacy scenes that carry no provider-language evidence, and no production reader of that field was found) is information only, being the intended fail-closed behavior of the PLAN-9B-PRODUCER boundary; F3 (pre-existing - a legacy broad literal can re-enter through the no-brief _latin_terms fallback in src/assets/query_adapter.py) is a follow-up recorded beside the existing C36/R01 compatibility history; and F4 (pre-existing - the Envato consumer cap [:3] applied before provider synthetic completion) is a follow-up in the existing unscheduled candidate ENVATO-CS1. No new PLAN-ID or finding ID was created for any of them and none was repaired in this docs-only closure. current_checkpoint therefore advances to PLAN-1C′ (the capability owner gate for asset/semantic), which stays pending / not started - this docs-only closure slice does not begin its implementation. THE NEXT EXACT ACTION is a separate owner-issued implementation slice for PLAN-1C′, in a fresh chat, strictly under its existing PLAN-1C′ contract - a docs-only ownership inventory that closes C01-SEM by recording, for every module in its declared scope, the canonical owner, the callers, the persisted contract, the duplicates and the tests, and that records the production dependency on docs/implementation/openai_live_evaluation as a defect without moving any file, the physical target owner remaining PLAN-13 work. PLAN-1C′ has no dependencies of its own and is the last remaining gate in front of both PLAN-9A and PLAN-9C; relying on a transitive dependency through PLAN-9B-2 instead of it is forbidden. Earlier context - PLAN-9B-2 (expansion + hardcode migration) is CLOSED 2026-08-07. Implementation commit 66fd2431 delivered a new canonical reusable query expansion owner src/content/visual_planning/expansion.py holding the controlled ladder (exact subject, subject plus action, subject plus action plus place, the entity's other stated names, a wider meaning-preserving context, the same idea as its environment, and the legacy truncation rung) plus the normalized provider-language planning input QueryPlanningInput; produce_brief in src/content/visual_planning/brief.py now seeds the ladder with what the evidence states verbatim and stores at most four provider queries instead of three; scene_to_legacy in src/content/visual_planning/legacy_format.py no longer calls legacy_broad_query and mirrors the ladder into the flat primary_query/alternative_queries fields instead; must_avoid is enforced as part of the query's meaning by consecutive-phrase matching. That implementation commit received an independent review - verdict ACCEPT WITH MINOR, blocking findings 0 - and a green implementation CI run 31164020130 (headSha 66fd2431, full offline suite 1772 tests OK, failures=0, errors=0). The single review finding F1 - _mentions_avoided in src/content/visual_planning/expansion.py compared must_avoid phrases against the query using a raw whitespace split, so punctuation glued to a word (a trailing comma, or a sentence-final period surviving the truncation rung) could let a query past an avoided phrase it should have blocked - was repaired in bounded commit 8c60295 - both sides of the comparison now go through the same existing _PROVIDER_TOKEN_RE-based tokenizer with edge-punctuation stripped, consecutive-phrase matching and case-insensitive behavior are unchanged, and the query text itself is never mutated. That repair commit was independently re-reviewed - verdict ACCEPT, findings 0 - and obtained a green repair CI run 31172361739 (headSha 8c60295). F2 - a must_avoid phrase written in a non-provider language cannot semantically match an English provider-language query without translation - is recorded as a known non-blocking limitation of PLAN-9B-2; no TranslatorService was created and none is planned. Deletion of the retirement candidates named in the PLAN-9B-3 contract (the obsolete GLOSSARY matcher, the orca topic hardcode, legacy_broad_query, the deprecated make_stock_query and the superseded semantic query_generator) stayed out of PLAN-9B-2 scope and was PLAN-9B-3 work; the query adapter's exclusion list _LEGACY_BROAD_QUERIES was named in an earlier draft of this sentence, which was a drafting error - it is a persisted-compatibility guard created by PLAN-9B-1, not a topic hardcode and not a retirement candidate. current_checkpoint advanced to PLAN-9B-3 (query-path cleanup) in that slice, which stayed pending / not started until its own owner-issued implementation slice; it is now closed, see above. Earlier context - the bounded owner-driven stabilization review of completed PLAN-STAB-1..9 - the review named in blocking gate item 8, which has no dedicated PLAN-ID - is finished 2026-08-07 with final verdict CLEAR TO PROCEED TO PLAN-9B-2 and blocking findings 0. It was read-only - it edited nothing, committed nothing and pushed nothing. It confirmed the four required properties - user-output preservation, offline/paid fail-closed behavior, rights safety and unambiguous current routing - and recorded that no preliminary architectural repair is required before PLAN-9B-2. Targeted evidence - tools.qa.check_agent_docs exit 0; permission/routing/governance tests 140 OK; rights/network cross-contract tests 78 OK; and the already confirmed closure CI run 31149780652 (headSha 2186b20c5592a264ab6d100c44eaa6dd664aae91) with the governance step success and the full offline suite 1749 tests OK, failures=0, errors=0. Blocking gate item 8 is satisfied, so the post-audit stabilization gate is passed in full and no new PLAN-ID was created for it. That review recorded the then-outstanding prerequisite - a separate owner-issued implementation prompt for PLAN-9B-2 - which has since been issued and executed, see above. The non-blocking stabilization backlog PLAN-STAB-10 through PLAN-STAB-17 and the review's non-blocking observations stay open and individually do not block PLAN-9B-2. Earlier context - PLAN-STAB-6 (Claude permission hardening) is closed 2026-08-07 - implementation commit 3cedff10 and repair commit b0a3547 (closing review findings F1-F5) are both independently reviewed and accepted (repair re-review verdict ACCEPT WITH MINOR, blocking findings 0), and docs-only retrigger commit 49385dd obtained green CI evidence on GitHub Actions run 31147454618 (headSha 49385dd) - Validate agent docs and skills governance and Run offline unit and integration suite both success, 1749 tests OK, failures=0, errors=0. The earlier GitHub Actions infrastructure incident (run 31123722270 for 3cedff10 cancelled before a runner was assigned with zero steps and zero logs, and no push-triggered run ever created for b0a3547) is recorded as historical infrastructure evidence, not an unresolved problem, and CI success is still not claimed for either of those two SHAs specifically. Blocking gate item 6 is satisfied. current_checkpoint became PLAN-9B-2 (expansion + hardcode migration) in that slice; its own status records that implementation is not started and stays blocked until a separate owner-issued implementation prompt. PLAN-STAB-7 and PLAN-STAB-8 stay closed and item 7 stays satisfied. Delivered in the PLAN-STAB-6 implementation and repair slices - the versioned .claude/settings.json is deny/ask-only with permissions.allow absent, ten confirmed governance paths (AGENTS.md, CLAUDE.md, skills/**, tools/qa/**, .github/workflows/**, docs/current/PROJECT_EXECUTION_PLAN.md, docs/archive/**, docs/handoff/**, .claude/settings.json, .claude/agents/**) require confirmation on Edit and Write while Read stays open, .claude/settings.local.json is denied to the agent for Read/Write/Edit and stays untracked and ignored, thirteen further secret .env.* names are covered for Read/Write/Edit at the root and recursively (and any deny rule that could reach the tracked .env.example is rejected positionally) while the tracked .env.example keeps the PLAN-6D-1 zero-deny-match property, the untrustworthy leading-wildcard rule for media-library migrate --apply (formerly deny) is replaced by six confirmed pipeline.py entrypoint prefixes in ask - a deliberate deny-to-confirmation change, coverage limited to those six spellings, with the runtime confirm_apply contract in src/media_library.py as the actual --apply barrier - destructive Git is split into a deny set (reset --hard, clean, force push, filter-branch, reflog delete/expire, update-ref -d, gc --prune) and an ask set (checkout --, restore, rm, branch -D, worktree remove) per owner decision, network plus package installation require confirmation, and only a rule in the tracked .gitignore proves an ignore exclusion. The contract is validated by validate_claude_permissions in the existing governance QA owner tools/qa/check_agent_docs.py, so the existing CI step covers it and no second QA framework or workflow was created; tests/test_claude_permission_contract.py owns the regressions (47 tests OK). The seven dangerous local grants (git add *, git commit *, python -c, three python.exe -c variants, python -) were removed by the owner by hand before this slice and a read-only precheck confirmed none of them remain. Recorded residual limitations - exact matcher wildcard semantics and bucket precedence are not empirically proven here, Bash is not path-restricted so global Git options, shell aliases and an arbitrary interpreter remain out of contract, the enumerated .env.* coverage is deliberately incomplete, and the effective merged user/managed/local configuration lives outside the repository and is not claimed to be protected. Blocking gate item 6 is satisfied.
...
# policy application through draft completion; retrieval symmetry remains next.
source_paths:
  - AGENTS.md
  - pyproject.toml
  - requirements.txt
...
- **status:** in progress · owner-issued 2026-08-10 (plan reconciliation после
  LIVE-4 и comparative audit) · первый bounded sub-slice (unified policy)
  выполнен 2026-08-10 commit `388b9b1`; blocking repair
  **PLAN-9C-2-B1** завершил применение policy через draft completion —
  **этот repair commit**; второй bounded
  sub-slice (retrieval symmetry) — pending, отдельный owner prompt.
...
- **retrieval symmetry — отдельный immediately-following bounded sub-slice
  того же PLAN-ID (второй commit).** `search_provider`
  (`src/news/asset_provider_adapters.py:98`) при preferred=image не запрашивает
  видео вовсе, тогда как preferred=video запрашивает оба вида. Владельцы разные
  (retrieval-адаптер против decision-слоя), поэтому в первый policy-commit не
  входит; без него честный AUTO для image-сцен неполон.
- **не входит:** metadata-evidence repair (**PLAN-9C-3**); shortlist/dedup/
  ...
  права.
- **required verification:** красные-сначала characterization tests на пять
  acceptance cases; B1 draft-completion regressions; owning radius
  selection/manifest; полный offline suite (меняется production selection
  contract). Для policy implementation и B1 repair выполнено; retrieval
  symmetry сохраняет собственную проверку в следующем commit.
- **rollback:** policy implementation commit + B1 repair commit (+ будущий
  retrieval-symmetry commit), revert в обратном порядке; migраций данных нет.
