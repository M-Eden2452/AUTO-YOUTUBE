...
- **Текущий шаг:** **PLAN-9D — offline visual-quality evidence.**
  ...
  **PLAN-9D-B** (current-HEAD retrieval capture) в отдельном новом чате,
  строго по его contract; требует собственного explicit owner network
  authorization для классов `provider_search` и `preview_download`
  (`src/runtime_network.py`). PLAN-9D-B этим docs closure **не запускается**.
...
- **status:** in progress · **commit:** `04fe035` (benchmark harness и
  ...
  **PLAN-9B** закрыт через свои под-слайсы PLAN-9B-2 (`66fd2431`/`8c60295`) и
  ...
  findings 0). Шаг разбит на под-слайсы PLAN-9D-A…PLAN-9D-G по Execution
  protocol п.5; каждый требует собственного owner-issued implementation slice.
...
- **status:** next · PLAN-9D-A снят (`2bae6f6`), остаётся blocked на отдельный
  explicit owner network approval для классов `provider_search` и
  `preview_download` (`src/runtime_network.py`) — это разрешение не выдаётся
  этим docs-only closure и PLAN-9D-B им не запускается · **commit:** —
- **цель:** снять свежий candidate corpus одним bounded прогоном текущего
  production-пути: visual planning → current query planning → current free
  providers → current candidate pools → previews/frames.
- **классификация:** DATA CAPTURE / EVALUATION PREPARATION. Это не
  implementation PLAN-10A, PLAN-10B, PLAN-10C или PLAN-10D.
- **network boundary:** требуются классы `provider_search` и
  `preview_download` существующего владельца `src/runtime_network.py`
  (default deny). Разрешение выдаётся владельцем на конкретное действие;
  наличие настроенного провайдера разрешением не является. Keyless-провайдеры
  автоматического набора (`wikimedia`, `nasa_images`, `internet_archive`)
  платными не являются; `pexels` и `pixabay` создаются только при наличии
  ключа. Paid Vision, TTS и render в capture не входят.
- **запрещено:** менять состав провайдеров, pagination, пороги и ranking;
  повторные capture-прогоны ради лучшего результата.
- **required verification:** targeted capture/contract tests; корпус
  замораживается своим `corpus_sha256`.

#### PLAN-9D-C — retrieval quality gate

- **status:** blocked (PLAN-9D-B) · **commit:** —
