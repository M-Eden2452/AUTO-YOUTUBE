...
- **Текущий шаг:** **PLAN-9D — offline visual-quality evidence.**
  ...
  **Уточнено owner decision 2026-08-09:** до отдельного owner decision PLAN-9D-D
  ...
  здесь не выполнялась. `current_checkpoint` не менялся. Следующее точное
  действие — independent read-only review именно того implementation commit,
  затем свежий diagnostic русский Short, и только после него — owner decision о
  возврате к PLAN-9D-D.
...
- **rollback:** implementation commit плюс два bounded repair commit; revert в
  обратном порядке. Миграций данных, новых artifact/layout и необратимых
  действий нет.
- **next:** повторный прогон **того же самого** real Russian diagnostic Short
  как **live-3**, отдельным owner-issued действием, чтобы измерить, сколько
  ответов принимается после выравнивания контракта, исчезает ли деградация
  `scene_004` и как ведёт себя отбор на корректно маршрутизированных родовых
  сценах. Отдельный independent review ни для одного из двух repair не
  назначался: они не меняют сеть, платные gate, авторитет selection, persisted
  schema и права. **PLAN-9D-D** остаётся NOT STARTED / blocked.
