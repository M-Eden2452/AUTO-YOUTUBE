...
- **verification.** Сначала красный (`ModuleNotFoundError`), затем
  ...
  **не выполнялись**.
- **rollback:** один commit; revert. Миграций данных, новых artifact/layout и
  необратимых действий нет.
- **next:** independent read-only review именно этого activation commit. При
  отсутствии BLOCKING — real diagnostic Short отдельным owner-issued действием.
