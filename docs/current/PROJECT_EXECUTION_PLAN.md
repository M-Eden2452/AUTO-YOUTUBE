...
- **Текущий шаг:** **PLAN-9D — offline visual-quality evidence.**
  ...
  **PLAN-9D-A** (historical evidence curation) в отдельном новом чате, строго
  по его contract: перевести существующий `corpus_v1.json` в явно названный
  historical failure fixture и сжать его, сохранив harness. Никакой сети,
  провайдеров, Vision и платных вызовов в этом под-слайсе.
...
**Оговорка PLAN-9D evidence (добавлено owner direction 2026-08-08).** Пока
PLAN-9D открыт, кадры превью под `projects/`, на которые ссылается
замороженный корпус PLAN-9D, disposable **не считаются**, хотя формально
подпадают под `*.jpeg` и «кэши» выше: `tests/data/plan9d/corpus_v1.json`
адресует 107 таких путей и хранит для каждого размер и SHA256. Оговорка
снимается автоматически на шаге (4) cleanup sequencing PLAN-9D — после того
как historical evidence dependency снята, то есть historical failure evidence
сжато (PLAN-9D-A), current corpus снят и заморожен (PLAN-9D-B) и ground truth
заморожен (PLAN-9D-D). До этого удаление выполняется только по явному owner
approval на конкретный путь. Отдельный PLAN-ID и вторая строка
`Preserved runtime corpus` не создаются: это ограничение порядка, а владелец
порядка — этот файл.
...
- **[FACT] evidence dependency, которую обязан снять шаг (4).** Замороженный
  `corpus_v1.json` ссылается на 107 путей кадров внутри `projects/`, а раздел
  «Safety boundaries» относит `*.jpeg` и кэши под `projects/` к disposable и
  не перечисляет эти кадры в `Preserved runtime corpus`. До закрытия PLAN-9D
  они защищены оговоркой в «Safety boundaries», см. там же.
...
- **status:** partially delivered · **commit:** `04fe035` (частично).
- **что уже есть в `04fe035`:** benchmark harness (`tests/plan9d_ground_truth.py`,
  `tests/plan9d_corpus_builder.py`, `tests/test_plan9d_ground_truth_baseline.py`),
  blind annotation pack, `assert_admissible_evidence` против fixture-backends и
  вызов production `select_best_with_video` → `select_best_candidate` без
  собственного selector. Это переиспользуемая основа, она сохраняется.
- **что остаётся:** перевести `corpus_v1.json` из роли current benchmark в
  явно названный **historical failure fixture**, показывающий retired
  subject-free retrieval и compatibility поведение, и сжать его до
  небольшого набора. Внутренний идентификатор `plan_step: "PLAN-9D-A"` в
  данных корпуса определён этой секцией — до неё он в плане отсутствовал.
- **запрещено:** удалять или перезаписывать historical evidence до шага (4)
  cleanup sequencing; выдавать historical corpus за current-quality benchmark.
- **required verification:** targeted evaluation tests.

#### PLAN-9D-B — current-HEAD retrieval capture

- **status:** blocked (PLAN-9D-A + отдельный owner network approval) ·
  **commit:** —
