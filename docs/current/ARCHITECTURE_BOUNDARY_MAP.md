...
Это результат read-only инвентаризации этапа 4.6, а не разрешение на массовое
перемещение файлов.
...
| `src.news.NewsProjectStore` | news pipeline, service, voice CLI, replacement | news models/pipeline, service, renderer, repository tests | сохранить `job.json` writer; перевести на существующий atomic primitive в 5A |
...
|---|---|---|---|
| `job.json` + stage state | `NewsProjectStore`; `ProjectRepository` reader | старые отсутствующие поля принимает `NewsJob.from_dict`; явной top-level schema version пока нет | не переименовывать и не мигрировать массово; добавить version только additive slice |
