...
**Уточнение, которое сужает будущую работу: слепы не оба локальных пути, а
один.** Проверка курируемой медиатеки через provider-contract
(`LocalLibraryStockProvider.search`, фильтр по пересечению токенов):

| Запрос | Что вернулось |
|---|---|
| `водопад` | `15504939`, `178732`, `37088` — все три водопада |
| `тропический лес река` | `18922150`, `26202145`, `27682644` |
| `солнечная панель` | `13198260`, `27009031`, `32386564` — все солнечные |
| `waterfall` | те же три водопада |
| `orca` | **NO RESULTS** — корректный отказ |

Тот же `orca` на legacy-пути (`search_local_assets` → `asset_manifest_builder`)
уверенно отдаёт солнечную электростанцию. То есть русские запросы теперь
работают (ключевые слова записаны на обоих языках), провайдерский путь честно
воздерживается, а чинить нужно именно `_score_asset`: его `min_score`
набирается без единого совпадения по смыслу. Порядок внутри провайдера тоже
стоит посмотреть отдельно — он сортирует по вертикальности и числу пикселей, а
не по релевантности, и релевантность остаётся обязанностью downstream-ранкера.
